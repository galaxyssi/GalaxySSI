...
| MCP | Implemented core, real-device acceptance pending | `AgentMcpSession.kt` and `AgentMcpRuntime.kt` provide bounded Streamable HTTP and declarative HTTP clients; `AgentMcpLocalRuntime.kt` runs reviewed local stdio packages through the Android Linux sandbox; credentials stay in encrypted storage and MCP calls remain behind native policy | Complete real-device local Python/Node MCP installation, process-death, cancellation, schema-change, and paired Desktop interoperability acceptance |
