"""Per-task allowlisted HTTP and HTTPS proxy for the SignalASI Linux guest."""
...
class AllowlistedHttpProxy:
    ...
    def start(self) -> ProxyEnvironment:
        ...
        proxy_url = f"http://signalasi:{self.token}@127.0.0.1:{port}"
        gradle_options = " ".join(
            (
                ...
                "-Dhttp.proxyUser=signalasi",
                f"-Dhttp.proxyPassword={self.token}",
                "-Dhttps.proxyUser=signalasi",
    ...
    def authorize(self, supplied: str) -> bool:
        expected = base64.b64encode(f"signalasi:{self.token}".encode("utf-8")).decode("ascii")
