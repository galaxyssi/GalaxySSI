...
const val AGENT_SELF_EVOLUTION_PROTOCOL = "signalasi.evolution-task.v1"
...
class EncryptedAgentSelfEvolutionStore(context: Context) : AgentSelfEvolutionStore {
    ...
    private companion object {
        const val DATABASE = "signalasi_self_evolution_v1"
...
class EncryptedAgentRemoteSelfEvolutionStore(context: Context) {
    private val database = AgentEncryptedDatabase(
        ...
        "signalasi_remote_self_evolution_v1"
    )
...
class AndroidAgentSelfEvolutionRuntime(
    ...
    override fun prepare(
        ...
        val response = execute(
            ...
            """
                ...
                if ! command -v git >/dev/null 2>&1; then
                  echo "SIGNALASI_ERROR=git_unavailable" >&2
                  exit 71
                fi
                rm -rf source
        ...
        if (response.exitCode != 0) {
            val missingGit = "SIGNALASI_ERROR=git_unavailable" in response.stderr
            throw AgentSelfEvolutionException(
        ...
        val base = response.marker("SIGNALASI_BASE")
    ...
    override fun applyPatch(
        ...
        val response = execute(
            ...
            """
                ...
                } | sed '/^$/d' | sort -u | sed 's/^/SIGNALASI_CHANGED=/'
            """.trimIndent(),
        ...
        return AgentSelfEvolutionPatchResult(
            response.stdout.lineSequence()
                .filter { it.startsWith("SIGNALASI_CHANGED=") }
        ...
        if (changedFiles.any { it.startsWith("apps/desktop/") || it.startsWith("core/") }) {
            gates += runGate(
                ...
                """
                    ...
                    python3 -m unittest discover -s core/signalasi-link/backend -p 'test_*.py'
    ...
    override fun commit(
        ...
        val response = execute(
            ...
            """
                ...
                cd source
                git add --all
                git -c user.name='SignalASI Evolution' -c user.email='signalasi@hotmail.com' \
                  commit -m 'Prepare evolution candidate ${task.taskId}'
                printf 'SIGNALASI_COMMIT=%s\n' "${'$'}(git rev-parse HEAD)"
            """.trimIndent(),
        ...
        val commit = response.marker("SIGNALASI_COMMIT")
    ...
    private companion object {
        const val OFFICIAL_REPOSITORY = "https://github.com/signalasi/SignalASI.git"
