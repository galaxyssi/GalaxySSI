...
object AgentMobileProjectArchiveTools {
    const val IMPORT_PROJECT = "signalasi.project.archive.import"
    const val IMPORT_GRADLE_CACHE = "signalasi.project.gradle_cache.import"
    ...
    fun definitions(context: Context): List<AgentNativeToolDefinition> {
        ...
        return listOf(
            definition(
                ...
                description = "Imports a tar.gz project staged for SignalASI into the current Android-local Linux project workspace. Shared Download aliases are resolved by the App; the guest never receives unrestricted shared-storage access."
            ) { invocation ->
    ...
    private fun definition(
        ...
        descriptor = AgentNativeToolDescriptor(
            ...
            requiredPermissions = listOf(
                AgentNativePermissionRequirement(
                    ...
                    "Limits imported project data to the current SignalASI workspace"
                )
            ),
            timeoutMillis = 30 * 60_000L,
        ...
        executorId = "signalasi.android_project_archive",
    ...
    internal val PROJECT_IMPORT_SCRIPT = """
        ...
        archive = root / '.signalasi-inputs' / 'project.tar.gz'
        stage = root / '.signalasi-import-stage'
        ...
        for child in root.iterdir():
            if child.name.startswith('.signalasi-'): continue
    ...
    internal val GRADLE_CACHE_IMPORT_SCRIPT = """
        ...
        root = pathlib.Path.cwd().resolve()
        archive = root / '.signalasi-inputs' / 'gradle-modules-2.tar.gz'
...
internal class AgentProjectArchiveSourceResolver(context: Context) {
    private val root = File(context.getExternalFilesDir(null), "SignalASI").canonicalFile.apply {
        check(mkdirs() || isDirectory) { "SignalASI archive staging is unavailable" }
    }
    ...
    fun resolve(requestedPath: String): File {
        ...
        val candidate = if (
            normalized.startsWith("/sdcard/Download/SignalASI/") ||
            normalized.startsWith("/storage/emulated/0/Download/SignalASI/")
        ) {
        ...
        require(candidate.path.startsWith(root.path + File.separator)) {
            "Archive must be staged in SignalASI's app-specific import directory"
        }
        require(candidate.isFile && candidate.canRead()) {
            "Archive is not staged for SignalASI: ${candidate.name}"
        }
