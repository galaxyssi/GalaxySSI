...
class AgentFailureMemoryStore(context: Context) {
    ...
    private companion object {
        const val DATABASE = "signalasi_agent_failure_memory_v1"
...
object AgentSkillMarkdownCodec {
    private const val JSON_BLOCK_START = "```signalasi-skill-json"
...
object AgentSkillMarkdownSigner {
    fun sign(markdown: String): String {
        ...
        return JSONObject()
            .put("format", "signalasi-signed-skill-md-v1")
            .put("skill_md", payload.toString(Charsets.UTF_8))
            .put("signer_fingerprint", SignalASICrypto.localIdentitySha256())
            .put("signer_public_key", SignalASICrypto.localIdentityPublicKey())
            .put("signature", SignalASICrypto.signLocalIdentity(payload))
            .toString()
    }

    fun inspect(raw: String): AgentSkillMarkdownInspection? {
        val envelope = runCatching { JSONObject(raw) }.getOrNull()
        val signed = envelope?.optString("format") == "signalasi-signed-skill-md-v1"
        ...
        val valid = SignalASICrypto.verifyPublicIdentitySignature(
