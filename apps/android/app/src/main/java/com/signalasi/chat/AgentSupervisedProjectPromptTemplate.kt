...
internal object AgentSupervisedProjectPromptTemplate {
    ...
    private fun StringBuilder.appendCompactContinuationContract() {
        ...
        append("Runtime: signalasi.workspace.* handles bounded files; signalasi.runtime.* handles phone Linux dependencies, builds, tests, and artifacts. Commands start in the project; use relative paths, never guessed /workspace or /root paths. If Linux is required, only a successful signalasi.runtime.execute receipt proves it. ")
    ...
    private fun StringBuilder.appendInitialPlanningContract() {
        ...
        append("Batch reads/searches; search large files; use start_line/max_lines. Reuse known_sha256 for identical ranges to omit unchanged text. Change failed approaches; use task-aware timeouts. test/build/lint/package requires host receipt. ")
