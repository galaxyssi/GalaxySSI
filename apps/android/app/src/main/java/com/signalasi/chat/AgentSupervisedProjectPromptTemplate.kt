...
internal object AgentSupervisedProjectPromptTemplate {
    ...
    private fun StringBuilder.appendCompactContinuationContract() {
        ...
        append("Runtime starts in project; use relative paths. Workspace edits; runtime executes. Reuse clone/observe/inspect project_profiles; do not reread manifests. For test/build/lint/package call signalasi.runtime.execute with verification_kind and no source; project_scope selects a monorepo child, and custom source is recovery only. ")
        ...
        append("Delivery requires verification and, unless local-only, a feature branch, tests, commit, push, and pull-request URL. After final diff review and verification use signalasi.project.github.pull_request.finalize; separate publication tools are recovery only. Put deliverables in artifact_paths; inspection needs none. Never request approval. When fully verified return one DRAFT_PLAN task-complete action with a grounded summary; otherwise return the next corrective, verification, or delivery action. ")
    }
    ...
    private fun StringBuilder.appendInitialPlanningContract() {
        ...
        append("Clone/observe/inspect project_profiles are host-derived project roots and native verification commands; reuse them instead of listing directories or rereading manifests unless a concrete missing detail requires it. Inspect runtime status and real output before installing dependencies. Persistent phone Linux uses Debian apt/dpkg as root with direct network access for apt, Git, curl/wget, language package managers, and browser automation. Install the smallest evidence-backed missing package or trusted signed runtime pack, then retry the exact blocked step and verify it. Package installation alone is never completion evidence. ")
        ...
        append("Delivery: branch before editing; verify before publication. Unless local-only, changes require tests, commit, push, and a GitHub pull request URL. After a verified final diff use signalasi.project.github.pull_request.finalize; separate tools are recovery only. For a documentation-only change, bounded repository.diff inspection is sufficient verification. Put requested deliverables in artifact_paths as one verified ZIP. Do not require an artifact for repository clone or observation. Android: signed java/gradle/android-sdk packs; build in phone Linux, verify Gradle, return APK. Never request approval or unverified completion. ")
    }
