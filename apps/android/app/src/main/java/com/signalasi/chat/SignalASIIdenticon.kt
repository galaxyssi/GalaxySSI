...
internal data class SignalASIIdenticonPattern(
...
internal object SignalASIIdenticon {
    ...
    fun fromIdentityFingerprint(fingerprint: String): SignalASIIdenticonPattern {
        val normalized = fingerprint.trim().lowercase().ifBlank { "signalasi-local-identity" }
        val digest = MessageDigest.getInstance("SHA-256")
            .digest(normalized.toByteArray(Charsets.UTF_8))
        val cells = MutableList(SignalASIIdenticonPattern.GRID_SIZE * SignalASIIdenticonPattern.GRID_SIZE) { false }
        ...
        for (row in 0 until SignalASIIdenticonPattern.GRID_SIZE) {
            for (sourceColumn in 0..2) {
                ...
                cells[row * SignalASIIdenticonPattern.GRID_SIZE + sourceColumn] = filled
                cells[row * SignalASIIdenticonPattern.GRID_SIZE + (4 - sourceColumn)] = filled
            }
        }
