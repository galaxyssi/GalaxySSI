...
internal object SignalASIDeviceIdentityName {
    private const val MAX_DEVICE_NAME_LENGTH = 48

    fun current(context: Context): String = format(
        deviceName = resolveDeviceName(context),
        signalasiId = SignalASICrypto.localSignalasiId()
    )

    fun format(deviceName: String, signalasiId: String): String {
        val normalizedName = deviceName
            ...
            .trim()
            .ifBlank { "Android" }
        val identity = signalasiId.substringAfter(':', signalasiId)
