...
internal object DesktopPairingLifecycle {
    fun belongsToDesktop(contact: JSONObject, desktopId: String): Boolean {
        ...
        val contactId = contact.optString("id").ifBlank {
            contact.optString("signalasi_id").ifBlank { contact.optString("hermes_id") }
        }
    ...
    fun remove(context: Context, desktopId: String): DesktopPairingRemoval {
        ...
        SignalASIMqttClient.unsubscribeServer(appContext, cleanDesktopId)
        SignalASICrypto.clearDesktopTrust(appContext, cleanDesktopId)
        SignalASILinkProtocol.removeServer(appContext, cleanDesktopId)
