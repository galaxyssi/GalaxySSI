...
import com.signalasi.chat.voice.model.WhisperDownloadDecision
import com.signalasi.chat.voice.model.WhisperLegacyMigration
import com.signalasi.chat.voice.model.WhisperLegacyMigrationState
import com.signalasi.chat.voice.model.WhisperModelCatalog
import com.signalasi.chat.voice.model.WhisperModelDownloadPolicy
import com.signalasi.chat.voice.model.WhisperModelInstallException
import com.signalasi.chat.voice.model.WhisperModelInstallFailure
import com.signalasi.chat.voice.model.WhisperModelProfile
import com.signalasi.chat.voice.model.WhisperModelStorage
...
object WhisperModelManager {
    private const val PREFS = "signalasi_whisper_models_v2"
    ...
    private const val PARTIAL_ROOT = "signalasi-asr-partials"
    ...
    private val background = Executors.newSingleThreadExecutor { runnable ->
        Thread(runnable, "SignalASI-WhisperModels").apply { isDaemon = true }
    }
            ...
            if (!verification.valid) {
                ...
                throw WhisperModelInstallException(
                    when (verification.failure) {
                        com.signalasi.chat.voice.model.WhisperVerificationFailure.SIZE_MISMATCH ->
                            WhisperModelInstallFailure.SIZE_MISMATCH
                        com.signalasi.chat.voice.model.WhisperVerificationFailure.SHA256_MISMATCH ->
    ...
    private fun legacyCandidates(context: Context, model: WhisperModelProfile): List<File> = listOfNotNull(
        context.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS)?.let { File(it, "signalasi-asr/${model.fileName}") },
        File(context.filesDir, "signalasi-asr/${model.fileName}"),
