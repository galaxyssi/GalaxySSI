...
internal object PhoneRelationshipIdentityBinding {
    ...
    fun deriveLinkSecret(
        ...
        return b64Url(hmac(sharedSecret, "signalasi-phone-link-v3\u0000$binding".toByteArray()))
    }
    ...
    fun deriveRouteId(
        ...
        require(SignalASILinkProtocol.validLinkSecret(linkSecret)) { "Invalid phone link secret" }
        val binding = canonicalBinding(firstFingerprint, secondFingerprint)
        val key = Base64.getUrlDecoder().decode(linkSecret)
        return b64Url(
            hmac(key, "signalasi-phone-route-v3\u0000$binding".toByteArray()).copyOf(16)
        )
    }
