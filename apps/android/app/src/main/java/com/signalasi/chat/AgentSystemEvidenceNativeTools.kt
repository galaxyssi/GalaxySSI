...
object AgentSystemEvidenceNativeTools {
    const val DEVICE_INFO = "signalasi.system.device.info"
    const val APP_INFO = "signalasi.system.app.info"
    const val LOCAL_TIME = "signalasi.system.time.local"
    const val JSON_VALIDATE = "signalasi.data.json.validate"
    ...
    fun definitions(context: Context): List<AgentNativeToolDefinition> {
        ...
        return listOf(
            ...
            readOnlyDefinition(
                id = APP_INFO,
                title = "Read SignalASI application version",
                description = "Returns this installed SignalASI build's package, version name and version code.",
                inputSchema = AgentNativeJsonSchema.objectSchema(additionalProperties = false)
            ) {
                AgentNativeToolExecutionResult.success(output = linkedMapOf(
    ...
    private fun readOnlyDefinition(
        ...
        executorId = "signalasi.android.system_evidence"
    )
}
