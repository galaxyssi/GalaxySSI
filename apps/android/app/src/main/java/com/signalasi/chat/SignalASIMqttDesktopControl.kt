...
internal object SignalASIMqttDesktopControl {
    ...
    fun publishServerRevocation(context: android.content.Context, desktopId: String): Boolean {
        ...
        return SignalASIMqttClient.publishDesktopControlPayload(
            desktopId,
            JSONObject()
    ...
    fun publishToolCall(desktopId: String, payload: JSONObject): Boolean =
        SignalASIMqttClient.publishDesktopControlPayload(desktopId, payload)

    fun publishRemoteWhisperPacket(desktopId: String, payload: JSONObject): Boolean =
        SignalASIMqttClient.publishDesktopControlPayload(desktopId, payload, durable = true)

    fun publishExecutorRequest(
        desktopId: String,
        payload: JSONObject,
        durable: Boolean
    ): Boolean = SignalASIMqttClient.publishDesktopControlPayload(desktopId, payload, durable)

    fun requestAuthorizations(desktopId: String): Boolean =
        SignalASIMqttClient.publishDesktopControlPayload(
            desktopId,
            JSONObject()
    ...
    fun createEvolutionTask(
        ...
        maxAttempts: Int,
        agentId: String
    ): Boolean = SignalASIMqttClient.publishDesktopControlPayload(
        desktopId,
        JSONObject()
    ...
    fun requestArtifactDownload(block: AgentRichBlock): Boolean {
        ...
        val accepted = SignalASIMqttClient.publishDesktopControlPayload(
            link.desktopId,
            JSONObject()
