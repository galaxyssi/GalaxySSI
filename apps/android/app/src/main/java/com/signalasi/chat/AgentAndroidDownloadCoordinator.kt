...
internal object AgentAndroidDownloadPolicy {
    ...
    private val genericTitles = setOf("signalasi download", "download")
    ...
    fun relativePath(fileName: String): String = "Download/SignalASI/$fileName"

    fun startedMessage(chinese: Boolean): String = if (chinese) {
        "\u5df2\u5f00\u59cb\u4e0b\u8f7d\u3002\u5b8c\u6210\u540e\u6587\u4ef6\u5c06\u4fdd\u5b58\u5230 Download/SignalASI\uff0c\u5e76\u663e\u793a\u5728\u5f53\u524d\u4f1a\u8bdd\u4e2d\u3002"
    } else {
...
internal class AgentAndroidDownloadStore(context: Context) {
    ...
    private companion object {
        const val DATABASE_NAME = "signalasi_android_downloads_v1"
    }
}
...
internal object AgentAndroidDownloadCoordinator {
    fun track(
        ...
        thread(name = "signalasi-download-race-$downloadId", isDaemon = true) {
            Thread.sleep(750L)
...
class AgentAndroidDownloadReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val pending = goAsync()
        thread(name = "signalasi-download-complete-$downloadId", isDaemon = true) {
            try {
