...
class AgentMemoryPssMonitor(
    ...
    private fun sampleFor(
        ...
        val agentId = workspace?.agentId.orEmpty().ifBlank {
            if (workspace == null) "" else "galaxyssi-mobile"
        }
    ...
    companion object {
        ...
        fun providerIdForAgent(agentId: String): String {
            ...
            return when (clean) {
                "galaxyssi-mobile", "mobile", "on-device" -> "on-device"
...
object AgentMemoryPssRuntime {
    private val executor = Executors.newSingleThreadScheduledExecutor { task ->
        Thread(task, "GalaxySSI-Agent-Memory-PSS").apply { isDaemon = true }
    }
    ...
    fun start(context: Context, activeWorkspaces: () -> List<AgentWorkspace>) {
        ...
        executor.schedule({
            ...
            initialized
                ...
                .onFailure { error ->
                    android.util.Log.w("GalaxySSIStartup", "agent_memory_pss_init_failed", error)
                }
