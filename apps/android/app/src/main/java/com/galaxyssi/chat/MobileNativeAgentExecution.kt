...
internal fun MobileNativeAgent.submitGoal(
    ...
    return try {
        if (!startExecutionLoop(activeConversationTurnId)) return snapshot()
        Log.i(
            "GalaxySSILatency",
...
internal fun MobileNativeAgent.executeSubmittedGoal(): AgentUiState {
    ...
    return beginInitialPlanning(planningStartedAt)
}
...
internal fun MobileNativeAgent.pauseCurrentTask(): AgentUiState {
    if ((currentPlan == null && pendingPlanning == null) ||
...
internal fun MobileNativeAgent.resumeCurrentTask(): AgentUiState {
    ...
    if (savedPlan == null && pendingPlanning != null) return resumeInitialPlanning()
