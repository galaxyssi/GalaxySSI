...
internal const val PHONE_DEVELOPMENT_MANIFEST_PARAMETER = "_galaxyssi_phone_development_manifest"
internal const val PHONE_DEVELOPMENT_FILE_PARAMETER = "_galaxyssi_phone_development_file"
internal const val PHONE_DEVELOPMENT_ERROR_PARAMETER = "_galaxyssi_phone_development_error"
...
internal object AgentPhoneDevelopmentPolicy {
    ...
    fun planningPrompt(goal: String): String = buildString {
        append("Act only as a code author for GalaxySSI's Android-local Linux runtime. ")
        ...
        append("Schema: {\"schema\":\"galaxyssi.phone-development-manifest.v2\",")
    ...
    fun repairPrompt(
        ...
        append("Repair a program that failed verification in GalaxySSI's Android-local Linux runtime. ")
        ...
        append("using schema galaxyssi.phone-development-manifest.v2 and no markdown or commentary. ")
    ...
    private const val PHONE_WORKSPACE_TOOL_PREFIX = "galaxyssi.workspace."
    private const val PHONE_PROJECT_TOOL_PREFIX = "galaxyssi.project."
    private const val PHONE_RUNTIME_SOFTWARE_TOOL_PREFIX = "galaxyssi.runtime.software."
...
internal data class AgentPhoneDevelopmentManifest(
    ...
    fun runtimeInput(): JSONObject {
        ...
        val wrappedSource = buildString {
            ...
            append("_galaxyssi_files = json.loads(base64.b64decode(")
                .append(JSONObject.quote(filesPayload)).append(").decode(\"utf-8\"))\n")
            append("for _galaxyssi_item in _galaxyssi_files:\n")
            append("    _galaxyssi_path = Path(_galaxyssi_item[\"path\"])\n")
            append("    _galaxyssi_path.parent.mkdir(parents=True, exist_ok=True)\n")
            append("    _galaxyssi_path.write_bytes(base64.b64decode(_galaxyssi_item[\"data\"]))\n")
...
internal object AgentPhoneDevelopmentManifestCodec {
    ...
    private const val SCHEMA = "galaxyssi.phone-development-manifest.v2"
    private const val LEGACY_SCHEMA = "galaxyssi.phone-development-manifest.v1"
...
internal const val PHONE_DEVELOPMENT_REQUIRED_PACKS_PARAMETER = "_galaxyssi_phone_development_required_packs"
...
internal fun AgentPlan.withPhoneDevelopmentPackInstalls(
    ...
    val installActions = missingPacks.mapIndexed { index, packId ->
        AgentAction(
            ...
            kind = AgentActionKind.CALL_NATIVE_TOOL,
            target = "GalaxySSI runtime package manager",
