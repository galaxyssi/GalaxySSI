...
class AgentAndroidHardwarePlatformFacade(
    ...
    override val implementationId: String = "galaxyssi.android.hardware_facade"
    ...
    override fun sampleSensor(
        ...
        val thread = HandlerThread("GalaxySSI-OneShotSensor").apply { start() }
    ...
    override fun discoverBluetooth(
        ...
        if (!adapter.isEnabled) {
            throw AgentHardwareNativeException(
                "bluetooth_disabled",
                "Bluetooth is disabled; GalaxySSI will not change that setting",
                retryable = true
            )
        }
    ...
    private fun currentLocation(
        ...
        val thread = HandlerThread("GalaxySSI-OneShotLocation").apply { start() }
...
object AgentHardwareNativeTools {
    const val BATTERY_STATUS = "galaxyssi.hardware.battery.status"
    const val POWER_STATUS = "galaxyssi.hardware.power.status"
    const val MEMORY_STATUS = "galaxyssi.hardware.memory.status"
    const val STORAGE_STATUS = "galaxyssi.hardware.storage.status"
    const val NETWORK_STATUS = "galaxyssi.hardware.network.status"
    const val LOCATION_FOREGROUND_READ = "galaxyssi.hardware.location.foreground.read"
    const val SENSORS_LIST = "galaxyssi.hardware.sensors.list"
    const val SENSOR_SAMPLE = "galaxyssi.hardware.sensor.sample"
    const val FLASHLIGHT_SET = "galaxyssi.hardware.flashlight.set"
    const val BLUETOOTH_STATUS = "galaxyssi.hardware.bluetooth.status"
    const val BLUETOOTH_DISCOVERY_FOREGROUND = "galaxyssi.hardware.bluetooth.discovery.foreground"
    const val BLUETOOTH_PAIRING_HANDOFF = "galaxyssi.hardware.bluetooth.pairing.handoff"
    const val NFC_STATUS = "galaxyssi.hardware.nfc.status"
    const val INSTALLED_APPS_LIST = "galaxyssi.hardware.apps.installed.list"
    const val PACKAGE_DETAIL = "galaxyssi.hardware.apps.package.detail"
    ...
    const val FOREGROUND_LOCATION_CONSENT = "galaxyssi.consent.location.foreground_once"
    const val SENSOR_SAMPLE_CONSENT = "galaxyssi.consent.sensor.foreground_once"
    const val FLASHLIGHT_CONTROL_CONSENT = "galaxyssi.consent.flashlight.control"
    const val BLUETOOTH_DISCOVERY_CONSENT = "galaxyssi.consent.bluetooth.discovery.foreground_once"
    const val BLUETOOTH_PAIRING_HANDOFF_CONSENT = "galaxyssi.consent.bluetooth.pairing_handoff"
    const val INSTALLED_APPS_CONSENT = "galaxyssi.consent.installed_apps.query_visible"
    const val PACKAGE_DETAIL_CONSENT = "galaxyssi.consent.package_detail.query_visible"
    ...
    private const val EXECUTOR_ID = "galaxyssi.android.hardware_native"
    ...
    private fun bluetoothPairingHandoffDefinition(platform: AgentHardwarePlatformFacade) = definition(
        ...
        descriptor = descriptor(
            ...
            description = "Hands pairing to Android Settings; GalaxySSI neither pairs silently nor claims the user completed pairing.",
            outputSchema = objectSchema(
                properties = mapOf(
                    "launched" to AgentNativeJsonSchema.boolean(),
                    "action" to AgentNativeJsonSchema.string(maxLength = 160),
                    "completed" to AgentNativeJsonSchema.boolean(),
                    "settings_changed_by_galaxyssi" to AgentNativeJsonSchema.boolean()
                ),
                required = setOf("launched", "action", "completed", "settings_changed_by_galaxyssi")
            ),
            risk = AgentNativeToolRisk.HIGH,
            capabilities = setOf(
        ...
        execute = {
            ...
            AgentNativeToolExecutionResult.success(
                output = mapOf(
                    ...
                    "completed" to false,
                    "settings_changed_by_galaxyssi" to false
                ),
        ...
        execute = { invocation ->
            ...
            AgentNativeToolExecutionResult.success(
                ...
                message = if (detail.visible) {
                    ...
                    "Package is not visible to GalaxySSI; installation state is unknown"
                },
                metadata = mapOf("android_package_visibility_filtered" to true)
            )
        }
