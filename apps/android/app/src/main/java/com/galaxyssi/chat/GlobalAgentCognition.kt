...
object GlobalAgentText {
    ...
    private val legacyProductTitles = mapOf(
        "Signal insight" to "GalaxySSI insight",
        "Signal prepared" to "GalaxySSI prepared",
        "Signal digest" to "GalaxySSI digest",
        "Signal \u5efa\u8bae" to "GalaxySSI \u5efa\u8bae",
        "Signal \u5df2\u51c6\u5907" to "GalaxySSI \u5df2\u51c6\u5907",
        "Signal \u6458\u8981" to "GalaxySSI \u6458\u8981"
    )
...
class GlobalUnderstandingPipeline {
    ...
    private fun extractEntities(value: String): Set<String> {
        val known = Regex("(?i)\\b(GalaxySSI|Android|Codex|Claude(?: Code)?|Hermes|OpenClaw|Linux|QEMU|Python|Node(?:\\.js)?|GitHub|MCP|Home Assistant|OpenAI|DeepSeek)\\b")
...
data class GlobalAgentSettings(
    ...
    val discoveryIntervalMillis: Long = 4L * 60L * 60L * 1_000L,
...
object GlobalProactiveMessageFactory {
    fun create(
        ...
        return GlobalProactiveMessage(
            ...
            sourceConversationId = event.conversationId,
            target = target,
            title = if (chinese) "GalaxySSI \u5efa\u8bae" else "GalaxySSI insight",
            content = content,
            topic = understanding.topic,
