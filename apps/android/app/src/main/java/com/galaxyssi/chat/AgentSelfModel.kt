...
object AgentSelfModelReducer {
    const val IDENTITY_ID = "galaxyssi-mobile"
    ...
    private fun inferResource(run: AgentRecordedRun): String = when {
        ...
        run.toolCalls.any { it.toolName.startsWith("android.") || it.toolName.startsWith("galaxyssi.runtime.") } -> IDENTITY_ID
        else -> IDENTITY_ID
    }
...
class AgentSelfModelStore(context: Context) {
    ...
    companion object {
        const val DATABASE_NAME = "galaxyssi_agent_self_model"
