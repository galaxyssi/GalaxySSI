...
enum class AgentRemoteExecutionTool(
    ...
    SHELL("galaxyssi.desktop.exec.shell", "shell"),
    PYTHON("galaxyssi.desktop.exec.python", "python"),
    UV("galaxyssi.desktop.exec.uv", "uv"),
    FFMPEG("galaxyssi.desktop.exec.ffmpeg", "ffmpeg")
}
...
data class AgentRemoteExecutionToolDefinition(
    ...
    init {
        ...
        require(transport == AgentRemoteExecutionTools.REQUIRED_TRANSPORT) {
            "Remote execution tools require encrypted GalaxySSI Link transport"
        }
 ...
 * through GalaxySSI Link; this contract intentionally exposes no local process or Android shell API.
 */
...
object AgentRemoteExecutionTools {
    const val CONTRACT_PROTOCOL = "galaxyssi.remote-execution"
    const val CONTRACT_VERSION = 1
    const val TOOL_VERSION = "1.0.0"
    const val REQUIRED_LOCATION = "trusted_paired_desktop"
    const val REQUIRED_TRANSPORT = "galaxyssi-link-encrypted"
...
class AgentRemoteExecutionToolRegistry(
    ...
    private fun gateResponse(
        ...
        provenance = AgentRemoteExecutionProvenance.forDesktop(
            request = request,
            executorId = "galaxyssi.android.remote_execution_gate",
