...
object ChatHistoryStore {
    ...
    @Synchronized
    internal fun pageAfter(
        context: Context,
        contactId: String,
        afterSequenceExclusive: Long,
        pageSize: Int = 100
    ): ChatHistoryForwardPage = database(context).pageAfter(
        contactId,
    ...
    fun appendIncoming(context: Context, payload: String): StoredIncomingMessage? {
        ...
        appendTrace(deliveryTrace, "decrypted", "GalaxySSI Link")
        ...
        return when (type) {
            ...
            "profile_update" -> {
                ...
                val senderId = profile.optString("sender")
                    .ifBlank { profile.optString("galaxyssi_id") }
