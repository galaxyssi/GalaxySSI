...
class AndroidRuntimePayloadVerifier(context: Context) {
    ...
    private fun trustedCertificates(): List<ByteArray> {
        val embedded = decodeTrustAnchors(runCatching {
            appContext.resources.openRawResource(R.raw.galaxyssi_runtime_trust_anchors)
...
class AgentOnDeviceRuntimeManager(
    ...
    private fun buildStatus(
        ...
        val reason = when {
            ...
            !engineReady -> "Install the GalaxySSI QEMU engine"
    ...
    internal fun runtimeBootstrapFiles(): AgentRuntimeBootstrapFiles {
        val engine = qemuEngineFile()
        check(engine.isFile) { "Install the GalaxySSI QEMU engine" }
    ...
    companion object {
        ...
        private const val QEMU_ENGINE_FILE = "libgalaxyssi_qemu.so"
        private const val AVF_FEATURE = "android.software.virtualization_framework"
        private const val INTEGRITY_CACHE = "galaxyssi_runtime_integrity_cache_v1"
...
object AgentOnDeviceRuntimeTools {
    const val STATUS = "galaxyssi.runtime.status"
    const val WORKSPACE_STATUS = "galaxyssi.runtime.workspace.status"
    const val WORKSPACE_ROLLBACK = "galaxyssi.runtime.workspace.rollback"
    const val LIST_PACKS = "galaxyssi.runtime.packs.list"
    const val INSTALL_PACK = "galaxyssi.runtime.packs.install"
    const val EXECUTE = "galaxyssi.runtime.execute"
    ...
    fun definitions(context: Context): List<AgentNativeToolDefinition> {
        ...
        return listOf(
            AgentNativeToolDefinition(
                ...
                executorId = "galaxyssi.android_runtime_broker"
            ),
            AgentNativeToolDefinition(
                descriptor = descriptor(
                    id = WORKSPACE_STATUS,
                ...
                executorId = "galaxyssi.android_runtime_workspace"
            ),
            AgentNativeToolDefinition(
                descriptor = descriptor(
                    id = LIST_PACKS,
    ...
    private fun runtimePackInstallDefinition(
        ...
        executorId = "galaxyssi.android_runtime_pack_manager",
