...
internal object PeerChatTransport {
    fun prepare(
        ...
        val link = endpointId.takeIf { desktopDevice && it.isNotBlank() }
            ?.let { GalaxySSILinkProtocol.serverLink(context, it) }
        ...
        if (!GalaxySSILinkProtocol.validRouteId(routeId)) return null
        ...
        val conversationId = if (link != null) {
            ...
            listOf(GalaxySSICrypto.localGalaxySSIId(), contactId).sorted().joinToString(":", "peer:")
        }
        ...
        val prepared = if (attachments.isEmpty()) emptyList() else runCatching {
            ...
            Log.e("GalaxySSIPeerTransport", "Could not prepare direct-message attachments", error)
        ...
        val payload = JSONObject()
            ...
            .put(
                "contact_id",
                if (desktopDevice) endpointId else GalaxySSICrypto.localGalaxySSIId()
            )
            ...
            .put("task_id", taskId)
            .put("turn_id", turnId)
            .put("sender", if (desktopDevice) "self" else GalaxySSICrypto.localGalaxySSIId())
            .put("peer_chat", true)
