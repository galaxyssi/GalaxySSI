...
internal const val AGENT_TEAM_SPEC_PARAMETER = "_galaxyssi_agent_team_spec"
internal const val AGENT_TEAM_RUN_PARAMETER = "_galaxyssi_agent_team_run_id"
internal const val AGENT_TEAM_SOURCE_PARAMETER = "_galaxyssi_agent_team_source_id"
...
internal object AgentTeamPlanCompiler {
    ...
    private const val AGENT_KNOWLEDGE_CONTEXT_KEY = "_galaxyssi_agent_knowledge_context"
}
...
internal object AgentTeamDispatchIds {
    fun teamId(plan: AgentPlan, actions: List<AgentAction>): String {
        val conversationId = actions.firstNotNullOfOrNull {
            it.parameters["_galaxyssi_conversation_id"]?.takeIf(String::isNotBlank)
        }.orEmpty()
        val turnId = actions.firstNotNullOfOrNull {
            it.parameters["_galaxyssi_turn_id"]?.takeIf(String::isNotBlank)
        }.orEmpty()
        val source = buildString {
            append("galaxyssi-agent-team\u001f")
    ...
    fun supervisorRunId(teamId: String): String = UUID.nameUUIDFromBytes(
        "galaxyssi-agent-team-run\u001f$teamId".toByteArray(Charsets.UTF_8)
    ).toString()

    fun sourceMessageId(supervisorRunId: String): Long {
        val uuid = UUID.nameUUIDFromBytes(
            "galaxyssi-agent-team-response\u001f$supervisorRunId".toByteArray(Charsets.UTF_8)
        )
