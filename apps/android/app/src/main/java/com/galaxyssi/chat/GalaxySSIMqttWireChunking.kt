...
internal class GalaxySSIMqttChunkAssembler(
    ...
    fun accept(scope: String, wire: JSONObject): String? {
        ...
        require(chunkCount in 1..GalaxySSIMqttWireChunking.MAX_CHUNK_COUNT) { "Invalid MQTT chunk count" }
        require(chunkIndex in 0 until chunkCount) { "Invalid MQTT chunk index" }
        require(totalBytes in 1..GalaxySSIMqttWireChunking.MAX_REASSEMBLED_BYTES) {
        ...
        require(chunk.isNotEmpty() && chunk.size <= GalaxySSIMqttWireChunking.DEFAULT_CHUNK_DATA_BYTES) {
            "Invalid MQTT chunk size"
        }
        require(GalaxySSIMqttWireChunking.sha256(chunk) == chunkHash) { "MQTT chunk integrity check failed" }
        ...
        require(GalaxySSIMqttWireChunking.sha256(assembled) == partial.sha256) {
