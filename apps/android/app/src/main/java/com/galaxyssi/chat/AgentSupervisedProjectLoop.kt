...
internal object AgentSupervisedProjectLoop {
    ...
    fun progressRepairPrompt(
        ...
        evidenceExpected = true,
        suffix = buildString {
            append("\nGalaxySSI rejected the proposed action because it would not advance verified project state. ")
...
internal object AgentSupervisedProjectToolInventory {
    ...
    private fun priority(tool: AgentNativeToolDescriptor): Int = when {
        tool.id.startsWith("galaxyssi.project.repository.") -> 0
        tool.id.startsWith("galaxyssi.project.github.") -> 1
        tool.id.startsWith("galaxyssi.workspace.") -> 2
        tool.id.startsWith("galaxyssi.runtime.") -> 3
        tool.id.startsWith("galaxyssi.project.") -> 4
        else -> 5
    }
...
internal object AgentSupervisedProjectToolCanonicalizer {
    ...
    private val TOOL_ALIASES = mapOf(
        "galaxyssi.project.repository.branch" to AgentMobileProjectNativeTools.CHECKOUT_BRANCH,
        "galaxyssi.project.repository.branch.create" to AgentMobileProjectNativeTools.CHECKOUT_BRANCH,
        "galaxyssi.project.repository.checkout_branch" to AgentMobileProjectNativeTools.CHECKOUT_BRANCH,
        "galaxyssi.project.repository.status" to AgentMobileProjectNativeTools.INSPECT,
        "galaxyssi.project.repository.history" to AgentMobileProjectNativeTools.LOG,
        "galaxyssi.project.repository.pull_request.create" to AgentMobileProjectNativeTools.CREATE_PULL_REQUEST,
        "galaxyssi.workspace.files.list" to AgentPhoneNativeToolCatalog.WORKSPACE_LIST,
        "galaxyssi.workspace.file.list" to AgentPhoneNativeToolCatalog.WORKSPACE_LIST,
        "galaxyssi.workspace.list" to AgentPhoneNativeToolCatalog.WORKSPACE_LIST
    )
...
internal fun MobileNativeAgent.acceptSupervisedProjectPlan(
    ...
    progressViolation?.let { violation ->
        Log.w("GalaxySSIAgentLoop", "supervised_progress_rejected reason=$violation")
        ...
        return supervisedRepairDecision(
            ...
            failureMessage = "GalaxySSI could not schedule a different project step after rejecting a non-progressing action: " +
