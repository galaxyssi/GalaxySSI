...
class AgentRichContentView(
    ...
    private fun htmlAnimationBlock(block: AgentRichBlock): View = LinearLayout(activity).apply {
        ...
        addView(GalaxySSIPinchZoomViewport(activity).apply {
            attach(webView)
        ...
        return LinearLayout(activity).apply {
            ...
            if (desktopArtifact) addView(ImageButton(activity).apply {
                ...
                setOnClickListener {
                    ...
                    } else if (GalaxySSIMqttClient.requestDesktopArtifactDownload(block)) {
                        alpha = 0.55f
                        Toast.makeText(
                            activity,
    ...
    private fun copyText(value: String) {
        ...
        clipboard.setPrimaryClip(ClipData.newPlainText("GalaxySSI", value))
    ...
    companion object {
        ...
        private val IMAGE_EXECUTOR = Executors.newFixedThreadPool(3) { runnable ->
            Thread(runnable, "galaxyssi-rich-image").apply { isDaemon = true }
        }
        private val ARTIFACT_EXECUTOR = Executors.newSingleThreadExecutor { runnable ->
            Thread(runnable, "galaxyssi-artifact-save").apply { isDaemon = true }
        }
    }
}

internal class GalaxySSIPinchZoomViewport(context: Context) : FrameLayout(context) {
