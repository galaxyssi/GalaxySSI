...
internal object GalaxySSIMqttMessagePublisher {
    ...
    fun publishGroupTextMessage(
        ...
        memberId: String,
        memberTopic: String
    ): Boolean = GalaxySSIMqttClient.publishJsonForTransport(
        JSONObject()
            .put("type", "text")
            .put("content", content)
            .put("sender", GalaxySSICrypto.localGalaxySSIId())
    ...
    fun publishFileMessage(
        ...
        return GalaxySSIMqttClient.publishJsonForTransport(
            JSONObject()
                .put("type", type)
                .put("file_id", fileId)
                .put("name", name)
                ...
                .put("time", System.currentTimeMillis()),
            topicOverride ?: GalaxySSIMqttClient.outgoingTopicFor(contactId),
            contactId
        )
    }
    ...
    fun publishTaskCancel(
        ...
        val payload = JSONObject()
            ...
            .put("time", System.currentTimeMillis())
        GalaxySSIMqttClient.applicationContext()?.let { context ->
    ...
    fun publishTaskApproval(
        ...
        return GalaxySSIMqttClient.publishJsonForTransport(
            payload,
            topicOverride ?: GalaxySSIMqttClient.outgoingTopicFor(decision.contactId),
            decision.contactId
        )
    }
    ...
    fun publishConversationDelete(conversationId: String, taskIds: Set<String>): Boolean {
        ...
        return GalaxySSIMqttClient.publishJsonForTransport(
            JSONObject()
                ...
                .put("time", System.currentTimeMillis()),
            GalaxySSIMqttClient.outgoingTopicFor("hermes"),
            "hermes"
        )
    }
    ...
    fun publishPhoneContactRequest(targetCard: JSONObject): Boolean {
        ...
        if (targetId == GalaxySSICrypto.localGalaxySSIId()) return false
        val localCard = PhoneContactCard.identityCard(context)
        ...
        return GalaxySSIMqttClient.publishOpaquePairingOrConnect(
            context,
    ...
    fun publishPhoneContactDecision(targetId: String, approved: Boolean): Boolean {
        val context = GalaxySSIMqttClient.applicationContext() ?: return false
        if (targetId.isBlank() || targetId == GalaxySSICrypto.localGalaxySSIId()) return false
        val routes = AppStore.phoneRoutesForIdentity(context, targetId) ?: return false
