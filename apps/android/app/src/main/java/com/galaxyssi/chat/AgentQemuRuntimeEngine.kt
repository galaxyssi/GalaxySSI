...
internal object AgentQemuLaunchPlanBuilder {
    fun build(
        ...
        val command = buildList {
            ...
            addAll(listOf(
                "-drive",
                "if=none,id=galaxyssi_system,file=${spec.systemDiskFile.absolutePath},format=raw,cache=none,aio=threads",
                "-device",
                "virtio-blk-device,drive=galaxyssi_system,serial=${AgentRuntimePersistentDisk.SERIAL}"
            ))
            spec.packAttachments.sortedBy(AgentRuntimePackAttachment::packId).forEachIndexed { index, pack ->
                val driveId = "galaxyssi_pack_$index"
                addAll(listOf(
                    "-drive",
...
class AgentQemuRuntimeEngineController(
    ...
    override val controllerId: String = "galaxyssi.qemu.tcg.v1"
    ...
    private fun validate(spec: AgentRuntimeEngineLaunchSpec) {
        check(spec.engineFile.isFile && spec.engineFile.canExecute()) { "Install the GalaxySSI QEMU engine" }
