...
/** Typed proxies for tools executed by a paired GalaxySSI Desktop. */
object AgentDesktopRemoteNativeTools {
    const val SYSTEM_STATUS = "galaxyssi.desktop.windows.system.status"
    const val PROCESS_LIST = "galaxyssi.desktop.windows.process.list"
    const val FILE_LIST = "galaxyssi.desktop.workspace.file.list"
    const val FILE_READ_TEXT = "galaxyssi.desktop.workspace.file.read.text"
    const val FILE_WRITE_TEXT = "galaxyssi.desktop.workspace.file.write.text"
    const val FILE_SHA256 = "galaxyssi.desktop.workspace.file.sha256"
    const val ARCHIVE_CREATE = "galaxyssi.desktop.workspace.archive.create"
    const val TERMINAL_RUN = "galaxyssi.desktop.terminal.run"
    const val OFFICE_INSPECT = "galaxyssi.desktop.office.document.inspect"
    const val OFFICE_CONVERT = "galaxyssi.desktop.office.document.convert"

    const val READ_CONSENT = "galaxyssi.consent.desktop.read"
    const val EXECUTE_CONSENT = "galaxyssi.consent.desktop.execute"
...
private object DesktopToolCapabilityStore {
    private const val PREFERENCES = "galaxyssi_desktop_native_tools_v1"
    ...
    fun invoke(context: Context, invocation: AgentNativeToolInvocation): AgentNativeToolExecutionResult {
        ...
        val confirmation = invocation.descriptor.requiredConsents.takeIf { it.isNotEmpty() }?.let {
            JSONObject()
                ...
                .put("policy", "galaxyssi_full_access")
        ...
        val cancellationRegistration = invocation.cancellationToken.invokeOnCancellation {
            GalaxySSIMqttClient.publishDesktopToolCancel(desktopId, callId, taskId, conversationId)
            waiting.latch.countDown()
        }
        try {
            if (!GalaxySSIMqttClient.publishDesktopToolCall(desktopId, payload)) {
