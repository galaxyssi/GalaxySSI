...
object GalaxySSILinkProtocol {
    const val NAME = "galaxyssi-link"
    const val VERSION = 2
    ...
    const val ACCESS_CONTRACT = "galaxyssi.pairing-access/1.0"
    ...
    fun normalizePairingQr(source: JSONObject): JSONObject? {
        ...
        val desktopName = source.optString("n").ifBlank { "GalaxySSI Desktop" }
    ...
    fun shouldRotateClientRoute(
        existing: ServerLink?,
        qr: JSONObject,
        localFingerprint: String = GalaxySSICrypto.localIdentitySha256()
    ): Boolean {
    ...
    fun ensureServerLink(context: Context, qr: JSONObject, rotateClientRoute: Boolean = false): ServerLink {
        ...
        val link = ServerLink(
            desktopId = desktopId,
            desktopName = qr.optString("desktop_name", "GalaxySSI Desktop"),
        ...
        existing?.let { GalaxySSILinkDeliveryStore.discardRoutes(context, it.routes) }
        save(context, link)
        return link
    }
    ...
    fun isCryptographicallyReady(context: Context, link: ServerLink): Boolean {
        ...
        val verifiedFingerprint = GalaxySSICrypto.verifiedDesktopFingerprint(link.desktopId)
    ...
    fun removeServer(context: Context, desktopId: String) {
        ...
        links.firstOrNull { it.desktopId == desktopId }?.let {
            GalaxySSILinkDeliveryStore.discardRoutes(context, it.routes)
        }
    ...
    private fun kdf(secret: ByteArray, label: ByteArray): ByteArray {
        ...
        return mac.doFinal("galaxyssi-opaque-v2\u0000".toByteArray(Charsets.UTF_8) + label)
    }
