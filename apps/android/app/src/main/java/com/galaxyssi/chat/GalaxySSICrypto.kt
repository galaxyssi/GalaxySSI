...
object GalaxySSICrypto {
    private const val TAG = "GalaxySSICrypto"
    ...
    private const val PREFS = "galaxyssi_signal_trust"
    ...
    fun localGalaxySSIId(): String = "galaxyssi:${localIdentitySha256().take(16)}"

    fun localHermesId(): String = localGalaxySSIId()
    ...
    fun verifyIdentitySignature(
        ...
        val identityKey = if (identityName == localGalaxySSIId()) {
    ...
    fun localSignalBundleJson(): JSONObject {
        ensureInitialized()
        return store.currentBundleJson(localGalaxySSIId(), DEFAULT_DEVICE_ID)
    ...
    fun exportSignalStoreJson(context: Context): JSONObject {
        ...
        return store.exportJson()
            ...
            .put("local_galaxyssi_id", localGalaxySSIId())
    ...
    fun verifyPcIdentityFromQr(contents: String): Boolean {
        ...
        if (!GalaxySSILinkProtocol.validatePairingQr(json)) return false
    ...
    private fun ensureInitialized() {
        check(::store.isInitialized) { "GalaxySSICrypto.initialize(context) must be called first" }
    }
