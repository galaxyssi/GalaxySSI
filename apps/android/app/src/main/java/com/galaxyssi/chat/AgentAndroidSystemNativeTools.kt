...
object AgentAndroidSystemNativeTools {
    const val TELEPHONY_STATUS = "galaxyssi.android.telephony.status"
    const val TELEPHONY_CALL_STATE = "galaxyssi.android.telephony.call_state"
    const val TELEPHONY_CALL_STATE_OBSERVE = "galaxyssi.android.telephony.call_state.observe"
    const val TELEPHONY_DIAL_HANDOFF = "galaxyssi.android.telephony.dial.handoff"
    const val SMS_LIST = "galaxyssi.android.sms.list"
    const val SMS_SEND = "galaxyssi.android.sms.send"
    const val SMS_COMPOSE_HANDOFF = "galaxyssi.android.sms.compose.handoff"
    const val CONTACTS_SEARCH = "galaxyssi.android.contacts.search"
    const val CONTACTS_UPSERT = "galaxyssi.android.contacts.upsert"
    const val CONTACTS_DELETE = "galaxyssi.android.contacts.delete"
    const val CALENDARS_LIST = "galaxyssi.android.calendar.calendars.list"
    const val CALENDAR_EVENTS_QUERY = "galaxyssi.android.calendar.events.query"
    const val CALENDAR_EVENT_UPSERT = "galaxyssi.android.calendar.event.upsert"
    const val CALENDAR_EVENT_DELETE = "galaxyssi.android.calendar.event.delete"
    const val WIFI_STATUS = "galaxyssi.android.wifi.status"
    const val WIFI_SCAN_RESULTS = "galaxyssi.android.wifi.scan_results"
    const val WIFI_SCAN_START = "galaxyssi.android.wifi.scan.start"
    const val WIFI_PANEL_OPEN = "galaxyssi.android.wifi.panel.open"
    const val WIFI_HOTSPOT_PANEL_OPEN = "galaxyssi.android.wifi.hotspot.panel.open"
    const val AUDIO_STATUS = "galaxyssi.android.audio.status"
    const val AUDIO_VOLUME_SET = "galaxyssi.android.audio.volume.set"
    const val AUDIO_MUTE_SET = "galaxyssi.android.audio.mute.set"
    const val DOWNLOAD_ENQUEUE = "galaxyssi.android.download.enqueue"
    const val DOWNLOAD_QUERY = "galaxyssi.android.download.query"
    const val DOWNLOAD_REMOVE = "galaxyssi.android.download.remove"
    const val BIOMETRIC_STATUS = "galaxyssi.android.biometric.status"
    const val BIOMETRIC_ENROLLMENT_OPEN = "galaxyssi.android.biometric.enrollment.open"
    const val VPN_STATUS = "galaxyssi.android.vpn.status"
    const val VPN_CONSENT_OPEN = "galaxyssi.android.vpn.consent.open"
    const val DEVICE_POLICY_STATUS = "galaxyssi.android.device_policy.status"
    const val DEVICE_POLICY_LOCK = "galaxyssi.android.device_policy.lock"
    const val DEVICE_POLICY_REBOOT = "galaxyssi.android.device_policy.reboot"

    const val CONSENT_PHONE_READ = "galaxyssi.consent.phone.read"
    const val CONSENT_SMS_READ = "galaxyssi.consent.sms.read"
    const val CONSENT_SMS_SEND = "galaxyssi.consent.sms.send"
    const val CONSENT_CONTACTS_WRITE = "galaxyssi.consent.contacts.write"
    const val CONSENT_CALENDAR_WRITE = "galaxyssi.consent.calendar.write"
    const val CONSENT_AUDIO_CHANGE = "galaxyssi.consent.audio.change"
    const val CONSENT_DOWNLOAD = "galaxyssi.consent.download"
    const val CONSENT_DEVICE_POLICY = "galaxyssi.consent.device_policy"

    private const val VERSION = "1.0.0"
    private const val EXECUTOR = "galaxyssi.android.system_native"
    ...
    fun definitions(context: Context): List<AgentNativeToolDefinition> {
        ...
        return listOf(
            ...
            readTool(app, DEVICE_POLICY_STATUS, "Read device policy status", "Reads active-admin and device-owner status for GalaxySSI.",
                setOf("device_policy.status"), emptyList(), ::emptyInput) { devicePolicyStatus(app) },
            writeTool(app, DEVICE_POLICY_LOCK, "Lock device through device policy", "Locks the device only when GalaxySSI is an active device admin.",
                ...
                if (!manager.isAdminActive(admin)) return@writeTool failure("device_admin_required", "GalaxySSI is not an active device admin")
                manager.lockNow()
                success(mapOf("locked" to true), "Device lock requested")
            },
            writeTool(app, DEVICE_POLICY_REBOOT, "Reboot device through device policy", "Reboots only when GalaxySSI is the provisioned device owner.",
                AgentNativeToolRisk.HIGH, setOf("device_policy.reboot"), emptyList(), emptyInput(),
                listOf(CONSENT_DEVICE_POLICY)) {
                val manager = app.getSystemService(DevicePolicyManager::class.java)
                if (!manager.isDeviceOwnerApp(app.packageName)) return@writeTool failure("device_owner_required", "GalaxySSI is not the device owner")
    ...
    private fun adminComponent(context: Context) = ComponentName(context, GalaxySSIDeviceAdminReceiver::class.java)
