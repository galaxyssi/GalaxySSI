...
internal object PhoneContactCard {
    ...
    private val galaxyssiIdPattern = Regex("^galaxyssi:[a-fA-F0-9]{16}$")
    ...
    fun createQr(context: Context, profile: JSONObject): JSONObject {
        ...
        val token = GalaxySSILinkProtocol.newLinkSecret()
        val secret = GalaxySSILinkProtocol.newLinkSecret()
        val topic = GalaxySSILinkProtocol.pairingTopic(secret)
    ...
    fun compactQr(card: JSONObject): JSONObject {
        ...
        return JSONObject()
            .put("t", QR_TYPE)
            .put("i", card.getString("galaxyssi_id"))
    ...
    fun normalizeQr(source: JSONObject): JSONObject? {
        ...
        if (!GalaxySSILinkProtocol.validLinkSecret(secret) ||
            !GalaxySSILinkProtocol.validLinkSecret(source.optString("x"))
        ) return null
        val card = JSONObject()
            .put("type", TYPE)
            .put("version", VERSION)
            .put("galaxyssi_id", source.optString("i"))
    ...
    fun controlPayload(
        ...
        require(targetGalaxySSIId.isNotBlank()) { "Target identity is required" }
        if (type == REQUEST_TYPE) {
            require(GalaxySSILinkProtocol.validLinkSecret(pairingToken)) {
        ...
        return JSONObject()
            ...
            .put("from", localCard.optString("galaxyssi_id"))
            .put("to", targetGalaxySSIId)
    ...
    fun isQrOfferValid(card: JSONObject): Boolean =
        isIdentityHeaderValid(card) &&
            ...
            GalaxySSILinkProtocol.validLinkSecret(card.optString("pairing_token")) &&
            GalaxySSILinkProtocol.validLinkSecret(card.optString("pairing_secret")) &&
            card.optString("pairing_topic") == GalaxySSILinkProtocol.pairingTopic(
                card.optString("pairing_secret")
            ) &&
            isFreshControlPayload(card) &&
            GalaxySSICrypto.verifyPublicIdentitySignature(
    ...
    fun isIdentityValid(card: JSONObject): Boolean {
        ...
        return GalaxySSICrypto.signalBundleFingerprint(bundle).equals(fingerprint, ignoreCase = true)
    }
    ...
    fun isAddressedToLocalIdentity(payload: JSONObject, localGalaxySSIId: String): Boolean =
        payload.optString("to") == localGalaxySSIId

    private fun baseIdentityCard(context: Context, profile: JSONObject, type: String): JSONObject {
        val bundle = GalaxySSICrypto.localSignalBundleJson()
        val fingerprint = GalaxySSICrypto.localIdentitySha256()
        val device = GalaxySSIDeviceIdentity.current(context, profile, fingerprint)
        return JSONObject()
            .put("type", type)
            .put("version", VERSION)
            .put("galaxyssi_id", GalaxySSICrypto.localGalaxySSIId())
            .put("name", profile.optString("name", "Me"))
            .put("identity_public_key", GalaxySSICrypto.localIdentityPublicKey())
            .put("identity_fingerprint", fingerprint)
            .put("signal_bundle", bundle)
            .put("bundle_identity_fingerprint", GalaxySSICrypto.signalBundleFingerprint(bundle))
    ...
    private fun signCard(card: JSONObject) {
        card.put("signature", GalaxySSICrypto.signLocalIdentity(canonicalBytes(card)))
    }
    ...
    private fun isIdentityHeaderValid(card: JSONObject): Boolean {
        ...
        val galaxyssiId = card.optString("galaxyssi_id")
        val fingerprint = card.optString("identity_fingerprint")
        return galaxyssiIdPattern.matches(galaxyssiId) &&
            ...
            galaxyssiId.substringAfter(':').equals(fingerprint.take(16), ignoreCase = true) &&
        ...
        val active = buildList {
            for (index in 0 until array.length()) {
                ...
                if (session.optLong("expires_at") >= nowMillis &&
                    GalaxySSILinkProtocol.validLinkSecret(session.optString("secret")) &&
                    session.optString("topic") == GalaxySSILinkProtocol.pairingTopic(
