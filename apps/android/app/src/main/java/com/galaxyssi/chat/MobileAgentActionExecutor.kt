...
class AndroidAgentActionExecutor(private val context: Context) : AgentActionExecutor {
    ...
    override fun execute(action: AgentAction, screen: ScreenContext): AgentActionResult = when (action.kind) {
        ...
        AgentActionKind.BACK -> serviceAction(action.id, "Back action executed") {
            GalaxySSIAccessibilityService.performGlobalBack()
        }
        AgentActionKind.TAP -> {
            ...
            if (bounds.isBlank()) {
                ...
                serviceAction(action.id, "Tap action executed") {
                    GalaxySSIAccessibilityService.performTap(bounds)
                }
            }
        }
            ...
            if (text.isBlank()) {
                ...
                serviceAction(action.id, "Text input executed") {
                    if (fieldBounds.isBlank()) {
                        GalaxySSIAccessibilityService.performTextInput(text)
                    } else if (fieldOrigin == AgentElementOrigin.VISUAL_OCR.name) {
                        GalaxySSIAccessibilityService.performGroundedTextInput(fieldBounds, text)
                    } else {
                        GalaxySSIAccessibilityService.performTextInput(fieldBounds, text)
                    }
                }
            }
        }
        AgentActionKind.DELETE_TEXT -> {
            ...
            serviceAction(action.id, "Text cleared") {
                if (fieldBounds.isBlank()) {
                    GalaxySSIAccessibilityService.performClearText()
                } else {
                    GalaxySSIAccessibilityService.performClearText(fieldBounds)
                }
            }
        }
        AgentActionKind.PASTE_TEXT -> {
            ...
            if (clipboardText.isBlank()) {
                ...
                serviceAction(action.id, "Clipboard pasted") {
                    if (fieldBounds.isBlank()) {
                        GalaxySSIAccessibilityService.performTextInput(clipboardText)
                    } else {
                        GalaxySSIAccessibilityService.performTextInput(fieldBounds, clipboardText)
                    }
                }
            }
        }
        AgentActionKind.SWIPE -> {
            ...
            serviceAction(action.id, "Swipe action executed") {
                GalaxySSIAccessibilityService.performSwipe(fromX, fromY, toX, toY)
            }
        }
        AgentActionKind.LONG_PRESS -> {
            ...
            if (bounds.isBlank()) {
                ...
                serviceAction(action.id, "Long press action executed") {
                    GalaxySSIAccessibilityService.performLongPress(bounds)
                }
            }
        }
        AgentActionKind.HOME -> serviceAction(action.id, "Home action executed") {
            GalaxySSIAccessibilityService.performGlobalHome()
        }
        AgentActionKind.RECENTS -> serviceAction(action.id, "Recent apps opened") {
            GalaxySSIAccessibilityService.performGlobalRecents()
        }
        AgentActionKind.LOCK_SCREEN -> serviceAction(action.id, "Screen locked") {
            GalaxySSIAccessibilityService.performGlobalLockScreen()
        }
    ...
    internal fun createLocalNotification(action: AgentAction): AgentActionResult {
        ...
        val title = action.parameters["title"].orEmpty().ifBlank { "GalaxySSI Agent" }
    ...
    internal fun replyToNotification(action: AgentAction): AgentActionResult {
        ...
        val result = GalaxySSINotificationListenerService.reply(notificationKey, replyText)
        return AgentActionResult(
            actionId = action.id,
    ...
    internal fun copyScreenText(action: AgentAction, screen: ScreenContext): AgentActionResult {
        val latestScreen = GalaxySSIAccessibilityService.captureCurrentScreen(
        ...
        clipboard.setPrimaryClip(ClipData.newPlainText("GalaxySSI screen text", text))
        return AgentActionResult(
            actionId = action.id,
            success = true,
    ...
    internal fun setAlarm(action: AgentAction): AgentActionResult {
        ...
        val intent = when {
            timerSeconds != null -> Intent(AlarmClock.ACTION_SET_TIMER)
                ...
                .putExtra(AlarmClock.EXTRA_MESSAGE, "GalaxySSI")
    ...
    internal fun dispatchLocalModelTask(action: AgentAction, prompt: String): AgentActionResult {
        ...
        LOCAL_MODEL_EXECUTOR.execute {
            ...
            val taskId = action.parameters["_galaxyssi_task_id"].orEmpty().ifBlank { turnId }
    ...
    internal fun dispatchDeviceTask(action: AgentAction): AgentActionResult {
        ...
        if (customDeviceId.isNotBlank()) {
            ...
            if (connector.transport == CustomDeviceTransport.GALAXYSSI_AGENT) {
    ...
    internal fun dispatchContactTask(action: AgentAction, contactId: String, prompt: String): AgentActionResult {
        ...
        val voiceAgentRun = if (
            ...
            runCatching {
                ...
                Log.w("GalaxySSIAgent", "Could not create the foreground agent run", failure)
            }.getOrNull()
        } else {
            null
        }
        ...
        if (persistDedicatedHistory) {
            ...
            Thread({
                runCatching {
                    ...
                    Log.w("GalaxySSILatency", "connector_history_write_failed source=$messageId", failure)
                }
            }, "galaxyssi-connector-history-$messageId").start()
        }
        return AgentActionResult(
            actionId = action.id,
            success = published,
    ...
    internal fun dispatchCloudModelTask(
        ...
        val connectorTaskId = action.parameters["_galaxyssi_task_id"].orEmpty()
        ...
        val contactId = contact.optString("id").ifBlank { contact.optString("galaxyssi_id") }
        ...
        val outgoingHistoryWrite = if (persistDedicatedHistory) {
            FutureTask<Long> {
                ...
                Thread(task, "galaxyssi-cloud-history-$messageId").start()
            }
        } else {
            null
        }
    ...
    companion object {
        private const val AGENT_NOTIFICATION_CHANNEL_ID = "galaxyssi_agent_actions"
        ...
        private val LOCAL_MODEL_EXECUTOR = Executors.newSingleThreadExecutor { task ->
            Thread(task, "GalaxySSI-LocalModel").apply { isDaemon = true }
        }
        private const val RICH_RESPONSE_CONTRACT =
            "GalaxySSI can render optional rich output. When a visual, table, media preview, animation, or public web page " +
                "would answer better than plain text, append one fenced galaxyssi-rich JSON document. " +
