...
class GalaxySSIReputationIdentity(context: Context) :
    AgentReputationSignatureVerifier,
    AgentReputationSigner {
    init {
        GalaxySSICrypto.initialize(context.applicationContext)
    }

    override val signerId: String
        get() = GalaxySSICrypto.localGalaxySSIId()

    override val signatureKeyId: String
        get() = GalaxySSICrypto.localIdentitySha256()

    override fun sign(payload: ByteArray): String = GalaxySSICrypto.signLocalIdentity(payload)
    ...
    override fun verify(
        ...
        payload: ByteArray,
        signature: String
    ): Boolean = GalaxySSICrypto.verifyIdentitySignature(
...
class EncryptedAgentReputationStore(context: Context) : AgentReputationStore {
    ...
    private companion object {
        const val DATABASE = "galaxyssi_agent_reputation_ledger_v1"
    ...
    companion object {
        ...
        fun encrypted(context: Context): AgentReputationLedger =
            processInstance ?: synchronized(this) {
                processInstance ?: run {
                    val identity = GalaxySSIReputationIdentity(context)
