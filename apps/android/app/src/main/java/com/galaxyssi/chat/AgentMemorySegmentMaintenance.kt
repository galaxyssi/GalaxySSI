...
internal class AgentMemorySegmentMaintenance(private val sql: SQLiteDatabase,
    ...
    fun run(maxSegments: Int, maxRows: Int): Result {
        ...
        for (entry in segments.catalog.next(maxSegments)) {
                ...
                if (live > 0 && live <= segments.size(entry.segment) / 2) {
                    ...
                    try {
                        for (row in rows) {
                            ...
                            val next = segments.relocate(row.key, row.value, aad(row.key), entry.segment, row.bytes)
            ...
            segments.catalog.advance(entry, disposable)
        }
        return Result(visited, moved, removed, reclaimed)
    }
