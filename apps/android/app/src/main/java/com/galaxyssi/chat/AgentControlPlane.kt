...
class TransportBackedAgentAdapter(
    ...
    override suspend fun connect(): AgentProtocolAgreement = connectionMutex.withLock {
        ...
        if (negotiated == null) {
            ...
            throw IllegalStateException("No compatible GalaxySSI Agent protocol version")
        }
...
class TransportBackedAgentProvider(
    ...
    override suspend fun connect(): AgentProtocolAgreement = connectionMutex.withLock {
        ...
        if (negotiated == null) {
            runCatching { transport.close() }
            throw IllegalStateException("No compatible GalaxySSI Provider protocol version")
        }
        agreement = negotiated
        negotiated
    }
...
class EncryptedAgentRegistry(context: Context) {
    ...
    companion object {
        private const val DATABASE = "galaxyssi_agent_registry_v2"
        private const val LEGACY_DATABASE = "galaxyssi_agent_registry_v1.db"
