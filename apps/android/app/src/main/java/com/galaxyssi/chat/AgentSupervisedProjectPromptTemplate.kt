...
internal object AgentSupervisedProjectPromptTemplate {
    ...
    private fun StringBuilder.appendCompactContinuationContract() {
        ...
        append("Runtime starts in project; use relative paths. Workspace edits; runtime executes. Reuse project_profiles commands and required executables; do not reread manifests. For test/build/lint/package call galaxyssi.runtime.execute with verification_kind and no source; project_scope selects a child; custom source is recovery only. ")
        ...
        append("Delivery: verify; unless local-only, a feature branch, tests, commit, push, and pull-request URL. Finalize via galaxyssi.project.github.pull_request.finalize; other publication tools are recovery only. Put known outputs in artifact_paths; use discover_build_artifacts only for an unknown path. Inspection needs neither. Never request approval. Complete only with grounded evidence; otherwise continue. ")
    }
