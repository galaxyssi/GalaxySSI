...
internal object AgentRuntimeDistributionSources {
    const val GITHUB_CATALOG_URL =
        "https://github.com/galaxyssi/GalaxySSI/releases/download/android-runtime-v1/android-runtime-catalog-v1.json"
    ...
    fun downloadCandidates(url: String, language: String): List<String> {
        if (language != AppLanguage.ZH_CN || !isGalaxySSIGitHubReleaseUrl(url)) return listOf(url)
        return (CHINA_ACCELERATOR_PREFIXES.map { prefix -> "$prefix$url" } + url).distinct()
    }

    private fun isGalaxySSIGitHubReleaseUrl(value: String): Boolean = runCatching {
        val uri = URI(value)
        uri.scheme.equals("https", ignoreCase = true) &&
            uri.host.equals("github.com", ignoreCase = true) &&
            uri.path.startsWith(GALAXYSSI_RELEASE_PATH)
    }.getOrDefault(false)

    private const val GALAXYSSI_RELEASE_PATH = "/galaxyssi/GalaxySSI/releases/download/"
...
class AgentRuntimePackCatalogStore(context: Context) {
    ...
    companion object {
        private const val DATABASE = "galaxyssi_runtime_catalog_v1"
