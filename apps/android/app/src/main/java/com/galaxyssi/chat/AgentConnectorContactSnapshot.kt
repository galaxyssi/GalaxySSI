...
internal class AgentConnectorContactSnapshot private constructor(
    ...
    fun matchingContactIds(targetId: String): List<String> {
        ...
        return contacts.mapNotNull { contact ->
            ...
            val galaxyssiId = contact.galaxyssiId()
            if (id !in aliases && agentId !in aliases && galaxyssiId !in aliases) return@mapNotNull null
            id.ifBlank { galaxyssiId.ifBlank { agentId } }.takeIf(String::isNotBlank)
        }.distinct()
    }
    ...
    companion object {
        fun from(source: JSONArray): AgentConnectorContactSnapshot {
            ...
            val byId = buildMap {
                contacts.forEach { contact ->
                    contact.galaxyssiId().takeIf(String::isNotBlank)?.let { put(it, contact) }
        ...
        private fun JSONObject.galaxyssiId(): String =
            optString("galaxyssi_id").ifBlank { optString("hermes_id") }.ifBlank { optString("id") }
    }
