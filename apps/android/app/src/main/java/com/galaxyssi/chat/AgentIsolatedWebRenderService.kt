...
class AgentIsolatedWebRenderService : Service() {
    ...
    private inner class RenderJob(
        ...
        private fun readSnapshotChunk(
            ...
            val script = """
                (() => {
                  const value = window.__galaxyssiFrozenDom || '';
                  const start = ${offset};
    ...
    private companion object {
        ...
        val PROBE_SCRIPT = """
            (() => {
              if (!window.__galaxyssiDomObserver) {
                window.__galaxyssiMutationAt = Date.now();
                window.__galaxyssiDomObserver = new MutationObserver(() => {
                  window.__galaxyssiMutationAt = Date.now();
                });
                window.__galaxyssiDomObserver.observe(document.documentElement, {
              ...
              return JSON.stringify({
                ...
                now: Date.now(),
                mutation_at: window.__galaxyssiMutationAt || Date.now()
              });
            })()
        """.trimIndent()
        val FREEZE_SCRIPT = """
            (() => {
              window.__galaxyssiFrozenDom = document.documentElement
                ? document.documentElement.outerHTML
                : '';
              return JSON.stringify({
                url: location.href,
                title: document.title || '',
                length: window.__galaxyssiFrozenDom.length
              });
            })()
        """.trimIndent()
    }
