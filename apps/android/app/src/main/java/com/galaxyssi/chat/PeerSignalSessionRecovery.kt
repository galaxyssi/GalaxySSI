...
internal object PeerSignalSessionRecoveryCoordinator {
    ...
    fun decryptOrRequestRefresh(
        ...
        is GalaxySSICrypto.EnvelopeDecryptionResult.Failure -> {
            if (PeerSignalSessionRecoveryGate.begin(contactId)) {
                if (GalaxySSIMqttMessagePublisher.requestSignalBundle(context, contactId)) {
                    Log.w(
                        TAG,
        ...
        GalaxySSICrypto.EnvelopeDecryptionResult.Rejected -> null
    }
    ...
    fun reencryptPendingMessages(context: Context, contactId: String): Int {
        ...
        return GalaxySSILinkDeliveryStore.reencryptRecoverableMessages(
            context,
            contactId,
            routes.up
        ) { applicationEnvelope ->
