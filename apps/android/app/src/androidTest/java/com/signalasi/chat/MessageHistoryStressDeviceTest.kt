...
class MessageHistoryStressDeviceTest {
    ...
    private fun seedPeerConversation(
        ...
        if (dispatchTransport) {
            SignalASIMqttClient.connect(context)
        } else {
            SignalASIMqttClient.prepareReliableQueue(context)
        }
    ...
    private fun cleanupPeerOutbox(
        ...
        val pendingBefore = SignalASILinkDeliveryStore.pendingCount(context)
        val removed = SignalASILinkDeliveryStore.discardClientSourceMessages(context, sourceMessageIds)
        val pendingAfter = SignalASILinkDeliveryStore.pendingCount(context)
