    ...
    fun aptPackageAndRootHomeSurviveGuestRestart() {
        ...
        val install = executeShell(
            ...
            source = """
                ...
                mkdir -p /root/.signalasi-runtime-tests
                printf 'persistent-package-manager\n' > /root/.signalasi-runtime-tests/apt-marker
        ...
        val persisted = executeShell(
            ...
            source = """
                set -eu
                test "${'$'}(id -u)" = "0"
                test "${'$'}(cat /root/.signalasi-runtime-tests/apt-marker)" = "persistent-package-manager"
                dpkg-query -W -f='${'$'}{Status}\n' bc
                printf 'bc_result=%s\n' "${'$'}(printf '6 * 7\n' | bc)"
            """.trimIndent(),
        ...
        val cleanup = executeShell(
            ...
            source = """
                ...
                apt-get remove -y bc
                apt-get clean
                rm -rf /root/.signalasi-runtime-tests
        ...
        val registry = AgentPhoneNativeToolCatalog.defaultRegistry(
            ...
            screenProvider = {
                ScreenContext(
                    ...
                    pageTitle = "SignalASI persistent package manager device test"
                )
            }
        )
