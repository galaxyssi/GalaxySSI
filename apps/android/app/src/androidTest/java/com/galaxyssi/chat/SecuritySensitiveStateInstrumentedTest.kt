...
class SecuritySensitiveStateInstrumentedTest {
    ...
    fun routesFingerprintAndGrantAreEncryptedAtRest() {
        ...
        val qr = requireNotNull(
            GalaxySSILinkProtocol.normalizePairingQr(
                JSONObject()
                    ...
                    .put("x", GalaxySSILinkProtocol.newLinkSecret())
                    .put("e", GalaxySSILinkProtocol.newLinkSecret())
                    .put("a", 0)
            )
        )

        val link = GalaxySSILinkProtocol.ensureServerLink(context, qr)
        GalaxySSILinkProtocol.markPaired(context, link.desktopId, qr.getJSONObject("pairing_access"))
        ...
        assertEquals(
            ...
            GalaxySSILinkProtocol.serverLink(context, link.desktopId)?.routes?.clientRouteId
        )
    }
    ...
    fun opaquePairingQrVerifiesItsPublicIdentity() {
        val qr = requireNotNull(
            GalaxySSILinkProtocol.normalizePairingQr(
                JSONObject()
                    ...
                    .put("k", GalaxySSICrypto.localIdentityPublicKey())
                    .put("h", GalaxySSICrypto.localIdentitySha256())
                    .put("c", System.currentTimeMillis() / 1000L)
                    .put("x", GalaxySSILinkProtocol.newLinkSecret())
                    .put("e", GalaxySSILinkProtocol.newLinkSecret())
                    .put("a", 0)
        ...
        assertTrue(GalaxySSICrypto.verifyPcIdentityFromQr(qr.toString()))
        assertEquals(
            qr.getString("identity_key_sha256"),
            GalaxySSICrypto.verifiedPcFingerprint()
        )
    }
    ...
    fun signalIdentitySessionsAndTrustFingerprintsAreEncryptedAtRest() {
        ...
        assertEquals(fingerprint, GalaxySSICrypto.verifiedPcFingerprint())
    }
