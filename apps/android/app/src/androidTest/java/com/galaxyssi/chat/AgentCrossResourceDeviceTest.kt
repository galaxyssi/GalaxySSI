...
class AgentCrossResourceDeviceTest {
    ...
    fun executesCrossResourceProductionPaths() {
        ...
        try {
            ...
            println("GALAXYSSI_CROSS_RESOURCE_REPORT=${reportFile.absolutePath}")
        }
    }

    private fun startGalaxySSI() {
        context.startForegroundService(Intent(context, MessageService::class.java))
        NotificationListenerService.requestRebind(
            android.content.ComponentName(context, GalaxySSINotificationListenerService::class.java)
        )
        waitUntil("notification listener", NOTIFICATION_TIMEOUT_MILLIS) {
            GalaxySSINotificationListenerService.currentContext().hasAccess
        }
    }

    private fun verifyNotificationReadAndReply(): JSONObject {
        val notificationManager = context.getSystemService(NotificationManager::class.java)
        val marker = "GalaxySSI notification acceptance ${UUID.randomUUID()}"
        val replyText = "GalaxySSI reply ${UUID.randomUUID()}"
        ...
        try {
            ...
            waitUntil("posted acceptance notification", NOTIFICATION_TIMEOUT_MILLIS) {
                val item = GalaxySSINotificationListenerService.currentContext(12).items.firstOrNull {
            ...
            waitUntil("notification removal", NOTIFICATION_TIMEOUT_MILLIS) {
                GalaxySSINotificationListenerService.currentContext(12).items.none { it.key == observedKey }
            }
    ...
    private fun verifyPairedDesktopTools(): JSONObject {
        waitUntil("secure paired Desktop", DESKTOP_CONNECTION_TIMEOUT_MILLIS) {
            GalaxySSIMqttClient.isConnected() && GalaxySSIMqttClient.isSecureReady()
        }
        val registry = registry()
        ...
        val fileText = "GalaxySSI paired Desktop acceptance"
        val written = invoke(
            registry,
        ...
        val documentXml = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>" +
            ...
            "<w:body><w:p><w:r><w:t>GalaxySSI paired Office acceptance</w:t></w:r></w:p></w:body></w:document>"
        ...
        assertTrue(inspected.output["text_items"].toString().contains("GalaxySSI paired Office acceptance"))
    ...
    private fun registry(): AgentNativeToolRegistry = AgentPhoneNativeToolCatalog.defaultRegistry(
        ...
        screenProvider = {
            ScreenContext(
                ...
                pageTitle = "GalaxySSI cross-resource acceptance"
            )
        }
    )
    ...
    companion object {
        private const val NOTIFICATION_CHANNEL_ID = "galaxyssi_cross_resource_acceptance"
        private const val REPLY_RESULT_KEY = "galaxyssi_acceptance_reply"
        private const val HOME_ASSISTANT_PORT_ARGUMENT = "galaxyssi_home_assistant_port"
        private const val DEFAULT_HOME_ASSISTANT_PORT = 18_123
        private const val HOME_ASSISTANT_TOKEN = "galaxyssi-cross-resource-acceptance"
