...
class AgentNativeToolsDeviceTest {
        ...
        registry = AgentPhoneNativeToolCatalog.defaultRegistry(
            ...
            screenProvider = {
                ScreenContext(
                    ...
                    pageTitle = "GalaxySSI device test"
                )
            }
        )
        reportFile = File(
    ...
    fun testAllRegisteredPhoneToolsOnDevice() {
        ...
        descriptors
            .filterNot { it.id.startsWith("galaxyssi.workspace.") }
    ...
    private fun testDownloadManagerLifecycle(enqueueDescriptor: AgentNativeToolDescriptor) {
        val enqueue = invoke(
            enqueueDescriptor,
            mapOf("url" to "https://example.com/", "title" to "GalaxySSI native tool test")
        )
    ...
    private fun inputFor(id: String): AgentNativeJsonObject {
        ...
        return when (id) {
            ...
            AgentHardwareNativeTools.INSTALLED_APPS_LIST -> mapOf("query" to "GalaxySSI", "limit" to 20)
            ...
            AgentAndroidSystemNativeTools.CONTACTS_SEARCH -> mapOf("query" to "GalaxySSI device test", "limit" to 1)
            ...
            AgentAndroidSystemNativeTools.SMS_SEND -> mapOf("phone_number" to "10086", "message" to "GalaxySSI device test")
            AgentAndroidSystemNativeTools.SMS_COMPOSE_HANDOFF -> mapOf("phone_number" to "10086", "message" to "GalaxySSI device test")
            AgentAndroidSystemNativeTools.CONTACTS_UPSERT -> mapOf("display_name" to "GalaxySSI device test", "phone_number" to "10086")
            AgentAndroidSystemNativeTools.CONTACTS_DELETE -> mapOf("contact_id" to 1)
            AgentAndroidSystemNativeTools.CALENDAR_EVENT_UPSERT -> mapOf("calendar_id" to 1, "title" to "GalaxySSI device test", "start_epoch_ms" to now, "end_epoch_ms" to now + 60_000)
            AgentAndroidSystemNativeTools.CALENDAR_EVENT_DELETE -> mapOf("event_id" to 1)
            AgentAndroidSystemNativeTools.DOWNLOAD_ENQUEUE -> mapOf("url" to "https://example.com/", "title" to "GalaxySSI device test")
            ...
            AgentWebMediaNativeTools.WEB_SEARCH -> mapOf("query" to "GalaxySSI", "max_results" to 3, "timeout_ms" to 10_000)
            AgentWebMediaNativeTools.CONTENT_EXTRACT -> mapOf("content" to "<h1>GalaxySSI</h1><p>Device test</p>")
            ...
            AgentNotificationNativeTools.NOTIFICATION_REPLY -> mapOf(
                "notification_key" to "galaxyssi-device-test-notification",
                "reply_text" to "GalaxySSI device test"
            )
    ...
    private fun sampleValue(name: String, schema: Map<*, *>): Any = when (name) {
        "target" -> ""
        "description" -> "GalaxySSI device contract test"
        "parameters" -> genericInput(schema.entries.associate { it.key.toString() to it.value })
        "requires_confirmation" -> true
        "bounds", "field_bounds" -> "[100,100][200,200]"
        "from_x", "from_y" -> "100"
        "to_x", "to_y" -> "200"
        "notification_key" -> "galaxyssi-device-test-notification"
        "reply_text" -> "GalaxySSI device test"
        "phone_number" -> "10086"
        "message", "text", "title" -> "GalaxySSI device test"
    ...
    private fun writeReport(toolCount: Int) {
        ...
        println("GALAXYSSI_NATIVE_TOOL_REPORT=${reportFile.absolutePath}")
    }
    ...
    private fun createTestImage(): Uri {
        ...
        val uri = requireNotNull(
            resolver.insert(
                ...
                ContentValues().apply {
                    put(MediaStore.Images.Media.DISPLAY_NAME, "galaxyssi-native-tool-test-${System.currentTimeMillis()}.png")
        ...
        Canvas(bitmap).apply {
            ...
            drawText("GalaxySSI DEVICE TEST", 36f, 130f, Paint(Paint.ANTI_ALIAS_FLAG).apply {
                color = Color.BLACK
                textSize = 48f
            })
        }
