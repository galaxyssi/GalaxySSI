...
class AgentRuntimeProjectVerificationPlannerTest {
    ...
    @Before
    fun setUp() {
        root = Files.createTempDirectory("galaxyssi-project-verifier-").toFile()
    }
    ...
    fun selectsTheProjectNativeNodeScriptAndPackageManager() {
        ...
        assertTrue(test.source.contains("GalaxySSI verification command: CI=1 pnpm run test"))
        assertEquals(listOf("node", "pnpm"), test.requiredExecutables)
        assertTrue(test.source.contains("command -v 'node'"))
        assertTrue(test.source.contains("GalaxySSI missing executable: pnpm"))
    }
    ...
    fun usesTheLockedUvEnvironmentForPythonVerification() {
        ...
        assertTrue(test.source.contains("GalaxySSI missing executable: uv"))
    }
