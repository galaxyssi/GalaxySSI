...
class AgentWebMediaNativeToolsTest {
    ...
    fun searchUsesFastProviderAndReturnsSchemaValidResults() {
        val html = """
            <html><body><li class="b_algo"><div class="b_algoheader"><a href="https://galaxyssi.example/result"><h2>GalaxySSI result</h2></a></div></li></body></html>
        ...
        val result = registry.invoke(
            AgentWebMediaNativeTools.WEB_SEARCH,
            mapOf("query" to "GalaxySSI", "max_results" to 2, "timeout_ms" to 10_000),
            webContext()
        )
        ...
        assertEquals("GalaxySSI result", first["title"])
        assertEquals("https://galaxyssi.example/result", first["url"])
    ...
    fun openHttpAndContentResultsSatisfyTheirDeclaredSchemas() {
        listOf(AgentWebMediaNativeTools.WEB_OPEN, AgentWebMediaNativeTools.BROWSER_RENDER).forEach { toolId ->
            ...
            assertTrue(result.toJson(), result.isSuccess)
            assertEquals("GalaxySSI", result.output["text"])
        }
    ...
    fun browserSessionUsesExplicitConversationScopedHandle() {
        ...
        val context = AgentNativeToolInvocationContext(
            conversationId = "conversation-1",
            callerId = "galaxyssi.mobile_agent",
    ...
    fun ocrUsesSelectedOrCapturedUriAndReflectsImplementationAvailability() {
        val ocr = FakeOcr(
            result = AgentOcrResult(
                "Galaxy SSI",
                listOf(AgentOcrLine("Galaxy SSI", 1, 2, 20, 12)),
                width = 100,
                height = 40,
        ...
        assertEquals("Galaxy SSI", result.output["text"])
