...
class AgentSupervisedProjectPromptTest {
        ...
        val raw = """
            {
              ...
              "actions":[
                {"ref":"branch","kind":"CALL_NATIVE_TOOL","target":"branch",
                 ...
                 "parameters":{"tool_id":"galaxyssi.project.repository.branch","arguments":{
        ...
        val raw = """
            {
              ...
              "actions":[
                {"ref":"list","kind":"CALL_NATIVE_TOOL","target":"list",
                 "depends_on":[],"use_outputs_from":[],
                 "parameters":{"tool_id":"galaxyssi.workspace.files.list","arguments":{
    ...
    fun `wraps an unambiguous single action object locally`() {
        val raw = """
            {"execution_location":"phone","summary":"Inspect the repository.","actions":
              {"ref":"inspect","kind":"CALL_NATIVE_TOOL","target":"galaxyssi.project.repository.inspect",
               "parameters":{"tool_id":"galaxyssi.project.repository.inspect","arguments":{"workspace_id":"current"}}}
            }
        """.trimIndent()
    ...
    fun `lifts direct native tool fields into the strict action schema locally`() {
        val raw = """
            {"execution_location":"phone","summary":"Inspect the repository.","action":
              {"ref":"inspect","tool_id":"galaxyssi.project.repository.inspect",
        ...
        assertEquals("galaxyssi.project.repository.inspect", action.getString("target"))
        assertEquals("galaxyssi.project.repository.inspect", parameters.getString("tool_id"))
    ...
    fun `moves flattened native tool parameters into arguments locally`() {
        val raw = """
            {"execution_location":"phone","actions":[
              {"ref":"read","parameters":{"tool_id":"galaxyssi.workspace.file.read.text",
        ...
        assertEquals("galaxyssi.workspace.file.read.text", parameters.getString("tool_id"))
                ...
                org.json.JSONArray().put(
                    JSONObject()
                        ...
                        .put(
                            "parameters",
                            JSONObject()
                                .put("tool_id", "galaxyssi.project.repository.inspect")
        ...
        val status = """
            {"execution_location":"phone","actions":[
              {"ref":"status","kind":"CALL_NATIVE_TOOL","target":"status",
               "depends_on":[],"use_outputs_from":[],
               "parameters":{"tool_id":"galaxyssi.project.repository.status","arguments":{"workspace_id":"current"}}}
            ]}
        """.trimIndent()
        val branch = """
            {"execution_location":"phone","actions":[
        ...
        val raw = """
            {"execution_location":"phone","actions":[
              {"ref":"history","kind":"CALL_NATIVE_TOOL","target":"history",
               "depends_on":[],"use_outputs_from":[],
               "parameters":{"tool_id":"galaxyssi.project.repository.history","arguments":{"workspace_id":"current"}}}
            ]}
        """.trimIndent()
        ...
        val raw = """
            {"execution_location":"phone","actions":[
              {"ref":"pr","kind":"CALL_NATIVE_TOOL","target":"pull-request",
               "depends_on":[],"use_outputs_from":[],
               "parameters":{"tool_id":"galaxyssi.project.repository.pull_request.create","arguments":{
    ...
    fun `prompt delegates completion intent to the model and evidence validation to Android`() {
        val prompt = AgentSupervisedProjectLoop.planningPrompt(
            request("Improve GalaxySSI and submit a pull request")
        )
    ...
    fun `recovery summaries explain changed evidence and approach`() {
        val action = AgentAction(
            id = "build",
            kind = AgentActionKind.CALL_NATIVE_TOOL,
            target = "galaxyssi.runtime.execute",
            risk = AgentRisk.LOW,
    ...
    fun `continuation uses a smaller equivalent contract without dropping phone boundaries`() {
        val request = request("Improve GalaxySSI on this phone and submit a pull request")
    ...
    fun `long project goal preserves its objective and final acceptance criteria`() {
        val opening = "Update the GalaxySSI Android project on the phone."
    ...
    fun `auto model continuation receives recent sanitized phone tool observations`() {
        ...
        assertTrue(prompt.contains("GalaxySSI-owned context"))
    ...
    fun `single action iteration removes stale dependencies from earlier rounds`() {
        val raw = """
            {"execution_location":"phone","summary":"Create the feature branch.","actions":[
              {"ref":"branch","kind":"CALL_NATIVE_TOOL","target":"galaxyssi.project.repository.branch.checkout",
               "description":"Create the feature branch","depends_on":["pull_latest"],"use_outputs_from":["pull_latest"],
               "parameters":{"tool_id":"galaxyssi.project.repository.branch.checkout","arguments":{
    ...
    fun `continuation prompt retains repository evidence from the conversation`() {
        ...
        val context = AgentConversationContext(
            ...
            turns = listOf(
                AgentTranscriptEntry(
                    id = "user-1",
                    role = AgentTranscriptRole.USER,
                    text = "开发 https://github.com/galaxyssi/GalaxySSI，测试并提交 PR",
                    timestampMillis = 1L,
                    conversationId = "prompt-test",
                    turnId = "turn-1"
                )
        ...
        assertTrue(prompt.contains("https://github.com/galaxyssi/GalaxySSI"))
    ...
    fun `project tools are ordered ahead of generic runtime tools`() {
        ...
        val ordered = AgentSupervisedProjectToolInventory.ordered(
            listOf(
                descriptor("galaxyssi.runtime.execute"),
                descriptor("galaxyssi.workspace.file.read"),
        ...
        assertEquals("galaxyssi.workspace.file.read", ordered[2])
        assertEquals("galaxyssi.runtime.execute", ordered[3])
    }
    ...
    fun `phase manifest compacts inactive schemas without hiding tools`() {
        fun descriptor(index: Int): AgentNativeToolDescriptor = AgentNativeToolDescriptor(
            id = "galaxyssi.workspace.synthetic.tool$index",
            version = "1.0.0",
    ...
    fun `generic runtime cannot execute project Git commands`() {
        val rawGitAction = """
            {"execution_location":"phone","summary":"Inspect Git.","actions":[
              {"ref":"inspect","kind":"CALL_NATIVE_TOOL","target":"galaxyssi.runtime.execute",
               "description":"Inspect repository","depends_on":[],"use_outputs_from":[],
               "parameters":{"tool_id":"galaxyssi.runtime.execute","arguments":{
    ...
    fun `format repair preserves correction after a full provider neutral context`() {
        val invalidTool = "galaxyssi.project.repository.state"
    ...
    fun `auto provider context keeps the durable summary and newest complete turn`() {
        ...
        assertTrue(prompt.contains(AgentTranscriptStore.GALAXYSSI_CONTEXT_TRANSPORT_HEADER))
        assertTrue(prompt.length <= 24_000)
    }
    ...
    private fun largeProjectToolCatalog(): List<AgentNativeToolDescriptor> = (1..64).map { index ->
        AgentNativeToolDescriptor(
            id = "galaxyssi.project.test.tool.${index.toString().padStart(2, '0')}",
            version = "1.0.0",
