...
class AgentRuntimeProjectWorkspaceTest {
    ...
    @Before
    fun setUp() {
        root = Files.createTempDirectory("galaxyssi-runtime-project-").toFile()
    ...
    fun stagesHostInputsForOneRunWithoutPersistingThemInTheProject() {
        ...
        val staged = File(prepared.directory, ".galaxyssi-inputs/archives/project.tar.gz")
        ...
        assertFalse(File(projectRoot, "workspace-one/.galaxyssi-inputs").exists())
    }
    ...
    fun repositoryCloneCanonicalizesCommonModelUrlAliasesBeforeValidation() {
        listOf("url", "repo_url", "repository").forEach { alias ->
            val bound = AgentWorkspaceScope.bindToolInput(
                ...
                mapOf(alias to "https://github.com/galaxyssi/GalaxySSI", "workspace_id" to "attacker"),
                "conversation-project"
            )

            assertEquals("conversation-project", bound["workspace_id"])
            assertEquals("https://github.com/galaxyssi/GalaxySSI", bound["repository_url"])
