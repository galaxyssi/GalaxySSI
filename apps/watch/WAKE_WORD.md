...
A foreground home detection vibrates and opens Samsung input. A background detection vibrates and posts **Tap to start voice input**. If the user has enabled the existing voice accessibility service, it requests GalaxySSI voice entry directly. The main activity requests the display to turn on and waits for an unlocked, focused foreground window before starting Samsung input. Without that service, or when the OEM declines the launch, the user taps the notification and unlocks if necessary. The app never enables accessibility on the user’s behalf. The service does not bypass Android background activity launch or lock-screen restrictions. Detection has an eight-second background cooldown. The notification has **Stop listening**, which disables both listening settings. The microphone service is non-sticky and is not started by boot receivers; reopen GalaxySSI after reboot or process termination to resume an enabled mode.
