...
SignalASI acts as an ACP client. Each configured agent owns a long-lived
subprocess while SignalASI owns conversation isolation, cancellation,
...
log = logging.getLogger("signalasi.acp_runtime")
...
class AcpRuntime:
    ...
    def _message_id(run_id: str) -> str:
        ...
        try:
            ...
            return str(uuid.uuid5(uuid.NAMESPACE_URL, f"signalasi:acp:{run_id}"))
    ...
    def _agent_environment(agent_id: str) -> dict[str, str]:
        env = dict(os.environ)
        env["SIGNALASI_AGENT_MODE"] = "1"
        env["SIGNALASI_ACP_CLIENT"] = "1"
        env["SIGNALASI_ACP_AGENT_ID"] = agent_id
        if (
            agent_id == "hermes"
            and str(
                env.get("SIGNALASI_HERMES_ACP_USE_USER_EXTENSIONS") or ""
            ).strip().casefold() not in {"1", "true", "yes", "on"}
        ):
            # SignalASI owns the MCP/Skill permission boundary. Loading the same
