...
def run(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="signalasi-evolution-runtime-") as state_dir:
        ...
        continuity_token.write_text("signalasi-candidate-reload-v1\n", encoding="utf-8")
        cycles = [
            _run_cycle(
                backend,
                state_root,
        ...
        if continuity_token.read_text(encoding="utf-8") != "signalasi-candidate-reload-v1\n":
...
def _run_cycle(
    ...
    *,
    ...
    env = {
        **os.environ,
        "SIGNALASI_STATE_DIR": str(state_root),
