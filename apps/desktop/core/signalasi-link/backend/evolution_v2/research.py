...
DEFAULT_SOURCES: dict[str, Any] = {
    "schema": "signalasi.evolution-sources.v2",
...
class TechnologyRadar:
    ...
    def run(self, query: str = "", *, trusted_only: bool = False, limit: int = 30) -> ResearchRun:
        run = ResearchRun(
            ...
            query=str(query or "").strip()[:500] or "SignalASI agent ecosystem radar",
    ...
    def proposals(self, run: ResearchRun, *, maximum: int = 8) -> list[EvolutionProposal]:
        ...
        for item in run.items:
            ...
            proposal = EvolutionProposal(
                ...
                title=f"Evaluate {item.name} for SignalASI",
                problem=(
                    f"Assess {item.repository} as a {', '.join(item.integration_targets[:3]) or 'capability'} "
                    "and integrate only the minimal interoperable ideas that improve SignalASI without importing untrusted runtime code."
                ),
                scope=scope,
    ...
    def _score(self, metadata: dict[str, Any], *, trusted_only: bool) -> RadarItem | None:
        ...
        rationale = [
            ...
            f"Matched SignalASI fit terms: {', '.join(matches[:8]) or 'none'}.",
...
def _integration_shape(item: RadarItem) -> tuple[list[str], list[str]]:
    scope = [
        "apps/desktop/core/signalasi-link/backend/evolution_v2",
        "config",
        "docs",
    ]
    acceptance = [
        ...
        "Add a narrow adapter behind a feature flag only when an existing SignalASI abstraction cannot satisfy the use case.",
