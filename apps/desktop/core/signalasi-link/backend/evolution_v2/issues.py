...
RULES = (
    IssueRule(
        ...
        "high",
        (
            "apps/android/app/src/main/java/com/signalasi/chat",
    ...
    IssueRule(
        "android-crash",
        re.compile(r"(?i)(FATAL EXCEPTION|Process: com\.signalasi\.chat|ANR in com\.signalasi\.chat)"),
    ...
    IssueRule(
        "desktop-crash",
        re.compile(r"(?i)(uncaught exception|backend exited|renderer process gone|ECONNREFUSED).{0,150}(8765|SignalASI|backend)?"),
        "Desktop renderer or backend became unavailable",
        "high",
        ("apps/desktop/src", "apps/desktop/core/signalasi-link/backend", "apps/desktop/scripts"),
        (
