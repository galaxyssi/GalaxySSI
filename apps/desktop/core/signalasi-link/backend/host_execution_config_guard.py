...
PROTOCOL = "signalasi.host-execution-config-guard/1.0"
...
_EXACT_RULES = {
    ".signalasi-task.json": (
        "signalasi_host_metadata",
        "SignalASI task metadata is owned by the host",
    ),
    ".mcp.json": (
...
_TREE_RULES = {
    ".signalasi": (
        "signalasi_host_control",
        "SignalASI execution checkpoints and control state are host-owned",
    ),
    ".github/workflows": (
        "ci_automatic_execution",
...
def _restore_snapshot(
    ...
    for relative in sorted(
        ...
        temporary = target.with_name(
            f".{target.name}.signalasi-restore-{uuid.uuid4().hex}.tmp"
        )
        try:
...
def _append_audit(
    ...
    *,
    ...
    configured_state_root = str(os.environ.get("SIGNALASI_STATE_DIR") or "").strip()
    state_root = (
