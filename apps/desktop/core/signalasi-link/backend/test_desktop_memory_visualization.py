...
class DesktopMemoryVisualizationTest(unittest.TestCase):
    ...
    def test_current_state_summarizes_temporal_and_namespace_boundaries(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            current = store.remember(
                "SignalASI phone battery status is available",
                kind="device_state",
            ...
            store.remember(
                "Plan the next SignalASI release",
                kind="goal",
            ...
            self.assertEqual(
                ...
                {"device:phone", "project:signalasi"},
            )
    ...
    def test_timeline_orders_memory_candidate_review_and_audit_events(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            store.remember(
                "SignalASI release status is ready",
                kind="project_state",
                key="release-status",
            )
            clock.advance()
    ...
    def test_graph_returns_real_nodes_relations_and_evidence_links(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            memory = store.remember(
                "SignalASI phone uses Qwen model",
                kind="device_state",
                namespace="device:phone",
                key="phone-model",
    ...
    def test_evidence_chain_preserves_versions_sources_and_graph_references(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            second = store.remember(
                "SignalASI phone uses Gemma model",
                kind="device_state",
                namespace="device:phone",
                key="phone-model",
    ...
    def test_lifecycle_events_distinguish_superseded_and_retracted_memory(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            store = DesktopMemoryStore(Path(directory) / "memory.db", now=clock)
            first = store.remember(
                "SignalASI status is preparing",
                key="runtime-status",
            )
            clock.advance()
            store.remember(
                "SignalASI status is ready",
                key="runtime-status",
            )
            clock.advance()
            removed = store.remember(
                "SignalASI temporary notice remains visible",
