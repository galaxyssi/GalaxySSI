...
PROTOCOL = "signalasi.secure-state/1.0"
CIPHER = "AES-256-GCM"
MASTER_KEY_ENV = "SIGNALASI_STATE_MASTER_KEY"
MASTER_KEY_NAME = ".signalasi-state-key"
...
_DPAPI_ENTROPY = b"SignalASI Desktop secure state master key v1"
_TEXT_PREFIX = "enc:v1:"
...
def _derive_storage_key(path: Path, purpose: str, *, length: int) -> bytes:
    ...
    return HKDF(
        algorithm=SHA256(),
        length=length,
        salt=b"SignalASI secure state HKDF v1",
...
def _dpapi_call(function_name: str, value: bytes) -> bytes:
    ...
    description = "SignalASI secure state" if function_name == "CryptProtectData" else None
    result = function(
