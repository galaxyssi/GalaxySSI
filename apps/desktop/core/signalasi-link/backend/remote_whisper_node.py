...
from signalasi_client import desktop_id, desktop_name
...
class RemoteWhisperNodeConfig:
    ...
    temp_root: Path = field(
        default_factory=lambda: Path(
            os.environ.get("SIGNALASI_REMOTE_WHISPER_TEMP", "").strip()
            or Path(tempfile.gettempdir()) / "SignalASI" / "remote-whisper"
        )
    )
    ...
    def from_environment(cls) -> "RemoteWhisperNodeConfig":
        return cls(
            enabled=os.environ.get("SIGNALASI_REMOTE_WHISPER_ENABLED", "1").strip().lower()
            not in {"0", "false", "off", "no"},
            model_profile_id=os.environ.get(
                "SIGNALASI_REMOTE_WHISPER_PROFILE",
                os.environ.get("SIGNALASI_WHISPER_MODEL", "medium"),
            ).strip()
            or "medium",
            max_pcm_bytes=max(
                32_000,
                int(os.environ.get("SIGNALASI_REMOTE_WHISPER_MAX_PCM_BYTES", str(16_000 * 2 * 120))),
            ),
            workers=max(1, min(2, int(os.environ.get("SIGNALASI_REMOTE_WHISPER_WORKERS", "1")))),
        )
    ...
    def __init__(
        ...
        *,
        ...
        self._executor = ThreadPoolExecutor(
            ...
            thread_name_prefix="signalasi-remote-whisper",
        )
