"""Verified runtime inventory for the SignalASI Desktop super agent."""
...
class DesktopRuntimeManager:
    ...
    def _probe_asr(self) -> dict:
        ...
        model = self.environment.get("SIGNALASI_WHISPER_MODEL", "").strip() or "medium"
    ...
    def _resolve_command(self, runtime_id: str, command: RuntimeCommand) -> str | None:
        env_key = f"SIGNALASI_RUNTIME_{runtime_id.upper()}_{command.name.upper()}"
        primary_env = {
            ("python", "python"): "SIGNALASI_PYTHON",
            ("uv", "uv"): "SIGNALASI_UV",
            ("node", "node"): "SIGNALASI_NODE",
    ...
    def _python_command(self) -> str | None:
        configured = self.environment.get("SIGNALASI_PYTHON", "").strip()
        return (
    ...
    def _source(self, command: str, runtime_id: str) -> str:
        ...
        configured_values = [
            ...
            if (key.startswith("SIGNALASI_RUNTIME_") or key in {"SIGNALASI_PYTHON", "SIGNALASI_UV", "SIGNALASI_NODE"})
            and value.strip()
        ]
        if command in configured_values:
            return "configured"
        normalized = str(Path(command)).replace("\\", "/").casefold()
        data_root = self.environment.get("SIGNALASI_DATA_DIR", "").strip()
