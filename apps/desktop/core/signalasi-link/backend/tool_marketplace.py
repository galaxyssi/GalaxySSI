...
CONTRACT = "signalasi.tool-marketplace/1.1"
...
AUTOMATION_CATALOG = (
    MarketplaceDefinition(
        ...
        permissions=(
            permission("desktop.status.read", "Read Desktop status", "Read local SignalASI runtime health.", risk="low"),
    ...
    def _native_items(self) -> list[dict[str, Any]]:
        ...
        for tool in list(manifest.get("tools") or []):
            ...
            definition = MarketplaceDefinition(
                ...
                release_notes="Delivered and updated with the signed SignalASI Desktop release.",
            )
            result.append(
                definition.public(
                    state=state,
