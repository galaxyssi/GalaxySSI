...
class HostExecutionConfigGuardTests(unittest.TestCase):
    def setUp(self):
        ...
        self.environment = patch.dict(
            os.environ,
            {"SIGNALASI_STATE_DIR": str(self.root / "state")},
        )
    ...
    def test_policy_protects_host_execution_surfaces_without_blocking_source(self):
        protected = (
            ".signalasi-task.json",
            ".signalasi/execution-checkpoint.json",
    ...
    def test_guard_restores_modified_host_state_and_removes_new_workflow(self):
        metadata = self.workspace / ".signalasi-task.json"
        ...
        self.assertEqual(
            {".signalasi-task.json", ".github/workflows", ".github/workflows/pwn.yml"},
        ...
        self.assertEqual("signalasi.host-execution-config-guard/1.0", audit["protocol"])
