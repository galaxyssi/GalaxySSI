...
class DesktopRuntimeManagerTests(unittest.TestCase):
    ...
    def manager(self, **overrides):
        values = {
            ...
            "environment": {"SIGNALASI_WHISPER_MODEL": str(self.model)},
            "python_executable": "",
    ...
    def test_inventory_reports_verified_capabilities_and_partial_components(self):
        ...
        self.assertEqual("signalasi.desktop-runtime/1.0", snapshot["contract_version"])
    ...
    def test_explicit_runtime_override_is_reported_as_configured(self):
        self.commands["custom-ffmpeg"] = "D:/SignalASI/ffmpeg.exe"
        manager = self.manager(environment={
            "SIGNALASI_WHISPER_MODEL": str(self.model),
            "SIGNALASI_RUNTIME_FFMPEG_FFMPEG": "custom-ffmpeg",
        })

        runtime = next(item for item in manager.snapshot()["runtimes"] if item["id"] == "ffmpeg")

        self.assertEqual("D:/SignalASI/ffmpeg.exe", runtime["commands"]["ffmpeg"])
