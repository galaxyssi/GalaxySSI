"""Bounded three-layer Desktop perception for SignalASI agents."""
...
class WindowsUiAutomationProvider:
    def __call__(self, max_elements: int, max_depth: int) -> dict[str, Any]:
        ...
        result = _run_powershell_json(
            ...
            environment={
                "SIGNALASI_UI_MAX_ELEMENTS": str(max_elements),
                "SIGNALASI_UI_MAX_DEPTH": str(max_depth),
            },
            timeout_seconds=6.0,
        )
...
class WindowsMediaOcrProvider:
    def __call__(self, image_bytes: bytes, max_chars: int) -> dict[str, Any]:
        ...
        try:
            ...
            result = _run_powershell_json(
                _WINDOWS_OCR_SCRIPT,
                environment={"SIGNALASI_OCR_IMAGE": temporary_path},
...
public static class SignalASIUser32 {
  [DllImport("user32.dll")]
  public static extern IntPtr GetForegroundWindow();
}
'@
$maxElements = [Math]::Max(1, [Math]::Min(120, [int]$env:SIGNALASI_UI_MAX_ELEMENTS))
$maxDepth = [Math]::Max(1, [Math]::Min(12, [int]$env:SIGNALASI_UI_MAX_DEPTH))
$handle = [SignalASIUser32]::GetForegroundWindow()
...
$path = [string]$env:SIGNALASI_OCR_IMAGE
