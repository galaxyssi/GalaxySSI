"""Cross-process ownership for the GalaxySSI Desktop external-service backend."""
...
class BackendInstanceLock:
    def __init__(self, name: str = "GalaxySSI.Backend.ExternalServices.v1") -> None:
        self.name = name
        self._handle = None
        self._file = None
    ...
    def _acquire_windows(self) -> None:
        ...
        if not handle:
            raise OSError(ctypes.get_last_error(), "Could not create the GalaxySSI backend mutex")
        if ctypes.get_last_error() == 183:
            kernel32.CloseHandle(handle)
            raise BackendInstanceAlreadyRunning("Another GalaxySSI external-service backend is already running")
        self._handle = handle
    ...
    def _acquire_file(self) -> None:
        ...
        try:
            ...
            raise BackendInstanceAlreadyRunning(
                "Another GalaxySSI external-service backend is already running"
            ) from error
