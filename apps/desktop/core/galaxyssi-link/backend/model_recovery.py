...
RECOVERY_ACTION_HEADER = "[GALAXYSSI_RECOVERY_ACTION_V1]"
RECOVERY_ACTION_FOOTER = "[/GALAXYSSI_RECOVERY_ACTION_V1]"
...
def recovery_contract(attachments: Iterable[ContextAttachment]) -> str:
    ...
    return (
        "GalaxySSI recovery contract (internal control protocol):\n"
        ...
        "using an exact artifact_id from the catalog. GalaxySSI will securely restore it and "
...
def recovery_follow_up(
    ...
    if decision.action == ModelRecoveryAction.REQUEST_ATTACHMENT:
        return (
            "GalaxySSI restored the attachment(s) you requested. Continue the same user goal "
...
def failure_review_prompt(
    *,
    ...
    return (
        "Act as GalaxySSI's recovery controller. Diagnose the failed execution from the bounded "
