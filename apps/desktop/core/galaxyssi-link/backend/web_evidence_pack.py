...
EVIDENCE_PACK_PROTOCOL = "galaxyssi.web-evidence-pack.v1"
PACK_OPERATIONS = {
    ...
    for _, encoded in encoded_tool_results:
        ...
        for item in pack.get("items", []) if isinstance(pack, Mapping) else []:
            ...
            if url and url not in allowed:
                allowed.append(url)
    lines = [
        f"Your draft did not pass GalaxySSI citation verification (status={validation.status}). Rewrite the complete user-facing answer once.",
