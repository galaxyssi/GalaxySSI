...
class RecoveryQueryTest(unittest.TestCase):
    def setUp(self):
        ...
        self.task = dict(zip(TASK_FIELDS, self.item.values())) | dict(
            status="running", status_seq=7, run_id="remote-run-a", execution_generation=1)
    ...
    def test_exact_scope_reads_current_status_without_execution(self):
        ...
        self.manager.recovery_snapshot.assert_called_once_with("task-a", client_route_id="route-a",
    ...
    def test_missing_task_is_not_replayed_or_fabricated(self):
        self.manager.recovery_snapshot.return_value = None
    ...
    def test_result_has_no_prompt_result_or_tool_payload(self):
        ...
        result = self.query()["items"][0]
        self.assertEqual(set(IDENTITY_FIELDS) | {"status", "remote_run_id", "status_sequence", "execution_generation"}, set(result))
    ...
    def test_repeated_queries_are_read_only_and_observe_newer_state(self):
        ...
        self.task["execution_generation"] = 2
        second = self.query()
        self.assertEqual("running", first["items"][0]["status"])
        self.assertEqual("completed", second["items"][0]["status"])
        self.assertEqual(2, second["items"][0]["execution_generation"])
        self.assertEqual(2, self.manager.recovery_snapshot.call_count)
