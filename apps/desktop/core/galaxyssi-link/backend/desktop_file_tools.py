...
def _run_excel_conversion(source: Path, target: Path, output_format: str, timeout: int = 45) -> None:
    ...
    action = (
        f"$book.ExportAsFixedFormat({format_code}, $env:GALAXYSSI_OUTPUT)"
        if output_format == "pdf"
        else f"$book.SaveAs($env:GALAXYSSI_OUTPUT, {format_code})"
    )
    script = f"""
...
$book = $null
try {{
    $book = $excel.Workbooks.Open($env:GALAXYSSI_INPUT)
    {action}
    $book.Close($false)
    $book = $null
}} finally {{
