"""Trusted workflow skills for the GalaxySSI Desktop super agent."""
...
class DesktopSkillRegistry:
    ...
    def upsert(self, value: dict[str, Any]) -> dict[str, Any]:
        ...
        if not SKILL_ID.fullmatch(skill_id) or skill_id.startswith("galaxyssi."):
            raise ValueError("Custom skill id must be valid and cannot use the galaxyssi namespace")
    ...
    def set_enabled(self, skill_id: str, enabled: bool) -> dict[str, Any]:
        if skill_id.startswith("galaxyssi."):
    ...
    def delete(self, skill_id: str) -> bool:
        if str(skill_id).startswith("galaxyssi."):
            return False
        with self._lock:
            rows = self._load_user()
