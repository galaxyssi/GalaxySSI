...
def workspace_root() -> Path:
    configured = os.environ.get("GALAXYSSI_WORKSPACE_ROOT", "").strip()
...
def task_workspace(task_id: str = "", agent_id: str = "") -> Path:
    ...
    metadata = directory / ".galaxyssi-task.json"
...
def referenced_relative_artifact_paths(content: str, limit: int = 20) -> list[PurePosixPath]:
    """Extract bounded GalaxySSI-owned relative artifact paths from model output."""
