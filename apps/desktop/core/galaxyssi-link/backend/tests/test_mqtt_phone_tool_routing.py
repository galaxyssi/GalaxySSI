...
class MqttPhoneToolRoutingTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.manifest_hash = phone_tool_broker.compute_manifest_hash(
            {"revision": 1, "tools": ["galaxyssi.workspace.file.read.text"]}
        )
        ...
        self.first = self._pair_client("first-route", "galaxyssi:first-phone")
        self.second = self._pair_client("second-route", "galaxyssi:second-phone")

    def test_phone_development_v2_uses_exact_content_transport(self) -> None:
        source = "def main():\n    print('ok')\n"
        manifest = json.dumps(
            {
                "schema": "galaxyssi.phone-development-manifest.v2",
                "language": "python",
                "entry_file": "main.py",
    ...
    def test_malformed_manifest_fallback_remains_exact(self) -> None:
        malformed = '{"schema":"galaxyssi.phone-development-manifest.v2"'
