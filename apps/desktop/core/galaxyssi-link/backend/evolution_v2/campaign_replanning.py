...
def planning_messages(graph: dict, store) -> list[dict]:
    ...
    return [{"role": "system", "content": (
        ...
        "A final_goal_observation assesses the original goal after all child tasks finished. "
        "If it establishes missing original-goal work, use revise to add tasks while preserving every completed node. "
        "An inconclusive final assessment or unavailable evaluator is not proof that implementation is wrong; "
        "wait for evidence or evaluator recovery rather than rewrite completed work. "
