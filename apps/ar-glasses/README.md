...
- 内置 Vosk 轻量中文和英文离线模型。应用在前台时用英文唤醒词专用识别器持续监听 `Hello Hello`，不显示麦克风开关（连续说或 7 秒内分两次说）；唤醒前不会把其它语音记为问题。唤醒后切换到中英文问题识别，最终结果出现后等待 1.5 秒，期间继续说话会重置计时；然后自动发送。未配置 Agent 时保持监听并提示手机配置。VENUS 上的 Vosk Small 中文准确率有限，不能保证所有短指令均被正确转写。
