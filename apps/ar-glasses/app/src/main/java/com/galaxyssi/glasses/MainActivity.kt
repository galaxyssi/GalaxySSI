...
class MainActivity : Activity() {
    ...
    private fun navigate(destination: String) {
        if (destination == "settings") { cancelAutoSend(); awake = false; livePartial = ""; stopListening() }
        if (page == "settings" && destination != "settings") stopPhoneSetup()
        page = destination; render()
        if (destination == "settings" && resumed) startPhoneSetup()
        if (destination == "chat" && resumed && handsFreeEnabled) startListening()
    }
    ...
    private fun updateConversationView() {
        ...
        when {
            ...
            replyVisible -> { title.text = "GalaxySSI"; subtitle.text = "回复"; answer.text = latestAnswer().orEmpty() }
                ...
                subtitle.text = when {
                    !listening -> "麦克风已暂停"
                    profile() == null -> "请先在手机 GalaxySSI 配置 AR 眼镜"
    ...
    private fun stopPhoneSetup() { setupServer?.close(); setupServer = null }
        ...
        try {
            recognizer = BilingualSpeech(this, chineseModel!!, englishModel!!,
                ...
                onPartial = { partial -> if (listening) {
                    ...
                    if (awake) {
                        val words = wakePhrase.replaceFirst(partial, "").trim()
                        if (words.isNotBlank() && !singleHello.matches(words) && words != livePartial) {
                            livePartial = words
                            setStatus("正在识别 · 停顿 1.5 秒后发送")
                ...
                onError = { reason ->
                    ...
                    if (resumed && handsFreeEnabled && page == "chat") main.postDelayed({ if (resumed) startListening() }, 1200)
                })
            listening = true
            recognizer?.start()
            micButton?.text = "暂停"
    ...
    private fun recognized(text: String) {
        ...
        val clean = wakePhrase.replaceFirst(text, "").trim().trim(',', '，', '。', '.', '!', '?', ' ')
        if (clean.isBlank() || singleHello.matches(clean)) { updateConversationView(); return }
        val command = clean.replace(" ", "").replace("。", "")
        if (command.isBlank()) return
        when (command) {
            "发送", "提交", "确认发送" -> sendDraft()
            "取消", "清空" -> { cancelAutoSend(); draft = ""; setStatus("已清空 · 正在听"); updateConversationView() }
            "朗读", "读出来" -> { stopListening(); latestAnswer()?.let(::speak) }
            "停止朗读", "别读了" -> { stopSpeaking(); setStatus("已停止播报") }
            "新对话" -> { cancelAutoSend(); awake = false; newSession(); render() }
            "设置", "打开设置" -> navigate("settings")
            "返回", "打开对话", "对话" -> navigate("chat")
    ...
    override fun onResume() {
        super.onResume()
        resumed = true
        if (page == "settings") startPhoneSetup()
        if (handsFreeEnabled && ::state.isInitialized && page == "chat") window.decorView.post { if (resumed) startListening() }
    }
