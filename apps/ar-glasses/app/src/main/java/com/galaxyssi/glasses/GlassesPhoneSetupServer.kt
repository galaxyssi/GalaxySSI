...
internal class GlassesPhoneSetupServer(
    ...
    fun start() {
        ...
        Thread({
            try {
                ...
                deviceName = "GalaxySSI AR · " + android.os.Build.MODEL
                val keys = KeyStore.getInstance("AndroidKeyStore").apply { load(null) }
                val alias = "glasses-phone-setup-ec-v1"
                if (!keys.containsAlias(alias)) {
                    KeyPairGenerator.getInstance("EC", "AndroidKeyStore").apply {
                        initialize(KeyGenParameterSpec.Builder(alias, KeyProperties.PURPOSE_SIGN or KeyProperties.PURPOSE_VERIFY)
                            ...
                            .setCertificateSubject(X500Principal("CN=GalaxySSI Glasses Setup"))
    ...
    private fun advertise() {
        ...
        runCatching { nsd.registerService(NsdServiceInfo().apply {
            serviceName = deviceName; serviceType = "_galaxyssi-glasses._tcp."; port = this@GlassesPhoneSetupServer.port
            setAttribute("v", "1")
    ...
    companion object {
        ...
        fun verificationCode(cert: ByteArray, peer: ByteArray, own: ByteArray): String {
            // Keep the watch protocol domain string so the shared phone pairing client computes the same code.
