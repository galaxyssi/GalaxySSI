...
final class GlobalAutonomousToolCatalogTests: XCTestCase {
  ...
  func testPromptBlockNamesHostValidationAndExactSchemas() throws {
    ...
    XCTAssertTrue(prompt.contains("id=signalasi.web.search"))
  ...
  func testExecuteProducesNativeToolReceiptEvidence() throws {
    ...
    XCTAssertEqual(observedContext?.callerId, "signalasi.global_super_agent")
    XCTAssertEqual(observedContext?.attributes["execution_authority"], "signalasi-ios")
  }
