...
final class SignalASIPeerVoiceTranscriptionTests: XCTestCase {
  func testVoiceMessagesOfferTranscriptionAndDeleteOnly() {
    let message = peerVoiceMessage()

    XCTAssertNotNil(SignalASIPeerMessageActionPolicy.voiceAttachment(in: message))
    XCTAssertEqual(
      SignalASIPeerMessageActionPolicy.actions(for: message),
      [.transcribe, .delete]
    )
  }
  ...
  func testPeerTranscriptionReturnsTextWithoutCommandExecution() {
    XCTAssertTrue(SignalASIPeerMessageActionPolicy.returnsTextWithoutCommandExecution())
  }
  ...
  func testPcmWaveDecodesAndResamplesEntirelyInMemory() throws {
    ...
    let wave = SignalASIPeerVoiceOpusCodec.pcmWave(source)

    var decoded = try SignalASIPeerVoiceInMemoryDecoder.decode(
      wave,
      fileExtension: "wav",
      mimeType: "audio/wav"
    )
