...
final class GlobalMemoryContextTests: XCTestCase {
  func testGlobalTopicProjectGraphRanksConversationAndProjectNodes() {
    let currentConversation = GlobalTopicNode(
      ...
      name: "SignalASI iOS memory active parity",
    ...
    let project = GlobalTopicNode(
      id: "project-ios",
      stableKey: "project:signalasi-ios",
      name: "SignalASI iOS project",
      kind: .project,
    ...
    let archived = GlobalTopicNode(
      ...
      name: "SignalASI iOS memory archived",
      status: .archived,
      conversationIds: ["chat-a"],
      confidence: 1,
      firstSeenAtMillis: 1_000,
    ...
    let relevant = graph.relevant(query: "SignalASI iOS memory", conversationId: "chat-a", limit: 3)
  ...
  func testGlobalMemoryPromptCompilerSelectsTemporalWorldSafely() {
    let world = PersonalWorldModel(items: [
      contextWorldItem(
        ...
        kind: .goal,
        namespace: .project,
        namespaceId: "signalasi-ios",
        topic: "Project SignalASI-ios goal",
        value: "Current SignalASI iOS goal is durable context parity",
        conversationIds: ["chat-a"],
      ...
      contextWorldItem(
        ...
        namespace: .project,
        namespaceId: "signalasi-ios",
        topic: "Project SignalASI-ios previous decision",
      ...
      contextWorldItem(
        ...
        namespace: .project,
        namespaceId: "signalasi-ios",
        topic: "Project SignalASI-ios hidden",
      ...
      contextWorldItem(
        ...
        namespace: .project,
        namespaceId: "signalasi-ios",
        topic: "Project SignalASI-ios expired",
      ...
      contextWorldItem(
        ...
        namespace: .project,
        namespaceId: "signalasi-ios",
        topic: "Project SignalASI-ios wrong conversation",
    ...
    let compiled = GlobalMemoryPromptCompiler.compile(
      ...
      query: "What changed previously and what is the current project SignalASI-ios goal?",
    ...
    XCTAssertTrue(compiled.contains("Current SignalASI iOS goal is durable context parity"))
  ...
  func testGlobalMemoryPromptCompilerIncludesConflictAndGraphSections() {
    let world = PersonalWorldModel(items: [
      contextWorldItem(
        "conflict",
        kind: .risk,
        topic: "SignalASI Desktop support conflict",
    ...
    let entityGraph = GlobalEntityMemoryGraph(
      nodes: [
        contextEntity("signalasi", label: "SignalASI", kind: .application, lastSeenAtMillis: 3_000),
        contextEntity("desktop", label: "Desktop", kind: .device, lastSeenAtMillis: 2_000)
      ],
      relations: [
        contextRelation("supports", from: "signalasi", to: "desktop", kind: .supports, confidence: 0.95, lastSeenAtMillis: 3_000)
      ]
    )
    let topicGraph = GlobalTopicProjectGraph(nodes: [
      GlobalTopicNode(
        ...
        name: "SignalASI Desktop support project",
        conversationIds: ["chat-a"],
        confidence: 0.9,
        lastSeenAtMillis: 3_000
      )
    ...
    let compiled = GlobalMemoryPromptCompiler.compile(
      ...
      query: "Does SignalASI support Desktop project conflict?",
      currentConversationId: "chat-a"
    )
    ...
    XCTAssertTrue(compiled.contains("- SignalASI supports Desktop [current]"))
    XCTAssertTrue(compiled.contains("Relevant topic/project nodes:"))
    XCTAssertTrue(compiled.contains("- [topic] SignalASI Desktop support project"))
  }
  ...
  func testGlobalMemoryContextModelsUseAndroidWireNames() throws {
    ...
    let entity = contextEntity("entity", label: "SignalASI", kind: .application, aliases: ["app"])
    ...
    XCTAssertEqual(restoredTopic.nodes.first?.entityKeys, Set(["entity:signalasi"]))
