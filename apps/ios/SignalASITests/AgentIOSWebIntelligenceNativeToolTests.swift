  ...
  func testAgentIOSURLSessionWebIntelligenceProviderBridgesWebMediaEvidence() throws {
    final class FakeWebMediaProvider: AgentIOSWebMediaToolProviding {
      ...
      func invoke(
        ...
        case .webSearch:
          return AgentNativeToolExecutionResult.success(
            output: [
              "results": .array([
                .object([
                  "title": .string("SignalASI Docs"),
                  "url": .string("https://docs.signalasi.example")
                ]),
                .object([
                  "title": .string("SignalASI Blog"),
                  "url": .string("https://blog.signalasi.example")
                ])
              ]),
              "result_count": .int(2)
            ],
        ...
        case .webOpen:
          let url = input["url"]?.stringValue ?? "https://signalasi.example/page"
          return webOutput(
            url: url,
        ...
        case .webFetch:
          let url = input["url"]?.stringValue ?? "https://signalasi.example/root"
    ...
    let search = registry.invoke(
      AgentIOSWebIntelligenceNativeToolCatalog.search,
      input: ["query": .string("SignalASI"), "limit": .int(2)],
      context: networkContext,
    ...
    let extract = registry.invoke(
      ...
      input: [
        "source_url": .string("https://supplied.signalasi.example"),
    ...
    let crawl = registry.invoke(
      ...
      input: [
        "url": .string("https://signalasi.example/root"),
        "max_pages": .int(2)
      ],
      context: networkContext,
      hooks: AgentNativeToolInvocationHooks(nowMillis: { 1_000 })
    ...
    XCTAssertEqual(search.metadata["implementation"], .string("signalasi.ios.urlsession_web_intelligence"))
    ...
    XCTAssertEqual(evidence.first?.objectValue?["url"], .string("https://docs.signalasi.example"))
