...
final class GlobalRealtimeContextTests: XCTestCase {
  ...
  func testSelectionIsConversationScopedAndQueryRelevant() {
    let now = 20 * hour
    let items = [
      item("same", "Android runtime", "SignalASI", ["conversation-a"], now),
      item("related", "Android package verification", "SignalASI", ["conversation-b"], now - 1_000),
  ...
  func testGlobalStatusQueryCanSurfaceActiveWorkFromOtherConversations() {
    let now = 30 * hour
    let items = [
      item("one", "Android build", "SignalASI", ["conversation-a"], now),
  ...
  func testCurrentWorkCanBeExcludedFromRelatedRealtimeContext() {
    let now = 40 * hour
    let items = [
      item("run:current", "Current run", "SignalASI", ["conversation-a"], now),
      item("run:other", "Related run", "SignalASI", ["conversation-a"], now - 1_000)
    ]
  ...
  func testRenderExposesBoundedHostStateWithoutInternalKeysOrSecrets() {
    ...
    XCTAssertFalse(rendered.contains("com.signalasi.chat"))
