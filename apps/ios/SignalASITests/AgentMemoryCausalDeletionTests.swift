...
final class AgentMemoryCausalDeletionTests: XCTestCase {
  ...
  func testStoreBackupRestoresDeletionIndexBeforeMemory() throws {
    ...
    let source = SignalASIStore(defaults: sourceDefaults, secrets: InMemorySecretStore())
    ...
    let restored = SignalASIStore(defaults: restoredDefaults, secrets: InMemorySecretStore())
