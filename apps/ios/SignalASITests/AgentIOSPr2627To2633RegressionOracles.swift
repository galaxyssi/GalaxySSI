...
enum AgentIOSPr2627To2633RegressionOracles {
  ...
  private static func verifyPairingIdentity(_ testCase: AgentIOSPr2627To2633Case) {
    ...
    let first = SignalASILinkProtocol.pairingTopic(secret: secret)
    XCTAssertFalse(first.isEmpty, testCase.id)
    XCTAssertEqual(first, SignalASILinkProtocol.pairingTopic(secret: secret))
    XCTAssertNotEqual(first, SignalASILinkProtocol.pairingTopic(secret: otherSecret))
  }
