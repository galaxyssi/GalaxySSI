...
extension SignalASIStoreTests {
  ...
  func testGlobalMemoryNamespacePolicyMatchesAndroidClassification() {
    let explicit = GlobalMemoryNamespacePolicy.resolve(
      event: globalMemoryEvent(
        ...
        metadata: ["memory_namespace": "PROJECT:SignalASI", "memory_namespace_id": "Workspace 1"]
      ),
    ...
    let project = GlobalMemoryNamespacePolicy.resolve(
      ...
      understanding: GlobalUnderstanding(topic: "iOS parity", project: "SignalASI iOS"),
      kind: .task,
      layer: .topic
    )
    ...
    XCTAssertEqual(project.key, "project:signalasi-ios")
  ...
  private func globalMemoryEvent(
    ...
    GlobalConversationEvent(
      ...
      content: content,
      conversationTitle: "Project SignalASI",
      metadata: metadata
    )
  }
}
