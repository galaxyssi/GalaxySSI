#include "SignalASIWhisperRuntime.h"
...
extern "C" int64_t signalasi_whisper_create_runtime(
...
extern "C" int64_t signalasi_whisper_create_session(
...
extern "C" char *signalasi_whisper_decode_json(
...
extern "C" char *signalasi_whisper_timings_json(int64_t session_handle) {
...
extern "C" void signalasi_whisper_request_abort(int64_t session_handle) {
    const auto session = find_session(session_handle);
    if (session != nullptr) session->abort_requested.store(true, std::memory_order_relaxed);
}

extern "C" void signalasi_whisper_destroy_session(int64_t session_handle) {
...
extern "C" void signalasi_whisper_destroy_runtime(int64_t runtime_handle) {
...
extern "C" int32_t signalasi_whisper_active_runtime_count(void) {
    std::lock_guard<std::mutex> lock(registry_mutex);
    return static_cast<int32_t>(runtimes.size());
}

extern "C" int32_t signalasi_whisper_active_session_count(void) {
    std::lock_guard<std::mutex> lock(registry_mutex);
    return static_cast<int32_t>(sessions.size());
}

extern "C" void signalasi_whisper_free_string(char *value) {
    std::free(value);
}
