...
Each `execute` call uses the same default limits as Android: 60 seconds wall clock, 45 seconds CPU, 512 MiB address space, 512 MiB workspace storage, 64 processes, 512 KiB returned output, and 256 MiB requested artifacts. The optional `resource_limits` request object supports Android's wire names and validation ranges. The iOS loopback protocol caps returned output at 512 KiB so a signed response remains below its 1 MiB frame limit. The requested timeout may reduce or extend the wall-clock and CPU budget up to 30 minutes, preserving Android's three-quarter CPU-to-wall-clock ratio. The device Python must expose POSIX `RLIMIT_CPU`, `RLIMIT_AS`, `RLIMIT_FSIZE`, and `RLIMIT_NPROC`; otherwise the broker reports `backend_ready: false` and rejects execution rather than pretending isolation is active. Workspace and requested artifact sizes are verified before and after each execution.
