...
struct SignalASIPrivacyControlCenterView: View {
  ...
  private var metricsStrip: some View {
    HStack(spacing: 8) {
      SignalASIPrivacyMetric(value: "\(summary.total)", label: t("cc_privacy_metric_events", "Recent flows"), tint: .blue)
      SignalASIPrivacyMetric(value: "\(summary.destinations)", label: t("cc_privacy_metric_destinations", "Destinations"), tint: .signalASIInsightText)
      SignalASIPrivacyMetric(value: "\(summary.cloud)", label: t("cc_privacy_metric_cloud", "Cloud flows"), tint: summary.cloud > 0 ? .orange : .signalASIAccent)
    }
  }
  ...
  private func statusTint(_ status: AgentDisclosureStatus) -> Color {
    switch status {
    case .sent: return .signalASIAccent
...
private struct SignalASIPrivacyMetric: View {
  var value: String
  var label: String
  var tint: Color
...
private struct SignalASIPrivacyDestinationSummary: Identifiable {
  var id: String
  var title: String
  ...
  func locationLabel(language: String) -> String {
    switch location {
    case .phone:
      return SignalASILocalization.string("cc_privacy_location_phone", fallback: "This iPhone", language: language)
    case .trustedDesktop:
      return SignalASILocalization.string("cc_privacy_location_desktop", fallback: "Trusted Desktop", language: language)
    case .privateNetwork:
      return SignalASILocalization.string("cc_privacy_location_private", fallback: "Private network", language: language)
    case .cloud:
      return SignalASILocalization.string("cc_privacy_location_cloud", fallback: "Cloud", language: language)
    }
  }

  func modelLabel(language: String) -> String {
    modelId.ifBlank(providerId).ifBlank(
      SignalASILocalization.string("cc_privacy_destination_unspecified", fallback: "Not specified", language: language)
    )
  }
