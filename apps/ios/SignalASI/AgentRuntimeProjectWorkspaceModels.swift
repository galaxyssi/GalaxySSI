...
final class AgentRuntimeProjectWorkspaceManager {
  ...
  init(
    ...
    self.checkpointRoot = checkpointRoot ?? projectRoot.appendingPathComponent(".signalasi-checkpoints", isDirectory: true)
  ...
  private func isPrivateRuntimePath(_ path: String) -> Bool {
    ...
    return components.contains { $0.hasPrefix(".signalasi-") } ||
      components.contains("request.json") ||
      components.first == ".signalasi"
  }
