...
enum SignalASIMemoryLifecycleState: String, CaseIterable, Identifiable {
  case current
  case planned
  case historical
  case deprecated
...
struct SignalASIMemoryCategoryDescriptor: Identifiable {
  var id: String
  var titleKey: String
  ...
  var tone: SignalASIMemoryTone
  var kinds: Set<AgentMemoryKind>
}

enum SignalASIMemoryTone {
  case accent
  case blue
  case green
  case purple
...
struct SignalASIMemoryControlSnapshot {
  ...
  ) -> SignalASIMemoryControlSnapshot {
    SignalASIMemoryControlSnapshot(
  ...
  var graph: SignalASIMemoryGraphSnapshot {
    SignalASIMemoryGraphSnapshot.make(
      items: allAgentItems,
  ...
  func items(for state: SignalASIMemoryLifecycleState) -> [AgentMemoryItem] {
    switch state {
    case .current:
  ...
  func candidates(for state: SignalASIMemoryLifecycleState) -> [GlobalMemoryCandidate] {
    switch state {
    case .planned:
...
struct SignalASIMemoryGraphSnapshot {
  var nodes: [SignalASIMemoryGraphNode]
  ...
  static func make(
  ...
  ) -> SignalASIMemoryGraphSnapshot {
    var nodes: [SignalASIMemoryGraphNode] = items.prefix(80).map { item in
      SignalASIMemoryGraphNode(
        id: item.id,
    ...
    return SignalASIMemoryGraphSnapshot(nodes: nodes, relations: relations)
  }
}

struct SignalASIMemoryGraphNode: Identifiable {
  var id: String
  var title: String
  var subtitle: String
  var badge: String
...
struct SignalASIMemoryGraphRelation: Identifiable {
  var id: String
  var title: String
  var subtitle: String
  var tone: SignalASIMemoryTone
