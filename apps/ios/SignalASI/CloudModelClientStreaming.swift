...
extension CloudModelClient {
  ...
  private static func validStreamingEndpoint(_ endpoint: String) throws -> String {
    ...
    guard lower.hasPrefix("https://") ||
      ...
      throw SignalASIError.invalidPayload("Cloud streaming endpoint must use HTTPS or local HTTP.")
    }
    return trimmed
  }

  private static func bodyJSON(_ object: [String: Any]) throws -> String {
    let data = try SignalASILinkProtocol.jsonData(object)
    guard let body = String(data: data, encoding: .utf8) else {
      throw SignalASIError.invalidPayload("Cloud request body is not valid UTF-8.")
    }
    return body
  }
