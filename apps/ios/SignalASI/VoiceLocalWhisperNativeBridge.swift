...
final class SignalASIWhisperNativeBridge: VoiceWhisperNativeAPI {
  func createRuntime(modelPath: String, threadCount: Int, useGPU: Bool) -> Int64 {
    #if SIGNALASI_NATIVE_WHISPER
    guard !useGPU else { return 0 }
    return modelPath.withCString {
      signalasi_whisper_create_runtime($0, Int32(threadCount), 0)
    }
    #else
    return 0
    #endif
  ...
  func createSession(runtimeHandle: Int64, config: VoiceLocalWhisperSessionConfig) -> Int64 {
    ...
    return config.language.withCString { language in
      config.prompt.withCString { prompt in
        signalasi_whisper_create_session(
          runtimeHandle,
          language,
  ...
  func decodePcm16(
    ...
    return pcm.withUnsafeBufferPointer { buffer in
      ...
      guard let pointer = signalasi_whisper_decode_json(
        sessionHandle,
      ...
      defer { signalasi_whisper_free_string(pointer) }
  ...
  func requestAbort(sessionHandle: Int64) {
    #if SIGNALASI_NATIVE_WHISPER
    signalasi_whisper_request_abort(sessionHandle)
    #endif
  }

  func getTimings(sessionHandle: Int64) -> VoiceNativeWhisperTimings {
    #if SIGNALASI_NATIVE_WHISPER
    guard let pointer = signalasi_whisper_timings_json(sessionHandle) else { return .empty }
    defer { signalasi_whisper_free_string(pointer) }
  ...
  func destroySession(sessionHandle: Int64) {
    #if SIGNALASI_NATIVE_WHISPER
    signalasi_whisper_destroy_session(sessionHandle)
    #endif
  }

  func destroyRuntime(runtimeHandle: Int64) {
    #if SIGNALASI_NATIVE_WHISPER
    signalasi_whisper_destroy_runtime(runtimeHandle)
    #endif
  }

  func activeRuntimeCount() -> Int {
    #if SIGNALASI_NATIVE_WHISPER
    return Int(signalasi_whisper_active_runtime_count())
    #else
    return 0
    #endif
  }
  ...
  func activeSessionCount() -> Int {
    #if SIGNALASI_NATIVE_WHISPER
    return Int(signalasi_whisper_active_session_count())
    #else
    return 0
    #endif
  }
...
#if SIGNALASI_NATIVE_WHISPER
@_silgen_name("signalasi_whisper_create_runtime")
private func signalasi_whisper_create_runtime(
  _ modelPath: UnsafePointer<CChar>,
  _ threadCount: Int32,
  _ useGPU: Int32
) -> Int64

@_silgen_name("signalasi_whisper_create_session")
private func signalasi_whisper_create_session(
...
@_silgen_name("signalasi_whisper_decode_json")
private func signalasi_whisper_decode_json(
  _ sessionHandle: Int64,
  _ pcm: UnsafePointer<Int16>,
  _ length: Int32
) -> UnsafeMutablePointer<CChar>?

@_silgen_name("signalasi_whisper_timings_json")
private func signalasi_whisper_timings_json(_ sessionHandle: Int64) -> UnsafeMutablePointer<CChar>?

@_silgen_name("signalasi_whisper_request_abort")
private func signalasi_whisper_request_abort(_ sessionHandle: Int64)

@_silgen_name("signalasi_whisper_destroy_session")
private func signalasi_whisper_destroy_session(_ sessionHandle: Int64)

@_silgen_name("signalasi_whisper_destroy_runtime")
private func signalasi_whisper_destroy_runtime(_ runtimeHandle: Int64)

@_silgen_name("signalasi_whisper_active_runtime_count")
private func signalasi_whisper_active_runtime_count() -> Int32

@_silgen_name("signalasi_whisper_active_session_count")
private func signalasi_whisper_active_session_count() -> Int32

@_silgen_name("signalasi_whisper_free_string")
private func signalasi_whisper_free_string(_ value: UnsafeMutablePointer<CChar>?)
#endif
