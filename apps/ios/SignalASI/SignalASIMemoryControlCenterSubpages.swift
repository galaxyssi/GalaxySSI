...
struct SignalASIMemoryCategoryView: View {
  @Environment(\.signalASIInterfaceLanguage) private var interfaceLanguage
  @EnvironmentObject private var store: SignalASIStore
  var category: SignalASIMemoryCategoryDescriptor

  var body: some View {
    SignalASIMemoryListPage(
  ...
  private func memoryRow(_ item: AgentMemoryItem) -> some View {
    SignalASISecurityStatusRow(
      ...
      badge: item.important ? t("signalasi.agent_memory.pinned", "Pinned") : memoryKindLabel(item.kind)
    )
  }
  ...
  private func memorySubtitle(_ item: AgentMemoryItem) -> String {
    String(
      ...
      item.key.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
        ? t("signalasi.agent_memory.key_none", "Unkeyed")
        : item.key
    )
  }
  ...
  private func memoryKindLabel(_ kind: AgentMemoryKind) -> String {
    SignalASIMemoryText.kindLabel(kind, language: interfaceLanguage)
  }
}
...
struct SignalASIMemoryLifecycleView: View {
  ...
  var state: SignalASIMemoryLifecycleState
  ...
  private var stateSubtitle: String {
    switch state {
    case .current:
      return t("cc_memory_state_current_subtitle", "Facts and decisions SignalASI currently treats as true")
    case .planned:
  ...
  private var stateTone: SignalASIMemoryTone {
    switch state {
  ...
  private func memorySubtitle(_ item: AgentMemoryItem) -> String {
    let label = item.key.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
      ? t("signalasi.agent_memory.key_none", "Unkeyed")
      : item.key
    return String(format: t("signalasi.agent_memory.item_subtitle", "%@ / v%d / %@"), memoryKindLabel(item.kind), item.version, label)
  }
  ...
  private func candidateTitle(_ candidate: GlobalMemoryCandidate) -> String {
    candidate.item.topic.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
      ? SignalASIMemoryText.candidateKindLabel(candidate.kind, language: interfaceLanguage)
  ...
  private func candidateSubtitle(_ candidate: GlobalMemoryCandidate) -> String {
    String(
      ...
      SignalASIMemoryText.evolutionActionLabel(candidate.action, language: interfaceLanguage),
      candidate.item.evidenceCount
    )
  }

  private func candidateRiskLabel(_ risk: GlobalMemoryCandidateRisk) -> String {
    SignalASIMemoryText.riskLabel(risk, language: interfaceLanguage)
  }
...
struct SignalASIMemoryInboxView: View {
  ...
  var body: some View {
    SignalASIMemoryListPage(
      ...
      heroIcon: "tray.full",
      heroTint: pending.isEmpty ? .signalASIAccent : .orange,
      ...
      if pending.isEmpty {
        ...
        ForEach(pending) { candidate in
          SignalASIMemoryCandidateCard(
            candidate: candidate,
            language: interfaceLanguage,
            approveTitle: t("cc_memory_candidate_approve", "Approve"),
            rejectTitle: t("signalasi.common.reject", "Reject"),
        ...
        ForEach(records.reversed()) { record in
          SignalASISecurityStatusRow(
            ...
            subtitle: String(
              ...
              SignalASIMemoryText.evolutionActionLabel(record.action, language: interfaceLanguage),
              SignalASIMemoryText.evolutionOutcomeLabel(record.outcome, language: interfaceLanguage),
              record.evidenceCount
            ),
            systemImage: "arrow.triangle.branch",
            tint: SignalASIMemoryText.outcomeTint(record.outcome),
            badge: SignalASIMemoryText.temporalStateLabel(record.temporalState, language: interfaceLanguage)
          )
        }
      }
    }
...
struct SignalASIMemoryGraphView: View {
  ...
  private var graph: SignalASIMemoryGraphSnapshot {
    SignalASIMemoryControlSnapshot.make(
      agentMemory: store.agentMemorySnapshot(),
      knowledgeStats: store.agentKnowledgeStats,
      inbox: inbox,
...
struct SignalASIMemoryAuditView: View {
  ...
  var body: some View {
    SignalASIMemoryListPage(
      ...
      heroTint: report.findings.isEmpty ? .signalASIAccent : .orange,
...
private struct SignalASIMemoryCandidateCard: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 10) {
      HStack(spacing: 12) {
        ...
        Spacer(minLength: 8)
        Text(SignalASIMemoryText.riskLabel(candidate.risk, language: language))
  ...
  private var title: String {
    candidate.item.topic.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
      ? SignalASIMemoryText.candidateKindLabel(candidate.kind, language: language)
      : candidate.item.topic
  }
  ...
  private var subtitle: String {
    ...
    return [
      SignalASIMemoryText.evolutionActionLabel(candidate.action, language: language),
      SignalASIMemoryText.temporalStateLabel(candidate.temporalState, language: language),
      compact(detail)
...
struct SignalASIMemoryListPage<Content: View>: View {
  var title: String
...
private enum SignalASIMemoryText {
  static func kindLabel(_ kind: AgentMemoryKind, language: String) -> String {
    switch kind {
    case .identity:
      return t("signalasi.agent_memory.kind_identity", "Identity", language)
    case .contact:
    ...
    case .safety:
      return t("signalasi.agent_memory.kind_safety", "Safety", language)
    }
  }
