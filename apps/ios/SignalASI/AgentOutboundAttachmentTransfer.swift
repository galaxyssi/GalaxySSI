...
struct AgentAttachmentTransferScope: Codable, Equatable {
  ...
  init(
    ...
    guard SignalASILinkProtocol.validRouteId(clientRouteId) else {
  ...
  private func transportPayload(
    ...
    if attachment.isImage,
      ...
      return (
        encoded.data,
        encoded.mimeType,
        SignalASIAttachmentPayloadBuilder.sanitizeName(encoded.displayName)
      )
    }
    return (
      attachment.data,
      attachment.mimeType.ifBlank("application/octet-stream"),
      SignalASIAttachmentPayloadBuilder.sanitizeName(attachment.displayName)
    )
  }
