...
struct AgentPhoneDevelopmentManifest: Equatable {
  static let schema = "signalasi.phone-development-manifest.v2"
  ...
  func runtimeInput() -> AgentMcpJSONObject {
    ...
    _signalasi_files = json.loads(base64.b64decode(\"\(Data(AgentMcpJSONCodec.stringify(.array(encodedFiles)).utf8).base64EncodedString())\").decode(\"utf-8\"))
    for _signalasi_file in _signalasi_files:
        _signalasi_path = pathlib.Path(_signalasi_file[\"path\"])
        _signalasi_path.parent.mkdir(parents=True, exist_ok=True)
        _signalasi_path.write_text(base64.b64decode(_signalasi_file[\"content_b64\"]).decode(\"utf-8\"), encoding=\"utf-8\")
    os.chdir(os.getcwd())
