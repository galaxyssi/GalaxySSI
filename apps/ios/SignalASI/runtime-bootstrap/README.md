...
The offline bundle also includes the same verified ARM64 Node.js 24.18.0 SquashFS runtime pack used by Android. QEMU attaches it as a read-only virtio disk, the guest validates its descriptor and capabilities, and `/opt/signalasi/packs/node-js/bin` precedes the Debian system path for Agent commands.
