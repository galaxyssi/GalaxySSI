...
enum AgentModelPlanningPrompt {
  ...
  private static func appendRuntimeRules(
    ...
    if allowsPhoneRuntimeTools(for: request) {
      ...
      append(&prompt, "For an existing phone project, use signalasi.project.repository.inspect, signalasi.project.repository.diff, and signalasi.project.repository.log for Git reads instead of running Git through signalasi.runtime.execute. ")
