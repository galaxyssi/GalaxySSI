...
enum AgentModelPlanningPrompt {
  ...
  private static func appendRuntimeRules(
    ...
    if allowsPhoneRuntimeTools(for: request) {
      ...
      append(&prompt, "Use signalasi.project.repository.clone, signalasi.project.repository.fetch, signalasi.project.repository.branch.checkout, and signalasi.project.repository.pull for phone-project Git setup and synchronization. For reads, use signalasi.project.repository.inspect, signalasi.project.repository.diff, and signalasi.project.repository.log instead of running Git through signalasi.runtime.execute. ")
      ...
      append(&prompt, "Do not use signalasi.runtime.* or signalasi.workspace.* tools for this goal. ")
