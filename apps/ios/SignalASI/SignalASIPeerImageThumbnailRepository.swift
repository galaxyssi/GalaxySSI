...
enum SignalASIPeerImageThumbnailPolicy {
...
final class SignalASIPeerImageThumbnailRepository {
  static let shared = SignalASIPeerImageThumbnailRepository()
  ...
  private let queue = DispatchQueue(
    label: "com.signalasi.ios.peer-image-thumbnail",
    qos: .userInitiated,
  ...
  private func sourceData(for block: AgentRichBlock) -> Data? {
    if let inline = SignalASIImageResourceDecoder.base64Data(block.dataB64) {
      return inline
    }
  ...
  private func cacheKey(block: AgentRichBlock, maxPixelSize: Int) -> String {
    "\(SignalASIPeerImageThumbnailPolicy.cacheIdentity(block))\u{0000}\(maxPixelSize)"
  }
...
struct SignalASIPeerCachedImageView<Placeholder: View>: View {
  ...
  var body: some View {
    Group {
      if let thumbnailData {
        SignalASIAnimatedImageView(data: thumbnailData)
      } else {
        placeholder()
      }
    }
    .onAppear(perform: load)
    .onChange(of: SignalASIPeerImageThumbnailPolicy.cacheIdentity(block)) { _ in
      thumbnailData = nil
      load()
    }
  }
  ...
  private func load() {
    SignalASIPeerImageThumbnailRepository.shared.load(block: block) { data in
      thumbnailData = data
