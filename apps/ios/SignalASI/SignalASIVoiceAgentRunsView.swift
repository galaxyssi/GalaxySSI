...
struct SignalASIVoiceAgentRunsView: View {
  ...
  var body: some View {
    NavigationView {
      VStack(spacing: 0) {
        SignalASITopBar(
          title: t("signalasi.voice_agent_runs.title", "Voice Agent runs"),
          ...
          trailing: {
            ...
            .accessibilityLabel(t("signalasi.voice_agent_runs.refresh", "Refresh runs"))
          }
        )
        ...
        ScrollView {
          VStack(alignment: .leading, spacing: 10) {
            Text(
              String(
                format: t(
                  "signalasi.voice_agent_runs.summary",
                  "%d active / %d saved"
                ),
            ...
            if runs.isEmpty {
              VoiceAgentRunEmptyView(
                title: t("signalasi.voice_agent_runs.empty", "No voice Agent runs"),
                subtitle: t(
                  "signalasi.voice_agent_runs.empty_subtitle",
  ...
  private func stateTint(_ state: VoiceAgentRunState) -> Color {
    if state.isTerminal {
      return state == .completed ? .signalASIAccent : .orange
    }
      ...
      HStack(alignment: .top, spacing: 10) {
        ...
        VStack(alignment: .leading, spacing: 3) {
          Text(run.goal.ifBlank("Voice Agent task"))
            ...
            .foregroundColor(.signalASITextPrimary)
            .lineLimit(2)
          Text(run.agentName.ifBlank(run.agentId).ifBlank("SignalASI Agent"))
            .font(.caption)
            .foregroundColor(.signalASITextSecondary)
            .lineLimit(1)
        }
    ...
    NavigationView {
      ScrollView {
        VStack(alignment: .leading, spacing: 14) {
          detail(t("signalasi.voice_agent_runs.detail.state", "State"), stateLabel)
          detail(t("signalasi.voice_agent_runs.detail.agent", "Agent"), run.agentName.ifBlank(run.agentId).ifBlank("-"))
          detail(t("signalasi.voice_agent_runs.detail.goal", "Goal"), run.goal.ifBlank("-"))
          detail(t("signalasi.voice_agent_runs.detail.stage", "Stage"), run.stage.ifBlank("-"))
          detail(t("signalasi.voice_agent_runs.detail.progress", "Progress"), run.progressMessage.ifBlank("-"))
          detail(t("signalasi.voice_agent_runs.detail.partial", "Partial result"), run.partialResult.ifBlank("-"))
          detail(t("signalasi.voice_agent_runs.detail.result", "Result"), run.resultSummary.ifBlank("-"))
          detail(t("signalasi.voice_agent_runs.detail.created", "Created"), createdLabel)
          detail(t("signalasi.voice_agent_runs.detail.updated", "Updated"), updatedLabel)
          detail(t("signalasi.voice_agent_runs.detail.run_id", "Run ID"), run.runId)
        }
        .padding(16)
      }
      .background(Color.signalASIPageBackground.ignoresSafeArea())
      .navigationTitle(t("signalasi.voice_agent_runs.detail.title", "Voice Agent run"))
      .toolbar {
