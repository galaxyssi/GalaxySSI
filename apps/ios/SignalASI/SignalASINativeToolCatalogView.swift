...
struct SignalASINativeToolCatalogView: View {
      ...
      ScrollView {
        VStack(alignment: .leading, spacing: 12) {
          NativeToolCatalogHeroView(
            title: t("signalasi.native_tool_catalog.hero_title", "Phone native tool catalog"),
            subtitle: t(
            ...
            metrics: [
              NativeToolMetric(
                value: "\(tools.count)",
                label: t("signalasi.native_tool_catalog.metric_tools", "Native tools")
              ),
              NativeToolMetric(
                value: "\(availableCount)",
                label: t("signalasi.native_tool_catalog.metric_available", "Available")
              ),
              NativeToolMetric(
                value: "\(highRiskCount)",
                label: t("signalasi.native_tool_catalog.metric_high_risk", "High risk")
              )
            ]
          )
          ...
          ForEach(groupedTools) { group in
            ...
            VStack(spacing: 8) {
              ForEach(group.tools) { tool in
                NavigationLink(destination: SignalASINativeToolDetailView(tool: tool)) {
  ...
  private func locationLabel(_ location: AgentNativeToolLocation) -> String {
    switch location {
    case .phone: return t("signalasi.native_tool_catalog.location_phone", "Phone")
    case .desktop: return t("signalasi.native_tool_catalog.location_desktop", "Desktop")
    case .application: return t("signalasi.native_tool_catalog.location_application", "Application")
    case .androidSystem: return t("signalasi.native_tool_catalog.location_ios_system", "iOS System")
    case .accessibilityService: return t("signalasi.native_tool_catalog.location_accessibility", "Accessibility")
    case .unknown: return t("signalasi.native_tool_catalog.location_other", "Other")
    }
  }

  private func riskLabel(_ risk: AgentNativeToolRisk) -> String {
    switch risk {
    case .low: return t("signalasi.native_tool_catalog.risk_low", "Low")
    case .medium: return t("signalasi.native_tool_catalog.risk_medium", "Medium")
    case .high: return t("signalasi.native_tool_catalog.risk_high", "High")
    case .blocked: return t("signalasi.native_tool_catalog.risk_blocked", "Blocked")
    }
  }
...
struct SignalASINativeToolDetailView: View {
  @Environment(\.signalASIInterfaceLanguage) private var interfaceLanguage
    ...
    VStack(spacing: 0) {
      ...
      ScrollView {
        VStack(alignment: .leading, spacing: 12) {
          ...
          sectionTitle(t("signalasi.native_tool_catalog.section_details", "Details"))
          NativeToolInfoRow(
            title: t("signalasi.native_tool_catalog.tool_id", "Tool ID"),
            value: tool.id,
            systemImage: "link",
          ...
          NativeToolInfoRow(
            title: t("signalasi.native_tool_catalog.run_scope", "Run scope"),
          ...
          NativeToolInfoRow(
            title: t("signalasi.native_tool_catalog.permissions", "Permissions"),
            value: String(
              format: t("signalasi.native_tool_catalog.permission_count", "%d permissions / %d consents"),
          ...
          NativeToolInfoRow(
            title: t("signalasi.native_tool_catalog.status", "Status"),
            value: tool.availability.reason.ifBlank(t("signalasi.native_tool_catalog.status_ready", "Ready for registered execution")),
            systemImage: "info.circle",
            tint: statusTint(availabilityStatus, risk: tool.risk),
            badge: statusLabel(availabilityStatus)
          )
          NativeToolInfoRow(
            title: t("signalasi.native_tool_catalog.execution_budget", "Execution budget"),
            value: String(
              format: t("signalasi.native_tool_catalog.timeout_value", "%@ timeout / %@"),
          ...
          sectionTitle(t("signalasi.native_tool_catalog.section_capabilities", "Capabilities"))
          if tool.capabilities.isEmpty {
            NativeToolInfoRow(
              title: t("signalasi.native_tool_catalog.no_capabilities", "No declared capabilities"),
              value: t("signalasi.native_tool_catalog.no_capabilities_subtitle", "This tool is exposed by registry scope rather than a phone capability boundary."),
          ...
          sectionTitle(t("signalasi.native_tool_catalog.section_requirements", "Requirements"))
          requirementRows(
            permissions: tool.requiredPermissions,
            consents: tool.requiredConsents
          )

          sectionTitle(t("signalasi.native_tool_catalog.section_schema", "Schemas"))
          NativeToolInfoRow(
            title: t("signalasi.native_tool_catalog.input_schema", "Input schema"),
          ...
          NativeToolInfoRow(
            title: t("signalasi.native_tool_catalog.output_schema", "Output schema"),
  ...
  private func requirementRows(
    ...
    if visiblePermissions.isEmpty && visibleConsents.isEmpty {
      NativeToolInfoRow(
        title: t("signalasi.native_tool_catalog.no_requirements", "No extra requirements"),
        value: t("signalasi.native_tool_catalog.no_requirements_subtitle", "This tool can run inside the existing SignalASI policy boundary."),
      ...
      VStack(spacing: 8) {
        ForEach(visiblePermissions) { permission in
          NativeToolInfoRow(
            ...
            systemImage: "lock",
            tint: .orange,
            badge: t("signalasi.native_tool_catalog.permission", "Permission")
          )
        }
        ForEach(visibleConsents) { consent in
          NativeToolInfoRow(
            ...
            tint: .red,
            badge: t("signalasi.native_tool_catalog.consent", "Consent")
          )
        }
      }
    }
  ...
  private func schemaSummary(_ fields: [String]) -> String {
    guard !fields.isEmpty else {
      return t("signalasi.native_tool_catalog.no_schema_fields", "No named fields")
    }
    ...
    return String(
      format: t("signalasi.native_tool_catalog.more_fields", "%@ / +%d more"),
      visible,
      fields.count - 10
    )
  }
  ...
  private func duration(_ millis: Int64) -> String {
    ...
    if millis % 1_000 == 0 {
      return String(format: t("signalasi.native_tool_catalog.seconds_value", "%@s"), "\(millis / 1_000)")
    }
    return String(format: t("signalasi.native_tool_catalog.millis_value", "%@ms"), "\(millis)")
  }
  ...
  private func idempotencyLabel(_ value: AgentNativeToolIdempotency) -> String {
    switch value {
    case .nonIdempotent: return t("signalasi.native_tool_catalog.idempotency_non", "non-idempotent")
    case .idempotent: return t("signalasi.native_tool_catalog.idempotency_yes", "idempotent")
    case .idempotencyKeyRequired: return t("signalasi.native_tool_catalog.idempotency_key", "idempotency key required")
    }
  }
}
