...
struct SignalASIAgentScreenUnderstandingView: View {
  @Environment(\.signalASIInterfaceLanguage) private var interfaceLanguage
  @EnvironmentObject private var store: SignalASIStore
  @State private var refreshToken = 0

  private var snapshot: SignalASIAgentScreenContextSnapshot {
    _ = refreshToken
    ...
    return SignalASIAgentScreenContextSnapshotBuilder.make(
      messages: [],
      draft: "",
      attachments: [],
      unreadTotal: 0,
  ...
  var body: some View {
    SignalASIAgentScreenContextDetailView(
