...
struct SignalASISelfEvolutionControlView: View {
  ...
  var body: some View {
    ...
    .sheet(isPresented: $isShowingCreateTask) {
      SignalASISelfEvolutionCreateTaskSheet(taskStore: taskStore) { task in
  ...
  private var hero: some View {
    VStack(alignment: .leading, spacing: 12) {
      ...
      HStack(spacing: 8) {
        SignalASISelfEvolutionMetricCard(
        ...
        SignalASISelfEvolutionMetricCard(
          ...
          tint: health.attentionTasks > 0 ? .orange : .signalASITextSecondary
        )
      }
    }
  }
  ...
  private var pipelineSection: some View {
    VStack(alignment: .leading, spacing: 8) {
      SignalASISecuritySectionTitle(title: t("cc_evolution_section_pipeline", "Evolution pipeline"))
      SignalASISecurityActionRow(
      ...
      SignalASISecurityStatusRow(
        ...
        tint: desktopExecutors.isEmpty ? .signalASITextSecondary : .signalASIAccent,
      ...
      SignalASISecurityStatusRow(
        ...
        tint: health.attentionTasks == 0 ? .signalASIAccent : .orange,
  ...
  private var recentSection: some View {
    VStack(alignment: .leading, spacing: 8) {
      SignalASISecuritySectionTitle(title: t("cc_evolution_section_recent", "Recent candidates"))
      if tasks.isEmpty {
        SignalASISecurityActionRow(
          title: t("cc_evolution_empty_title", "No evolution tasks"),
          subtitle: t("cc_evolution_empty_subtitle", "Create a scoped task or ask the Agent to improve SignalASI"),
        ...
        ForEach(Array(tasks.prefix(8))) { task in
          NavigationLink(destination: SignalASISelfEvolutionTaskDetailView(taskId: task.taskId)) {
            SignalASISelfEvolutionPlainRow(
              title: task.problem,
              subtitle: String(
  ...
  private var desktopSection: some View {
    if !desktopExecutors.isEmpty {
      VStack(alignment: .leading, spacing: 8) {
        SignalASISecuritySectionTitle(title: t("cc_evolution_section_desktop", "Desktop candidates"))
  ...
  private func statusLabel(_ status: AgentIOSSelfEvolutionTaskStatus) -> String {
    SignalASISelfEvolutionText.statusLabel(status, language: interfaceLanguage)
  }

  private func statusTint(_ status: AgentIOSSelfEvolutionTaskStatus) -> Color {
    SignalASISelfEvolutionText.statusTint(status)
  }
...
struct SignalASISelfEvolutionTaskDetailView: View {
  @Environment(\.signalASIInterfaceLanguage) private var interfaceLanguage
  ...
  private func taskSections(_ task: AgentIOSSelfEvolutionTask) -> some View {
    VStack(alignment: .leading, spacing: 12) {
      ...
      VStack(alignment: .leading, spacing: 8) {
        SignalASISecuritySectionTitle(title: t("cc_evolution_quality_gates", "Quality gates"))
        if let latest = task.attempts.last, !latest.gates.isEmpty {
          ForEach(latest.gates) { gate in
            SignalASISecurityStatusRow(
              ...
              systemImage: "checkmark.shield",
              tint: gate.status == .passed ? .signalASIAccent : (gate.status == .failed ? .orange : .blue),
  ...
  private func actionRows(_ task: AgentIOSSelfEvolutionTask) -> some View {
    VStack(alignment: .leading, spacing: 8) {
      SignalASISecuritySectionTitle(title: t("cc_evolution_section_actions_ios", "Actions"))
      if [.proposed, .blocked].contains(task.status) {
        SignalASISecurityActionRow(
          ...
          systemImage: "play.circle",
          tint: runtimeAvailability.status == .available ? .signalASIAccent : .orange,
...
private struct SignalASISelfEvolutionCreateTaskSheet: View {
  @Environment(\.signalASIInterfaceLanguage) private var interfaceLanguage
...
private struct SignalASISelfEvolutionMetricCard: View {
  var value: String
  var title: String
...
private struct SignalASISelfEvolutionPlainRow: View {
  var title: String
  var subtitle: String
  var systemImage: String
  var tint: Color
...
enum SignalASISelfEvolutionText {
