...
struct SignalASIAgentAuditOperationsView: View {
  ...
  private var metricsSection: some View {
    HStack(spacing: 8) {
      ...
      auditMetric(
        ...
        tint: failedToolCount == 0 ? .signalASIAccent : .orange
      )
    }
  }
  ...
  private var nativeToolSection: some View {
    VStack(alignment: .leading, spacing: 8) {
      SignalASISecuritySectionTitle(title: t("cc_tool_audit_title", "Native tool audit"))
  ...
  private var taskSection: some View {
    VStack(alignment: .leading, spacing: 8) {
      SignalASISecuritySectionTitle(title: t("cc_tasks_title", "Recent Agent tasks"))
  ...
  private var executionEventSection: some View {
    VStack(alignment: .leading, spacing: 8) {
      SignalASISecuritySectionTitle(title: t("cc_audit_execution_title", "Agent execution events"))
  ...
  private var executionEvents: [SignalASIAuditExecutionEvent] {
    taskRecords.flatMap { task in
      task.executionLog.enumerated().map { index, detail in
        SignalASIAuditExecutionEvent(
  ...
  private func emptyRow(title: String, subtitle: String) -> some View {
    auditRow(
      title: title,
      subtitle: subtitle,
      status: t("signalasi.status.ready", "Ready"),
  ...
  private func dateText(_ millis: Int64) -> String {
    guard millis > 0 else { return t("signalasi.status.unknown", "Unknown") }
  ...
  private func toolTint(_ status: AgentNativeToolResultStatus) -> Color {
    status == .succeeded ? .signalASIAccent : .orange
  }
...
private struct SignalASIAuditExecutionEvent: Identifiable {
  var id: String
  var detail: String
