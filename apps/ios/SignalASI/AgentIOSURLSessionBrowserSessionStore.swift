...
struct AgentIOSURLSessionBrowserSessionScope: Equatable {
  ...
  init(context: AgentNativeToolInvocationContext) {
    ...
    ownerId = owner.isEmpty ? "signalasi.mobile_agent" : owner
...
final class AgentIOSURLSessionBrowserSessionStore {
  static let toolHandleContract = "signalasi.tool-handle/1.0"
  ...
  func create(
    ...
    let record = Record(
      browserId: browserId,
      ownerId: clean(scope.ownerId, maxLength: 240).nilIfEmpty ?? "signalasi.mobile_agent",
  ...
  private func resolvedLocked(
    ...
    let ownerId = clean(scope.ownerId, maxLength: 240).nilIfEmpty ?? "signalasi.mobile_agent"
