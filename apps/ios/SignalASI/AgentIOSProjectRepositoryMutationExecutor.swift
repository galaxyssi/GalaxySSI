  ...
  private func runtimeInput(
    ...
    if networkEnabled && !token.isEmpty {
      input[AgentIOSRuntimeBrokerInternalInput.secretEnvironment] = .object([
        "SIGNALASI_GITHUB_TOKEN": .string(token)
      ])
    }
    return input
  }
  ...
  private func commandFailure(
    ...
    default:
      if normalized.contains("authentication failed") || normalized.contains("could not read username") {
        ...
        fallback = "GitHub authentication failed. Update the GitHub credential in SignalASI and retry."
...
github_token="${SIGNALASI_GITHUB_TOKEN-}"
unset SIGNALASI_GITHUB_TOKEN
repository_url="$1"
base_branch="$2"
feature_branch="$3"
depth="$4"
...
metadata_root="${SIGNALASI_GIT_METADATA_ROOT:-/var/lib/signalasi/git}"
...
reset_workspace() {
  find . -mindepth 1 -maxdepth 1 ! -name "$control_dir" ! -name '.signalasi-tools' \
    ! -name '.signalasi-inputs' ! -name '.tmp' ! -name 'request.json' ! -name 'status.json' \
    ! -name '.signalasi-checkpoint.json' ! -name '.signalasi-stdout' ! -name '.signalasi-stderr' \
    ! -name '.signalasi-main' -exec rm -rf -- {} +
}
mkdir -p "$control_dir"
...
configure_excludes() {
  ...
  for pattern in '.signalasi-runtime/' '.signalasi-tools/' '.signalasi-inputs/' '.tmp/' \
    'request.json' 'status.json' '.signalasi-checkpoint.json' '.signalasi-stdout' \
    '.signalasi-stderr' '.signalasi-main'; do
...
if [ "$existing" = true ]; then
  SIGNALASI_GITHUB_TOKEN="$github_token" git -c credential.helper= fetch --depth "$depth" origin "$remote_ref"
  if [ -n "$(git status --porcelain --untracked-files=all -- . ':(exclude).signalasi-runtime')" ]; then
...
if [ -n "$ref" ]; then
  ...
  SIGNALASI_GITHUB_TOKEN="$github_token" git -c credential.helper= fetch --prune "$remote" "$refspec"
else
  SIGNALASI_GITHUB_TOKEN="$github_token" git -c credential.helper= fetch --prune "$remote"
fi
