...
enum AgentNoReplyReasonPolicy {
  ...
  static func display(for signal: AgentNoReplySignal) -> AgentNoReplyDisplay {
    display(for: classify(signal))
  }

  static func display(for reason: AgentNoReplyReason) -> AgentNoReplyDisplay {
  ...
  private static let displayText: [AgentNoReplyReason: (title: String, message: String)] = [
    .networkUnavailable: (
      "Network unavailable",
      "SignalASI could not reach the route because this phone is offline."
    ),
    .desktopOffline: (
      "Desktop offline",
    ...
    .unknown: (
      "No reply",
      "SignalASI did not receive a usable response for this turn."
    )
  ]
}
