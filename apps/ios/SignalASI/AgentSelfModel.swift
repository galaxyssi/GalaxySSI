...
struct AgentSelfModel: Codable, Equatable {
  static let identityId = "signalasi-mobile"
...
final class UserDefaultsAgentSelfModelStore {
  ...
  static let key = "signalasi_agent_self_model"
  ...
  func observe(task: AgentTaskRecord) -> AgentSelfModelMutation {
    ...
    if mutation.changed, let data = try? JSONEncoder().encode(mutation.after) {
      _ = SignalASIEncryptedUserDefaultsStore.write(data, defaults: defaults, key: Self.key, secrets: secrets)
    }
    return mutation
  }
  ...
  func clear() {
    lock.lock(); defer { lock.unlock() }
    SignalASIEncryptedUserDefaultsStore.destroy(defaults: defaults, key: Self.key, secrets: secrets)
  }

  private func loadLocked() -> AgentSelfModel {
    guard let data = SignalASIEncryptedUserDefaultsStore.load(defaults: defaults, key: Self.key, secrets: secrets),
...
struct SignalASIAgentSelfModelView: View {
  ...
  private var summarySection: some View {
    VStack(alignment: .leading, spacing: 8) {
      SignalASISecuritySectionTitle(title: t("self_model_summary", "Summary"))
      ...
      SignalASISecurityStatusRow(
        ...
        tint: model.successfulRuns >= model.failedRuns ? .signalASIInsightText : .orange,
  ...
  private func percent(_ value: Double) -> String {
    ...
    formatter.locale = SignalASILocalization.interfaceLocale(language: interfaceLanguage)
