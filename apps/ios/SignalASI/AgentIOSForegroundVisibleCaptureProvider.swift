...
struct AgentIOSForegroundVisibleCaptureProvider: AgentIOSVisibleCaptureToolProviding {
  var implementationId: String = "signalasi.ios.visible_capture.uikit_avfoundation"
...
private struct AgentIOSUIKitVisibleCaptureRuntime {
  ...
  func capturePhoto(
    ...
    guard Self.onMain({ UIApplication.shared.applicationState == .active }) else {
      return failure(
        ...
        message: "User-visible photo capture requires SignalASI to be in the foreground",
        retryable: true
      )
    }
  ...
  func recordAudio(
    ...
    guard Self.onMain({ UIApplication.shared.applicationState == .active }) else {
      return failure(
        code: "visible_capture_not_foreground",
        message: "User-visible audio capture requires SignalASI to be in the foreground",
        retryable: true
      )
    }
