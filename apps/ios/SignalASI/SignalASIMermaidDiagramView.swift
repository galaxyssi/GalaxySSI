...
struct SignalASIMermaidDiagramView: View {
  ...
  var body: some View {
    ...
    .fullScreenCover(isPresented: $showsFullscreen) {
      SignalASIMermaidFullscreenView(source: source, title: title)
    }
  }
...
private struct SignalASIMermaidFullscreenView: View {
  ...
  var body: some View {
    NavigationView {
      ZStack {
        Color.signalASISurface.ignoresSafeArea()
        SignalASIMermaidWebView(
          source: source,
          darkMode: colorScheme == .dark,
          snapshotRequest: snapshotRequest,
...
private struct SignalASIMermaidWebView: UIViewRepresentable {
  let source: String
  let darkMode: Bool
  ...
  final class Coordinator: NSObject, WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler {
    static let messageName = "signalasiMermaid"

    var parent: SignalASIMermaidWebView
    weak var webView: WKWebView?
    var loadedSource = ""
    var loadedDarkMode = false
    var snapshotRequest = 0

    init(parent: SignalASIMermaidWebView) {
      self.parent = parent
    }
    ...
    func snapshot() {
      guard let webView, webView.bounds.width > 0, webView.bounds.height > 0 else {
        parent.onSnapshot(.failure(SignalASIError.invalidPayload("Diagram preview is unavailable.")))
        return
      }
      ...
      webView.takeSnapshot(with: configuration) { [weak self] image, error in
        ...
        do {
          ...
          guard let data = image?.pngData() else {
            throw SignalASIError.invalidPayload("Diagram snapshot could not be encoded.")
          }
          let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("signalasi-mermaid", isDirectory: true)
          try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
          let url = directory.appendingPathComponent("SignalASI-diagram-\(UUID().uuidString).png")
    ...
    private static func document(source: String, darkMode: Bool) -> String {
      ...
      <html>
        ...
        <body>
          ...
          <script>
            (() => {
              const notify = value => window.webkit.messageHandlers.signalasiMermaid.postMessage(value);
              try {
                mermaid.initialize({startOnLoad:false,securityLevel:'strict',theme:'\(theme)',suppressErrorRendering:true});
                mermaid.render('signalasi-diagram', document.getElementById('source').textContent)
                  .then(result => {
