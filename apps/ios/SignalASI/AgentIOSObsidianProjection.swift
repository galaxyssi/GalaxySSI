...
enum AgentIOSObsidianBridge {
  ...
  static func note(
    ...
    let resolvedTitle = cleanTitle.ifBlank("SignalASI")
    ...
    var lines = [
      "---",
      "signalasi_id: \"\(yaml(sourceKey))\"",
      "signalasi_type: \"\(yaml(type))\"",
      "status: current",
    ...
    lines.append("managed_by: signalasi")
  ...
  private static func projectionSpecs(appStore: SignalASIStore) -> [AgentIOSObsidianProjectionSpec] {
    ...
    if !plans.isEmpty {
      ...
      specs.append(.init(sourceKey: "plans:current", relativePath: "50 Plans/SignalASI plans.md", sourceRevision: revision) {
        note(
          sourceKey: "plans:current",
          type: "plan",
          title: "SignalASI plans",
          source: "SignalASI memory",
    ...
    if !insights.isEmpty {
      ...
      specs.append(.init(sourceKey: "insights:current", relativePath: "40 Insights/SignalASI insights.md", sourceRevision: revision) {
        note(
          sourceKey: "insights:current",
          type: "insight",
          title: "SignalASI insights",
          source: "SignalASI proactive cognition",
    ...
    for conversation in appStore.agentSessions(includeArchived: true) where !conversation.privateMode && !conversation.trackingPaused {
      ...
      specs.append(.init(
        ...
        let body = messages.map { message in
          let role = message.isMine ? "You" : "SignalASI"
  ...
  private static func fileName(_ title: String, sourceKey: String) -> String {
    ...
    let bounded = String(clean.prefix(80)).ifBlank("SignalASI")
