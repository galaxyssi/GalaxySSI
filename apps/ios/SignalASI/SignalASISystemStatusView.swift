...
struct SignalASISystemStatusView: View {
    ...
    HStack(alignment: .center, spacing: 12) {
      ZStack {
        RoundedRectangle(cornerRadius: 8, style: .continuous)
          .fill((needsAttention ? Color.orange : Color.signalASIAccent).opacity(0.16))
        Image(systemName: needsAttention ? "exclamationmark.triangle" : "checkmark.shield")
          .font(.system(size: 20, weight: .semibold))
          .foregroundColor(needsAttention ? .orange : .signalASIAccent)
      }
      .frame(width: 44, height: 44)
  ...
  private var coreServicesSection: some View {
    VStack(alignment: .leading, spacing: 8) {
      SignalASISecuritySectionTitle(title: t("cc_section_core_services", "Core Services"))
      ...
      SignalASISecurityNavigationRow(
        ...
        tint: store.agentKnowledgeStats.sourceCount > 0 ? .blue : .signalASITextSecondary,
  ...
  private var diagnosticsSection: some View {
    VStack(alignment: .leading, spacing: 8) {
      ...
      SignalASISecurityNavigationRow(
        ...
        systemImage: "antenna.radiowaves.left.and.right",
        tint: linkSnapshot.failureCount > 0 ? .orange : .signalASIAccent,
...
private struct SignalASISystemStatusMetricCard: View {
  var title: String
  var value: String
