...
struct SignalASIWebPagePreviewView: View {
  let url: URL
  let title: String
    ...
    VStack(alignment: .leading, spacing: 7) {
      ...
      ZStack {
        ...
        if isLoading && !hasFailed {
          ProgressView()
            .padding(12)
            .background(Color.signalASISurface.opacity(0.92))
...
private struct SignalASIWebPageView: UIViewRepresentable {
  let url: URL
  let onStart: () -> Void
  let onFinish: () -> Void
  let onFailure: () -> Void
