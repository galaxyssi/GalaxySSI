...
enum GalaxySSIVoiceProviderFormatter {
...
struct GalaxySSIVoiceASRProviderView: View {
      ...
      ScrollView {
        VStack(alignment: .leading, spacing: 12) {
          GalaxySSISecurityHeroView(
            ...
            tint: GalaxySSIVoiceProviderFormatter.capabilityTint(route.capability),
            badge: GalaxySSIVoiceProviderFormatter.capabilityStatus(route.capability, language: interfaceLanguage)
          )
  ...
  private var recognitionSection: some View {
    VStack(alignment: .leading, spacing: 8) {
      GalaxySSISecuritySectionTitle(title: t("voice_asr_recognition_mode_section", "Recognition"))
      GalaxySSIVoiceProviderMenuRow(
      ...
      GalaxySSISecurityStatusRow(
        title: t("galaxyssi.voice.active_route", "Active route"),
        subtitle: route.provider,
        systemImage: "arrow.triangle.branch",
        tint: route.usesFallback ? .orange : .galaxySSIAccent,
        badge: route.usesFallback
          ? t("galaxyssi.voice.fallback_route", "Fallback")
          : GalaxySSIVoiceProviderFormatter.capabilityStatus(route.capability, language: interfaceLanguage)
      )
    }
  }
  ...
  private var onlinePrivacySection: some View {
    VStack(alignment: .leading, spacing: 8) {
      GalaxySSISecuritySectionTitle(title: t("voice_asr_online_privacy_section", "Online audio privacy"))
      toggleRow(
      ...
      GalaxySSISecurityStatusRow(
        ...
        tint: VoiceOnlineRealtimeASRConfiguration.isConfigured ? .galaxySSIAccent : .orange,
      ...
      GalaxySSISecurityStatusRow(
        title: t("galaxyssi.voice.network_status", "Network status"),
        subtitle: networkDetail,
        systemImage: "network",
        tint: validatedNetworkAvailable ? .galaxySSIAccent : .orange,
  ...
  private var remoteNodeSection: some View {
    VStack(alignment: .leading, spacing: 8) {
      GalaxySSISecuritySectionTitle(title: t("voice_asr_remote_section", "My Desktop accuracy node"))
      toggleRow(
      ...
      GalaxySSISecurityStatusRow(
        ...
        systemImage: "display",
        tint: verifiedRemoteWhisperNodes.isEmpty ? .orange : .galaxySSIAccent,
  ...
  private var deviceCapabilitySection: some View {
    VStack(alignment: .leading, spacing: 8) {
      GalaxySSISecuritySectionTitle(title: t("voice_provider_device_capabilities", "Device capabilities"))
  ...
  private var runtimePolicySection: some View {
    VStack(alignment: .leading, spacing: 8) {
      GalaxySSISecuritySectionTitle(title: t("voice_asr_runtime_mode_section", "Runtime policy"))
      GalaxySSIVoiceProviderMenuRow(
  ...
  private var modelSection: some View {
    VStack(alignment: .leading, spacing: 8) {
      GalaxySSISecuritySectionTitle(title: t("voice_asr_model_section", "Whisper model"))
  ...
  private var recheckSection: some View {
    VStack(alignment: .leading, spacing: 8) {
      GalaxySSISecuritySectionTitle(title: t("voice_provider_device_check", "Device check"))
  ...
  private func asrCapabilityRow(_ capability: VoiceProviderCapability) -> some View {
    ...
    let badge = GalaxySSIVoiceProviderFormatter.capabilityStatus(capability, language: interfaceLanguage)
    let tint = GalaxySSIVoiceProviderFormatter.capabilityTint(capability)
    let icon = GalaxySSIVoiceProviderFormatter.capabilityIcon(capability.id)
    if capability.state == .needsPermission {
      GalaxySSISecurityActionRow(
        title: GalaxySSIVoiceProviderFormatter.capabilityTitle(capability.id, language: interfaceLanguage),
        subtitle: subtitle,
        systemImage: icon,
        tint: tint,
        badge: badge
  ...
  private var recognitionPreferenceChoices: [GalaxySSIVoiceProviderChoice] {
    ...
    return preferences.map {
      GalaxySSIVoiceProviderChoice(value: $0.rawValue, title: recognitionPreferenceTitle($0))
    }
  }

  private var recognitionModeChoices: [GalaxySSIVoiceProviderChoice] {
    VoiceWhisperUserVoiceMode.allCases.map {
      GalaxySSIVoiceProviderChoice(value: $0.rawValue, title: recognitionModeTitle($0))
    }
  }
  ...
  private var networkDetail: String {
    ...
    if validatedNetworkAvailable {
      return t("galaxyssi.voice.network_ready_detail", "Validated network is available") + " / " + transports
    }
  ...
  private func asrCapabilityDetail(_ capability: VoiceProviderCapability) -> String {
    ...
    return GalaxySSIVoiceProviderFormatter.capabilityDetail(capability, language: interfaceLanguage)
  }
...
struct GalaxySSIVoiceTTSProviderView: View {
  @Environment(\.galaxySSIInterfaceLanguage) private var interfaceLanguage
  @EnvironmentObject private var store: GalaxySSIStore
  @State private var refreshGeneration = 0
        ...
        VStack(alignment: .leading, spacing: 12) {
          GalaxySSISecurityHeroView(
            ...
            subtitle: t(
              ...
              "Choose a provider only after GalaxySSI verifies it on this device"
            ),
            systemImage: "speaker.wave.2",
            tint: GalaxySSIVoiceProviderFormatter.capabilityTint(activeTtsCapability),
  ...
  private func ttsCapabilityRow(provider: VoiceTTSProvider, capability: VoiceProviderCapability) -> some View {
    ...
    let action: String = {
      ...
      return GalaxySSIVoiceProviderFormatter.capabilityStatus(capability, language: interfaceLanguage)
    }()
    if capability.ready && !selected {
      GalaxySSISecurityActionRow(
        title: GalaxySSIVoiceProviderFormatter.capabilityTitle(capability.id, language: interfaceLanguage),
        subtitle: GalaxySSIVoiceProviderFormatter.capabilityDetail(capability, language: interfaceLanguage),
        systemImage: GalaxySSIVoiceProviderFormatter.capabilityIcon(capability.id),
        tint: GalaxySSIVoiceProviderFormatter.capabilityTint(capability),
        badge: action
      ) {
  ...
  private var voiceLanguageChoices: [GalaxySSIVoiceProviderChoice] {
    LanguagePolicySettings.voiceChoices.map {
      GalaxySSIVoiceProviderChoice(value: $0, title: voiceLanguageTitle($0))
    }
  }
...
private struct GalaxySSIVoiceProviderChoice: Identifiable {
  var value: String
  var title: String
...
private struct GalaxySSIVoiceProviderMenuRow: View {
  var title: String
  var subtitle: String
  var systemImage: String
  var tint: Color
  var badge: String
  var choices: [GalaxySSIVoiceProviderChoice]
