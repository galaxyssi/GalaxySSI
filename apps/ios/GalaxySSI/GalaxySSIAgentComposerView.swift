...
struct GalaxySSIAgentComposerView: View {
  ...
  var onRemoveAttachment: (GalaxySSIDraftAttachment) -> Void
  ...
  var onVoiceTranscript: (GalaxySSIVoiceTranscriptSubmission) -> Void
  var t: (String, String) -> String

  private var uiState: GalaxySSIAgentComposerUiState {
    GalaxySSIAgentComposerUiPolicy.resolve(
      hasInput: canSend,
  ...
  var body: some View {
    VStack(spacing: 8) {
      if newGlobalInsightCount > 0 {
        GalaxySSIAgentHomeInsightBarView(count: newGlobalInsightCount)
      }
      ...
      HStack(spacing: 8) {
        ForEach(mentionSuggestions, id: \.id) { target in
          ...
          .accessibilityLabel(Text(
            String(
              format: t("galaxyssi.agent.mention_select", "Select Agent %@"),
              target.title
            )
          ))
        }
  ...
  private var inputShell: some View {
    GalaxySSIGrowingComposerEditor(
      text: $draft,
      placeholder: t("galaxyssi.agent.goal_hint", "Enter message or hold to talk..."),
      focus: $inputFocused,
  ...
  private var recordingSurface: some View {
    VStack(spacing: 12) {
      ...
      Text(holdToTalk.elapsedLabel)
        .font(.system(size: 12, weight: .semibold))
        .foregroundColor(holdToTalk.cancelPending ? .galaxySSIAgentVoiceCancel : .white)
    }
    ...
    .background(
      LinearGradient(
        gradient: Gradient(stops: [
          ...
          .init(
            color: Color.galaxySSIAgentRecordingLight.opacity(30.0 / 255.0),
            location: 0.16
          ),
          .init(
            color: Color.galaxySSIAgentRecordingLight.opacity(132.0 / 255.0),
            location: 0.36
          ),
          .init(
            color: Color.galaxySSIAgentRecordingMid.opacity(224.0 / 255.0),
            location: 0.64
          ),
          .init(color: Color.galaxySSIAgentRecordingDeep, location: 1.0),
        ]),
        startPoint: .top,
        endPoint: .bottom
      )
  ...
  private var primaryActionButton: some View {
    if !uiState.showPrimaryActionSlot {
      ...
      .accessibilityLabel(Text(canSend
        ? t("galaxyssi.common.send", "Send")
        : pendingPrimaryActionResumesTask
          ? t("galaxyssi.agent.resume_task", "Resume task")
          : pendingPrimaryActionApprovesTask
            ? t("galaxyssi.agent.confirmation.allow_once", "Allow once")
            : pendingPrimaryActionWaitingForResponse
      ...
      Button {
        ...
        GalaxySSIComposerMoreButtonIcon(expanded: trayVisible)
          .frame(width: 54, height: 54)
      }
      .buttonStyle(.plain)
      .frame(minWidth: minimumTouchSize, minHeight: minimumTouchSize)
  ...
  private var trayActions: [GalaxySSIComposerTrayAction] {
    [
...
struct GalaxySSIAgentVoiceAttachmentSummaryView: View {
  var attachments: [GalaxySSIDraftAttachment]
  var t: (String, String) -> String
...
struct GalaxySSIVoiceTranscriptionPendingView: View {
    ...
    HStack {
      ...
      HStack(spacing: 5) {
        ForEach(0..<3, id: \.self) { index in
          Circle()
            .fill(Color.galaxySSIAgentRecordingDeep)
      ...
      .background(Color.galaxySSIAgentRecordingLight)
      .clipShape(RoundedRectangle(cornerRadius: 8, style: .continuous))
    }
    .frame(maxWidth: .infinity)
    .accessibilityElement(children: .ignore)
    .accessibilityLabel(t("galaxyssi.voice.transcription_pending", "Recognizing voice"))
    .onAppear {
      isPulsing = true
    }
    .onDisappear {
...
private struct GalaxySSIAgentRecordingTranscript: View {
...
private struct GalaxySSIAgentRecordingWaveform: View {
  var phase: Double
  ...
  var body: some View {
    GeometryReader { proxy in
      ...
      HStack(alignment: .center, spacing: max(1, step * 0.68)) {
        ForEach(0..<sampleCount, id: \.self) { index in
          Capsule(style: .continuous)
            .fill(cancelPending ? Color.galaxySSIAgentVoiceCancel : .white)
      ...
      .overlay {
        Rectangle()
          .fill((cancelPending ? Color.galaxySSIAgentVoiceCancel : .white).opacity(0.28))
          .frame(height: 1)
      }
