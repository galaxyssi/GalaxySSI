...
final class AgentIncomingAttachmentTransferStore {
    ...
    locked {
      ...
      return directories.compactMap { directory in
        ...
        let pending = PendingDownload(
          transferId: manifest.string("transfer_id"),
          sourceId: manifest.string("source_id"),
          localGalaxySSIId: manifest.string("contact_id"),
          clientRouteId: manifest.string("client_route_id")
        )
        guard isDigest(pending.transferId),
              !pending.sourceId.isEmpty,
              !pending.localGalaxySSIId.isEmpty,
              GalaxySSILinkProtocol.validRouteId(pending.clientRouteId) else { return nil }
        return pending
      }
    }
  }
  ...
  func resumeReceipt(for pending: PendingDownload) -> [String: Any]? {
    locked {
      ...
      guard let manifest = readManifest(directory),
            ...
            manifest.string("contact_id") == pending.localGalaxySSIId,
      ...
      var value = receipt(
        manifest,
        status: "missing",
        localGalaxySSIId: pending.localGalaxySSIId
      )
  ...
  func progressEvent(
    ...
    locked {
      ...
      var event: [String: Any] = [
        "type": GalaxySSIPeerAttachmentTransferProgress.payloadType,
        "contact_id": localGalaxySSIId,
        "direction": "inbound",
        ...
        "progress": GalaxySSIPeerAttachmentTransferProgress.percent(
  ...
  private func normalizedManifest(
    ...
    guard isDigest(transferId), isDigest(digest),
          ...
          payload.string("contact_id") == localGalaxySSIId,
  ...
  private func manifestMatches(
    ...
    manifest.string("source_id") == sourceId &&
      manifest.string("contact_id") == localGalaxySSIId &&
  ...
  private func pruneLocked() throws {
    ...
    for directory in directories where isDigest(directory.lastPathComponent) {
      ...
      if GalaxySSIPeerMessageAttachmentStore.shouldPruneIncoming(
