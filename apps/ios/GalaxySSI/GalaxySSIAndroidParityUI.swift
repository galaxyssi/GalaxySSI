...
struct AgentHomeView: View {
  ...
  @State var homeActionEditorSelection: GalaxySSIAgentRuntimeActionSelection?
  ...
  @State var voiceAttachmentSnapshot: [GalaxySSIDraftAttachment] = []
  @State var voicePendingAttachments: [GalaxySSIDraftAttachment] = []
  @State var agentVoiceDraftSnapshot: AgentVoiceDraftSnapshot?
  @State var runtimeArtifactPreview: GalaxySSIRuntimeArtifactPreview?
  @State var runtimeArtifactDocument: GalaxySSIRuntimeArtifactDocument?
  ...
  @State private var publicPageExportDocument: GalaxySSIPublicPageHTMLExportDocument?
  ...
  var contact: GalaxySSIContact {
    store.contact(id: "hermes") ?? GalaxySSIContact.hermes()
  }
  ...
  var body: some View {
    NavigationView {
      ...
      .onReceive(
        ...
        scanStatus = t(
          "galaxyssi.agent.scan.selecting",
      ...
      .onReceive(
        NotificationCenter.default.publisher(for: .galaxySSIAgentRoutingDidUpdate)
      ) { _ in
        refreshAgentRouteState()
        retryPendingScannedAgentSelection()
      }
  ...
  private func presentPendingPhonePublicPageExport() {
    ...
    publicPageExportDocument = GalaxySSIPublicPageHTMLExportDocument(data: export.data)
  ...
  func retryAgentTask(_ task: AgentTaskRecord, mode: AgentTaskRestartMode) {
    ...
    case .replan:
      request = String(
        format: t(
          "galaxyssi.agent.task_control.replan_prompt",
    ...
    richActionStatus = mode == .replan
      ? t("galaxyssi.agent.task_control.replanning", "Re-planning task...")
      : t("galaxyssi.agent_tasks.retrying", "Retrying task...")
    Task { @MainActor in
      if mode == .replan,
         await coordinator.replanLocalNativeAction(taskId: task.taskId) {
        retryingAgentTaskIDs.remove(task.taskId)
        richActionStatus = t("galaxyssi.agent.task_control.replanned", "Task re-planned")
      ...
      if !sent {
        richActionStatus = t(
          "galaxyssi.agent_tasks.retry_failed",
  ...
  var headerPresentation: GalaxySSIAgentHomeHeaderPresentation {
    GalaxySSIAgentHomeHeaderPresentation.make(
  ...
  func prefillAgentScreenCommand(_ command: String) {
    ...
    case "paste clipboard":
      ...
      guard !clipboardText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
        attachmentError = t("galaxyssi.agent.input.clipboard_empty", "Clipboard is empty.")
        return
      }
      draft = clipboardText
      actionTrayPresented = false
  ...
  func mergedSourceLabel(for message: ChatMessage) -> String? {
    ...
    return String(
      format: t(
        "galaxyssi.agent.session.merged_from",
        "Merged from session: %@"
      ),
      sourceTitle
    )
