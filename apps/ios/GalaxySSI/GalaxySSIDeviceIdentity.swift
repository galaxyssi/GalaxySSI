...
struct GalaxySSILocalDeviceProfile {
  let deviceId: String
...
enum GalaxySSIDeviceIdentity {
  static func current(profile: GalaxySSIProfile) -> GalaxySSILocalDeviceProfile {
...
enum GalaxySSIDeviceIdentityName {
  private static let maxDeviceNameLength = 48

  static func current(profile: GalaxySSIProfile) -> String {
    current(galaxySSIId: profile.galaxySSIId)
  }

  static func current(galaxySSIId: String) -> String {
    format(
  ...
  static func format(deviceName: String, galaxySSIId: String) -> String {
    let normalizedName = deviceName
      ...
      .trimmingCharacters(in: .whitespacesAndNewlines)
      .ifBlank("iPhone")
    let identitySource = galaxySSIId
