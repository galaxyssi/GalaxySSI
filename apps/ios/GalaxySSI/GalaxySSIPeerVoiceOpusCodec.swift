...
enum GalaxySSIPeerVoiceOpusError: LocalizedError {
...
struct GalaxySSIPeerVoiceEncodingResult {
  var data: Data
...
final class GalaxySSIPeerVoicePCMRecorder {
  ...
  func start() throws {
    ...
    try session.setPreferredSampleRate(Double(GalaxySSIPeerVoiceMessageAudio.sampleRateHz))
    ...
    guard format.sampleRate > 0, format.channelCount > 0 else {
      throw GalaxySSIPeerVoiceOpusError.captureUnavailable
    }
    lock.lock()
  ...
  func stopAndEncode() throws -> GalaxySSIPeerVoiceEncodingResult {
    let capture = stopCapture()
    guard !capture.samples.isEmpty, capture.sampleRate > 0 else {
      throw GalaxySSIPeerVoiceOpusError.emptyRecording
    }
    return try GalaxySSIPeerVoiceOpusCodec.encode(
      capture.samples,
  ...
  private func append(_ buffer: AVAudioPCMBuffer) {
    ...
    lock.lock()
    let limit = Int(max(captureSampleRate, 1) * GalaxySSIPeerVoiceMessageAudio.maximumDuration)
...
enum GalaxySSIPeerVoiceOpusCodec {
  static func encode(
    _ source: [Float],
    sourceSampleRate: Double,
    voiceProcessingEnabled: Bool
  ) throws -> GalaxySSIPeerVoiceEncodingResult {
    var samples = resample(source, sourceRate: sourceSampleRate)
    guard !samples.isEmpty else { throw GalaxySSIPeerVoiceOpusError.emptyRecording }
    let dsp = GalaxySSIPeerVoiceDSP.process(&samples)
    let pcm = samples.map { value -> Int16 in
      Int16(clamping: Int((value * Float(Int16.max)).rounded()))
    }
    let duration = Int64(pcm.count) * 1_000 / Int64(GalaxySSIPeerVoiceMessageAudio.sampleRateHz)
    var packets = try GalaxySSIPeerVoiceCoreAudioOpusEncoder.encode(pcm)
    defer {
      for index in packets.indices { packets[index].wipeSensitive() }
      packets.removeAll(keepingCapacity: false)
    }
    return GalaxySSIPeerVoiceEncodingResult(
      data: try GalaxySSIOggOpus.write(packets: packets, inputSampleCount: pcm.count),
  ...
  private static func resample(_ source: [Float], sourceRate: Double) -> [Float] {
    let targetRate = Double(GalaxySSIPeerVoiceMessageAudio.sampleRateHz)
  ...
  static func pcmWave(_ samples: [Int16]) -> Data {
    ...
    data.appendLE(UInt16(1))
    data.appendLE(UInt32(GalaxySSIPeerVoiceMessageAudio.sampleRateHz))
    data.appendLE(UInt32(GalaxySSIPeerVoiceMessageAudio.sampleRateHz * 2))
...
enum GalaxySSIPeerVoiceOpusPlayback {
  static func pcmWaveData(fromOggOpus data: Data) throws -> Data {
    var packets = try GalaxySSIOggOpus.audioPackets(from: data)
    defer {
      for index in packets.indices { packets[index].wipeSensitive() }
      packets.removeAll(keepingCapacity: false)
    }
    ...
    return GalaxySSIPeerVoiceOpusCodec.pcmWave(samples)
  }

  private static func decode(_ packets: [Data]) throws -> [Int16] {
    var inputDescription = AudioStreamBasicDescription(
      mSampleRate: Double(GalaxySSIPeerVoiceMessageAudio.sampleRateHz),
    ...
    guard let inputFormat = AVAudioFormat(streamDescription: &inputDescription),
          let outputFormat = AVAudioFormat(
            ...
            sampleRate: Double(GalaxySSIPeerVoiceMessageAudio.sampleRateHz),
            channels: 1,
            interleaved: false
          ),
          let converter = AVAudioConverter(from: inputFormat, to: outputFormat) else {
      throw GalaxySSIPeerVoiceOpusError.encoderUnavailable
    }
    var packetIndex = 0
    var samples: [Int16] = []
    ...
    while !reachedEnd {
      guard let output = AVAudioPCMBuffer(pcmFormat: outputFormat, frameCapacity: 9_600) else {
        throw GalaxySSIPeerVoiceOpusError.encodingFailed("Unable to allocate Opus playback buffer.")
      }
      ...
      if let conversionError {
        throw GalaxySSIPeerVoiceOpusError.encodingFailed(conversionError.localizedDescription)
      }
      ...
      case .endOfStream:
        reachedEnd = true
      case .error:
        throw GalaxySSIPeerVoiceOpusError.encodingFailed("Core Audio rejected the Opus message.")
      case .haveData, .inputRanDry:
        if packetIndex >= packets.count, emptyDrainPasses > 2 {
          throw GalaxySSIPeerVoiceOpusError.encodingFailed("The Opus decoder did not finish draining.")
        }
      @unknown default:
        throw GalaxySSIPeerVoiceOpusError.encodingFailed("Unknown Core Audio decoding status.")
      }
    }
    guard !samples.isEmpty else { throw GalaxySSIPeerVoiceOpusError.emptyRecording }
    return samples
...
struct GalaxySSIPeerVoiceDSPResult {
  var measuredLUFS: Double?
  var appliedGainDecibels: Double
  var outputPeakDBFS: Double
}

enum GalaxySSIPeerVoiceDSP {
  private static let absoluteGateLUFS = -70.0
  private static let relativeGateLU = 10.0
  private static let loudnessOffset = -0.691

  static func process(_ samples: inout [Float]) -> GalaxySSIPeerVoiceDSPResult {
    applyHighPass(&samples, cutoffHz: Double(GalaxySSIPeerVoiceMessageAudio.highPassHz))
    let measured = integratedLUFS(samples)
    let requestedDB = measured.map { GalaxySSIPeerVoiceMessageAudio.targetLUFS - $0 } ?? 0
    let requestedGain = pow(10, requestedDB / 20)
    let peak = samples.reduce(0.0) { max($0, Double(abs($1))) }
    let limit = pow(10, GalaxySSIPeerVoiceMessageAudio.peakDBFS / 20)
    ...
    return GalaxySSIPeerVoiceDSPResult(
  ...
  static func applyHighPass(_ samples: inout [Float], cutoffHz: Double) {
    let dt = 1.0 / Double(GalaxySSIPeerVoiceMessageAudio.sampleRateHz)
  ...
  static func integratedLUFS(_ samples: [Float]) -> Double? {
    let block = GalaxySSIPeerVoiceMessageAudio.sampleRateHz * 400 / 1_000
    let step = GalaxySSIPeerVoiceMessageAudio.sampleRateHz * 100 / 1_000
...
enum GalaxySSIPeerVoiceCoreAudioOpusEncoder {
  ...
  static func encode(_ samples: [Int16]) throws -> [Data] {
    ...
    guard let inputFormat = AVAudioFormat(
      ...
      channels: 1,
      interleaved: false
    ) else { throw GalaxySSIPeerVoiceOpusError.encoderUnavailable }
    ...
    converter.bitRate = GalaxySSIPeerVoiceMessageAudio.opusBitRateBPS
    ...
    while !reachedEnd {
      ...
      case .endOfStream:
        reachedEnd = true
      case .error:
        throw GalaxySSIPeerVoiceOpusError.encodingFailed("Core Audio rejected the Opus stream.")
      case .haveData, .inputRanDry:
        if inputEnded, emptyDrainPasses > 2 {
          throw GalaxySSIPeerVoiceOpusError.encodingFailed("The Opus encoder did not finish draining.")
        }
      @unknown default:
        throw GalaxySSIPeerVoiceOpusError.encodingFailed("Unknown Core Audio conversion status.")
      }
    }
    guard !packets.isEmpty else {
      throw GalaxySSIPeerVoiceOpusError.encodingFailed("The Opus encoder produced no audio packets.")
    }
    return packets
  }
  ...
  private static func appendPackets(
    ...
    if let descriptions = buffer.packetDescriptions {
      ...
      throw GalaxySSIPeerVoiceOpusError.encodingFailed("Opus packet boundaries are unavailable.")
    }
  }
}

enum GalaxySSIOggOpus {
  ...
  static func write(packets: [Data], inputSampleCount: Int) throws -> Data {
    guard !packets.isEmpty, inputSampleCount > 0 else {
      throw GalaxySSIPeerVoiceOpusError.encodingFailed("Opus packets are empty.")
    }
    var output = Data()
  ...
  static func audioPackets(from data: Data) throws -> [Data] {
    ...
    while offset + 27 <= data.count {
      guard data.ascii(at: offset, count: 4) == "OggS", data[offset + 4] == 0 else {
        throw GalaxySSIPeerVoiceOpusError.invalidContainer
      }
      let segments = Int(data[offset + 26])
      let tableStart = offset + 27
      let payloadStart = tableStart + segments
      guard payloadStart <= data.count else { throw GalaxySSIPeerVoiceOpusError.invalidContainer }
  ...
  private static func opusTags() -> Data {
    let vendor = Data("GalaxySSI iOS".utf8)
  ...
  private static func page(
    ...
    guard lacing.count <= 255 else {
      throw GalaxySSIPeerVoiceOpusError.encodingFailed("An Ogg page has too many segments.")
    }
