...
enum GalaxySSIPeerMessageAction: Equatable {
  case copy
  case transcribe
  case delete
}

struct GalaxySSIPeerVoiceAttachment: Equatable {
  ...
  static func decode(from message: ChatMessage) -> GalaxySSIPeerVoiceAttachment? {
    AgentRichContentCodec.decode(message.richOutputJson)
      ...
      .map { block in
        ...
        let metadataExtension = block.metadata["display_extension"]?
          ...
          .lowercased()
        return GalaxySSIPeerVoiceAttachment(
  ...
  func loadSensitiveData() throws -> Data {
    if storage == "attachment_aes_256_gcm",
       ...
       let sourceURL,
       sourceURL.isFileURL {
      return try GalaxySSIAttachmentAtRestCipher.shared.read(
        from: sourceURL,
        purpose: encryptionPurpose
      )
    }
    guard let data = GalaxySSIPeerMessageAttachmentStore().resolveAudioData(
      displayName: displayName,
      sourceURL: sourceURL
    ), !data.isEmpty else {
      throw GalaxySSIPeerVoiceTranscriptionError.audioUnavailable
    }
    return data
  }
}

enum GalaxySSIPeerMessageActionPolicy {
  static func voiceAttachment(in message: ChatMessage) -> GalaxySSIPeerVoiceAttachment? {
    GalaxySSIPeerVoiceAttachment.decode(from: message)
  }

  static func actions(for message: ChatMessage) -> [GalaxySSIPeerMessageAction] {
...
enum GalaxySSIPeerVoiceTranscriptionError: LocalizedError, Equatable {
...
actor GalaxySSIPeerVoiceTranscriber {
  static let shared = GalaxySSIPeerVoiceTranscriber()
  ...
  func transcribe(
    ...
    guard !busy else { throw GalaxySSIPeerVoiceTranscriptionError.busy }
    busy = true
    defer { busy = false }

    guard let attachment = GalaxySSIPeerMessageActionPolicy.voiceAttachment(in: message) else {
      throw GalaxySSIPeerVoiceTranscriptionError.audioUnavailable
    }
    var encodedAudio = try attachment.loadSensitiveData()
    defer { encodedAudio.wipeSensitive() }
    var decodedAudio = try GalaxySSIPeerVoiceInMemoryDecoder.decode(
      encodedAudio,
    ...
    guard !transcript.isEmpty else {
      throw GalaxySSIPeerVoiceTranscriptionError.emptyTranscript
    }
    return transcript
  }
}

enum GalaxySSIPeerVoiceInMemoryDecoder {
  static func decode(
    _ data: Data,
    ...
    if normalizedExtension == "opus" ||
      ...
      var wave = try GalaxySSIPeerVoiceOpusPlayback.pcmWaveData(fromOggOpus: data)
    ...
    if ["m4a", "aac", "mp4", "caf"].contains(normalizedExtension) ||
      ...
      return try GalaxySSIAppleInMemoryAudioDecoder.decode(
        data,
        fileExtension: normalizedExtension
      )
    }
    throw GalaxySSIPeerVoiceTranscriptionError.unsupportedAudioFormat(normalizedExtension)
  }
}

private final class GalaxySSIInMemoryAudioReader {
  var data: Data
...
private enum GalaxySSIAppleInMemoryAudioDecoder {
  ...
  private static let readCallback: AudioFile_ReadProc = {
    ...
    actualCount
    in
    let reader = Unmanaged<GalaxySSIInMemoryAudioReader>
  ...
  static func decode(_ source: Data, fileExtension: String) throws -> VoiceWhisperAudio {
    let reader = GalaxySSIInMemoryAudioReader(data: source)
    ...
    guard openStatus == noErr, let audioFile else {
      throw GalaxySSIPeerVoiceTranscriptionError.audioDecodeFailed(openStatus)
    }
    ...
    guard wrapStatus == noErr, let extendedFile else {
      throw GalaxySSIPeerVoiceTranscriptionError.audioDecodeFailed(wrapStatus)
    }
    ...
    guard sourceStatus == noErr else {
      throw GalaxySSIPeerVoiceTranscriptionError.audioDecodeFailed(sourceStatus)
    }
    ...
    guard clientStatus == noErr else {
      throw GalaxySSIPeerVoiceTranscriptionError.audioDecodeFailed(clientStatus)
    }
    ...
    while true {
      ...
      guard readStatus == noErr else {
        throw GalaxySSIPeerVoiceTranscriptionError.audioDecodeFailed(readStatus)
      }
