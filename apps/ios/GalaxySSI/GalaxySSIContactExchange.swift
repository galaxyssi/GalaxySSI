...
enum GalaxySSIQRCodeImport {
  case desktopPairing(PairingQRCode)
  case contact(GalaxySSIFriendRequest)
  case contacts([GalaxySSIFriendRequest])
}

struct GalaxySSIConnectorAgentSource {
  var parentPayload: [String: Any]
  var agents: [[String: Any]]
}

enum GalaxySSIQRCodePayload {
  static func decodeObject(from contents: String, label: String) throws -> [String: Any] {
    ...
    while textIndex < texts.count || objectIndex < objects.count {
      ...
      while objectIndex < objects.count {
        ...
        objectIndex += 1
        if looksLikeGalaxySSIObject(object) {
          return object
        }
        if fallback == nil {
          fallback = object
    ...
    throw GalaxySSIError.invalidPayload("\(label) root must be a JSON object.")
  }
  ...
  private static func looksLikeGalaxySSIObject(_ object: [String: Any]) -> Bool {
    if object.string("t") == GalaxySSIContactExchange.compactPhoneQRType {
      return true
    }
    let type = object.string("type").trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    let hasIdentity = !object.string("galaxyssi_id").isEmpty ||
    ...
    let hasPairing = !object.string("pairing_token").isEmpty ||
      ...
      object.string("protocol") == GalaxySSILinkProtocol.name
    let hasAgentList = GalaxySSIContactExchange.connectorAgentListKeys.contains { object[$0] != nil }
    ...
    case "galaxyssi_contact", "hermes_contact", "opaque_contact", "opaque_identity":
      return hasIdentity
    case "agent", "agent_contact", "galaxyssi_agent", "galaxyssi_agent_contact",
         "device", "device_contact", "galaxyssi_device", "galaxyssi_device_contact":
...
enum GalaxySSIContactExchange {
  static let contactType = "galaxyssi_contact"
  ...
  static let verifyType = "galaxyssi_verify"
  ...
  static func makeContactQRText(
    ...
    let data = try GalaxySSILinkProtocol.jsonData(payload)
  ...
  static func makeContactPayload(
    ...
    let device = GalaxySSIDeviceIdentity.current(profile: profile)
    return [
      "type": contactType,
      "version": version,
  ...
  static func makeSignedPhoneContactQRText(
    profile: GalaxySSIProfile,
    signalIdentity: GalaxySSISignalIdentity,
    pairingToken: String,
    ...
    guard GalaxySSILinkProtocol.validLinkSecret(pairingToken),
          GalaxySSILinkProtocol.validLinkSecret(pairingSecret),
          pairingTopic == GalaxySSILinkProtocol.pairingTopic(secret: pairingSecret),
          signalIdentity.name.hasPrefix("galaxyssi:"),
    ...
    var card: [String: Any] = [
      ...
      "galaxyssi_id": signalIdentity.name,
      ...
      "device_id": GalaxySSIDeviceIdentity.current(profile: profile).deviceId,
    ...
    let data = try GalaxySSILinkProtocol.jsonData(compact)
    guard data.count < maximumCompactPhoneQRBytes else {
      throw GalaxySSIError.invalidPayload("Compact contact QR payload exceeds 1 KB.")
    }
    return String(data: data, encoding: .utf8)
  }
  ...
  static func normalizeCompactPhoneContactQR(_ source: [String: Any]) -> [String: Any]? {
    ...
    guard GalaxySSILinkProtocol.validLinkSecret(secret),
          GalaxySSILinkProtocol.validLinkSecret(source.string("x")) else {
      return nil
    }
    let card: [String: Any] = [
      "type": opaqueContactType,
      "version": GalaxySSILinkProtocol.version,
      "galaxyssi_id": source.string("i"),
      ...
      "pairing_topic": GalaxySSILinkProtocol.pairingTopic(secret: secret),
  ...
  static func validateSignedPhoneContactCard(_ card: [String: Any]) throws {
    ...
    guard !signature.isEmpty else {
      if opaque {
        throw GalaxySSIError.invalidPayload("Opaque contact card is unsigned.")
      }
      return
    }
    ...
    let idMatches = galaxySSIId.range(
      of: "^galaxyssi:[a-fA-F0-9]{16}$",
      options: .regularExpression
    ) != nil
    ...
    guard [opaqueContactType, opaqueIdentityType].contains(card.string("type")),
          card.int("version") == GalaxySSILinkProtocol.version,
          idMatches,
          galaxySSIId.dropFirst("galaxyssi:".count).caseInsensitiveCompare(fingerprint.prefix(16)) == .orderedSame,
          ...
          (card.string("type") == opaqueIdentityType || (
            GalaxySSILinkProtocol.validLinkSecret(card.string("pairing_token")) &&
              GalaxySSILinkProtocol.validLinkSecret(pairingSecret) &&
              card.string("pairing_topic") == GalaxySSILinkProtocol.pairingTopic(secret: pairingSecret)
          )),
          createdAtMillis > 0,
          abs(nowMillis - createdAtMillis) <= GalaxySSIPhoneContactControl.maximumAgeMillis,
          signature.count >= 40,
          signature.count <= 256,
          GalaxySSISignalEngine.verifyContactCard(
            publicKey: publicKey,
            payload: canonicalPhoneContactCardBytes(card),
            signature: signature
          ) else {
      throw GalaxySSIError.invalidPayload("Signed contact QR verification failed.")
    }
  }

  static func classifyQRCode(_ contents: String, now: Date = Date()) throws -> GalaxySSIQRCodeImport {
    let rawObject = try decodeQRCodeObject(contents, label: "QR")
    let object = GalaxySSILinkProtocol.normalizePairingQRCode(rawObject)
      ?? normalizeCompactPhoneContactQR(rawObject)
      ?? rawObject
    let type = normalized(object.string("type"))
    let isPairingQRCode = type == "opaque_pairing"
    if isPairingQRCode {
      return .desktopPairing(try GalaxySSILinkProtocol.validatePairingQRCode(object, now: now))
    }
    ...
    throw GalaxySSIError.invalidPayload("Unsupported GalaxySSI QR code.")
  }

  static func importContactQRCode(_ contents: String, now: Date = Date()) throws -> GalaxySSIFriendRequest {
  ...
  static func connectorAgentSource(from object: [String: Any]) -> GalaxySSIConnectorAgentSource? {
    ...
    for candidate in candidates {
      ...
      return GalaxySSIConnectorAgentSource(parentPayload: parent, agents: agents)
    }
    return nil
  }

  private static func importContactQRCodeObject(_ object: [String: Any], now: Date) throws -> GalaxySSIFriendRequest {
    let type = normalized(object.string("type"))
    guard isContactQRCodeObject(object) else {
      throw GalaxySSIError.invalidPayload("Contact QR type is not supported.")
    }
    ...
    guard !fingerprint.isEmpty, !publicKey.isEmpty || desktopAgent else {
      throw GalaxySSIError.invalidPayload("Contact QR is missing identity material.")
    }
    let galaxySSIId = object.string("galaxyssi_id")
      ...
      .ifBlank("galaxyssi:\(fingerprint.prefix(16))")
    ...
    let deviceMetadata = GalaxySSIDesktopDeviceMetadata.from(payload: object)
    return GalaxySSIFriendRequest(
      ...
      desktopAccessProfile: object.string("desktop_access_profile")
        .ifBlank(desktopAgent ? GalaxySSILinkProtocol.accessRestricted : ""),
  ...
  private static func importConnectorAgentRequests(_ object: [String: Any], now: Date) throws -> [GalaxySSIFriendRequest] {
    ...
    guard ["connector_status", "capability_manifest", "pairing_confirmed"].contains(type) ||
      ["agent_manifest", "connector_manifest", "desktop_status", "galaxyssi_agents"].contains(type) ||
    ...
    var requests: [GalaxySSIFriendRequest] = []
  ...
  private static func isContactQRCodeObject(_ object: [String: Any]) -> Bool {
    ...
    return [contactType, hermesContactType, verifyType, opaqueContactType, opaqueIdentityType].contains(type) ||
      ...
      !object.string("galaxyssi_id").isEmpty ||
      !object.string("hermes_id").isEmpty ||
  ...
  private static func desktopName(from object: [String: Any]) -> String {
    GalaxySSIDesktopDeviceMetadata.displayName(from: object)
  ...
  private static func defaultAgentKind(for requestType: String, object: [String: Any]) -> String {
    ...
    case "agent":
      let identifier = normalized(
        object.string("agent_id")
          .ifBlank(object.string("mobile_contact_id"))
          .ifBlank(object.string("galaxyssi_id"))
      )
  ...
  private static func decodeQRCodeObject(_ contents: String, label: String) throws -> [String: Any] {
    try GalaxySSIQRCodePayload.decodeObject(from: contents, label: label)
  }
