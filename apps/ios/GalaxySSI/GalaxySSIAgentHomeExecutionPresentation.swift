...
extension AgentHomeView {
  ...
  func agentExecutionLocationSummary(_ task: AgentTaskRecord) -> String {
    ...
    let summary = [
      ...
      .filter { !$0.isBlank }
      .joined(separator: " · ")
      .ifBlank(t("galaxyssi.agent.execution.unknown", "Execution location unavailable"))
  ...
  func remoteAgentStatusLabel(_ status: String) -> String {
    GalaxySSIRemoteTaskStatusPresentation.title(status, language: interfaceLanguage)
  }

  func remoteAgentStatusTint(_ status: String) -> Color {
    GalaxySSIRemoteTaskStatusPresentation.tint(status)
  }
  ...
  func executionDuration(elapsedMillis: Int64) -> String {
    AgentTranscriptPresentationPolicy.formatProcessedDuration(
      elapsedMillis,
      hoursUnit: t("galaxyssi.agent.trace.duration_hours", "h"),
      minutesUnit: t("galaxyssi.agent.trace.duration_minutes", "m"),
      secondsUnit: t("galaxyssi.agent.trace.duration_seconds", "s")
    )
  }
