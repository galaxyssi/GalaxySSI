...
struct AgentDesktopArtifactRequestPayload: Equatable {
  ...
  static func decode(_ raw: String) -> AgentDesktopArtifactRequestPayload? {
    ...
    return AgentDesktopArtifactRequestPayload(
      ...
      displayName: string(object, keys: ["display_name", "name"]).ifBlank("GalaxySSI-artifact"),
...
final class AgentDesktopArtifactStore {
  ...
  func resolveBlock(_ block: AgentRichBlock) -> AgentRichBlock {
    ...
    let safeURI = "galaxyssi-local-artifact://\(record.artifactId)"
    return AgentRichBlock(
      id: block.id,
      type: block.type,
      title: block.title,
  ...
  func saveArtifactUriToDownloads(sourceURI: String) throws -> String {
    try locked {
      ...
      let fileName = Self.safeFileName(record.name).ifBlank("GalaxySSI-artifact")
      guard let downloadsDirectory = fileManager.urls(for: .documentDirectory, in: .userDomainMask)
        .first?
        .appendingPathComponent("Downloads", isDirectory: true)
        .appendingPathComponent("GalaxySSI", isDirectory: true) else {
      ...
      return "Downloads/GalaxySSI/\(fileName)"
    }
  }
  ...
  static func isGalaxySSIArtifactURI(_ value: String) -> Bool {
    URLComponents(string: value)?.scheme?.lowercased() == "galaxyssi-artifact"
  }
...
private struct AgentDesktopArtifactChunkPayload {
  ...
  init(_ payload: [String: Any]) throws {
    ...
    guard AgentDesktopArtifactStore.isGalaxySSIArtifactURI(artifactURI) else {
      throw AgentDesktopArtifactStoreError.invalidPayload("Artifact URI must use galaxyssi-artifact")
    }
...
private struct AgentDesktopArtifactRecord: Codable, Equatable {
  ...
  init(from decoder: Decoder) throws {
    ...
    self.init(
      ...
      name: try container.decodeIfPresent(String.self, forKey: .name) ?? "GalaxySSI-artifact",
