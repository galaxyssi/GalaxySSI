...
enum GalaxySSIMessageActionPolicy {
...
struct GalaxySSIMessageActionsView: View {
        ...
        VStack(alignment: .leading, spacing: 12) {
          ...
          if !statusMessage.isEmpty {
            GalaxySSISecurityStatusRow(
              title: statusMessage,
              subtitle: t("galaxyssi.message.actions_status_detail", "Action status for this message"),
              systemImage: statusIsError ? "exclamationmark.circle" : "checkmark.circle",
              tint: statusIsError ? .orange : .galaxySSIAccent,
              badge: statusIsError
                ? t("galaxyssi.status.error", "Error")
  ...
  private var actionsSection: some View {
    VStack(alignment: .leading, spacing: 8) {
      GalaxySSISecuritySectionTitle(title: t("section_actions", "Actions"))
      ...
      if let remoteTask = remoteAgentTask, remoteTask.isCancellable {
        ...
        GalaxySSISecurityActionRow(
          ...
          subtitle: t("galaxyssi.message.cancel_task_subtitle", "Ask the connected Agent to stop this task"),
  ...
  private var detailsSection: some View {
    VStack(alignment: .leading, spacing: 8) {
      ...
      if let remoteTask = remoteAgentTask {
        GalaxySSISecurityStatusRow(
          title: t("galaxyssi.agent_task_details_title", "Agent task"),
          subtitle: remoteTask.taskId + remoteTaskStepSuffix(remoteTask),
          systemImage: "cpu",
          tint: GalaxySSIRemoteTaskStatusPresentation.tint(remoteTask.status),
          badge: GalaxySSIRemoteTaskStatusPresentation.title(
            remoteTask.status,
  ...
  private var identifierRows: some View {
    GalaxySSISecurityStatusRow(
      title: t("galaxyssi.identifier.message", "Message"),
    ...
    if !message.conversationId.isBlank {
      GalaxySSISecurityStatusRow(
        title: t("galaxyssi.identifier.conversation", "Conversation"),
    ...
    if !message.turnId.isBlank {
      GalaxySSISecurityStatusRow(
        title: t("galaxyssi.identifier.turn", "Turn"),
    ...
    if !message.remoteMessageId.isBlank {
      GalaxySSISecurityStatusRow(
        title: t("galaxyssi.identifier.remote_message", "Remote Message"),
  ...
  private func cancelRemoteAgentTask(_ task: AgentRemoteTaskStatusSnapshot) {
    ...
    statusMessage = t("galaxyssi.agent.remote_status.cancelling", "Sending cancellation...")
    statusIsError = false
    Task { @MainActor in
      let sent = await coordinator.cancelRemoteAgentTask(task)
      cancellingRemoteTaskIDs.remove(task.id)
      guard sent else {
        statusMessage = t("galaxyssi.agent.remote_status.cancel_failed", "The cancellation could not be sent.")
        statusIsError = true
        return
      }
      ...
      statusMessage = t("galaxyssi.agent.remote_status.cancel_sent", "Cancellation sent.")
    }
  }

  private var heroTint: Color {
    message.isMine ? .galaxySSIAccent : .blue
  }
  ...
  private var securityStatusText: String {
    ...
    case .link, .pcConnector:
      return t("message_security_status_subtitle", "Protected by the GalaxySSI Link end-to-end session")
    case .cloudAPI:
      return t(
        "galaxyssi.security.cloud",
  ...
  private var securityBadge: String {
    ...
    case .local:
      return t("galaxyssi.status.local", "Local")
    }
  }

  private var displayLocale: Locale {
    GalaxySSILocalization.dateLocale(language: interfaceLanguage)
  }
