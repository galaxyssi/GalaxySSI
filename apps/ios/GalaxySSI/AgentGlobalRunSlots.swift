...
enum AgentMentionCandidatePolicy {
  ...
  static func registration(for target: AgentCallableTarget, activeRuns: Int = 0) -> AgentRegistration {
    ...
    return AgentRegistration(
      ...
      providerId: profile?.providerId.ifBlank("galaxyssi-connectors") ?? "galaxyssi-connectors",
      ...
      connectionKind: target.kind == .model ? .http : .galaxyssiLink,
...
final class UserDefaultsAgentGlobalRunSlotStore: AgentGlobalRunSlotStoring {
  ...
  init(
    ...
    key: String = "galaxyssi_agent_global_run_slots_v1",
  ...
  private func loadPrunedLocked(now: Int64) -> AgentGlobalRunSlotLedger {
    ...
    if let data = GalaxySSIEncryptedUserDefaultsStore.load(defaults: defaults, key: key, secrets: secrets),
  ...
  static func ownerId(action: AgentAction, connectorId: String) -> String {
    ownerId(components: [
      action.parameters["_galaxyssi_conversation_id"] ?? "",
      action.parameters["_galaxyssi_turn_id"] ?? "",
      action.id,
      connectorId
    ])
  }
