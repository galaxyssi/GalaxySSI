...
@MainActor
final class GalaxySSIVoiceWakeController: ObservableObject {
  ...
  private let openWakeWord = GalaxySSIOpenWakeWordListener()
  ...
  private func startOpenWakeWordCapture(settings captureSettings: VoiceSettings, generation: Int) {
    Task { @MainActor [weak self] in
      ...
      let granted = await GalaxySSIOpenWakeWordListener.requestMicrophonePermission()
      guard generation == configurationGeneration,
            shouldListen,
