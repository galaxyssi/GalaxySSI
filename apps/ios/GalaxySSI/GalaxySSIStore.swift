  ...
  func setString(_ value: String, account: String) throws {
    ...
    if status == errSecItemNotFound {
      ...
      guard addStatus == errSecSuccess else {
        throw GalaxySSIError.invalidPayload("Unable to save secret in Keychain.")
      }
    } else if status != errSecSuccess {
      throw GalaxySSIError.invalidPayload("Unable to update secret in Keychain.")
    }
  }
...
final class InMemorySecretStore: GalaxySSISecretStore {
...
@MainActor
final class GalaxySSIStore: ObservableObject {
  @Published private(set) var profile: GalaxySSIProfile
  @Published private(set) var contacts: [GalaxySSIContact]
  @Published private(set) var friendRequests: [GalaxySSIFriendRequest]
  ...
  private struct PersistedState: Codable {
    var profile: GalaxySSIProfile
    var contacts: [GalaxySSIContact]
    var friendRequests: [GalaxySSIFriendRequest]
    ...
    init(
      profile: GalaxySSIProfile,
      contacts: [GalaxySSIContact],
      friendRequests: [GalaxySSIFriendRequest],
    ...
    init(from decoder: Decoder) throws {
      ...
      profile = try container.decode(GalaxySSIProfile.self, forKey: .profile)
      contacts = try container.decode([GalaxySSIContact].self, forKey: .contacts)
      friendRequests = try container.decodeIfPresent([GalaxySSIFriendRequest].self, forKey: .friendRequests) ?? []
  ...
  private let phoneContactCardsKey = "galaxyssi.phone_contact_cards"
  ...
  init(defaults: UserDefaults = .standard, secrets: GalaxySSISecretStore = KeychainSecretStore.shared) {
    defaults.removeObject(forKey: "galaxyssi.phone_contact_cards")
    defaults.removeObject(forKey: "galaxyssi.phone_contact_inbox_route")
    ...
    if let data = stateData,
      let state = try? JSONDecoder.galaxySSI.decode(PersistedState.self, from: data) {
      ...
      let shouldMigrateProfileName = GalaxySSIDeviceIdentityName.isLegacyDefault(restoredProfile.name)
      if shouldMigrateProfileName {
        restoredProfile.name = GalaxySSIDeviceIdentityName.current(profile: restoredProfile)
      }
      ...
      let generatedProfile = GalaxySSIStore.makeProfile(secrets: secrets, account: identityPrivateKeyAccount)
  ...
  var chatContacts: [GalaxySSIContact] {
  ...
  func visibleContacts(matching query: String) -> [GalaxySSIContact] {
    visibleContacts.filter { contactMatchesSearch($0, query: query) }
  }

  func chatContacts(matching query: String) -> [GalaxySSIContact] {
    chatContacts.filter { contactMatchesSearch($0, query: query) }
  }

  func contactList(matching query: String) -> [GalaxySSIContact] {
    contacts
      .filter { !$0.deleted && $0.id != "system" }
  ...
  var cloudModelContacts: [GalaxySSIContact] {
    contacts
      .filter { !$0.deleted && $0.deliveryMode == .cloudAPI }
      .sorted { $0.displayName.localizedCaseInsensitiveCompare($1.displayName) == .orderedAscending }
  }

  var pendingFriendRequests: [GalaxySSIFriendRequest] {
    friendRequests
      .filter { $0.status == .pending }
      .sorted { $0.createdAt > $1.createdAt }
  }

  func hasPendingFriendRequest(for galaxySSIId: String) -> Bool {
    friendRequests.contains {
      $0.galaxySSIId == galaxySSIId && $0.status == .pending
    }
  }
  ...
  func approvedIncomingPhoneContactIds() -> [String] {
    ...
    return friendRequests.compactMap { request in
      guard request.status == .approved,
            ...
            request.opaquePhoneRoutes != nil,
            !request.galaxySSIId.isEmpty,
            seen.insert(request.galaxySSIId).inserted else {
        return nil
      }
  ...
  var visibleFriendRequests: [GalaxySSIFriendRequest] {
    friendRequests
      .filter { request in
        let verified = contact(id: request.galaxySSIId)?.isCommunicable == true
        return GalaxySSIFriendRequestPresentationPolicy.isVisible(
          request,
  ...
  var unreadFriendRequestCount: Int {
    GalaxySSIFriendRequestUnreadPolicy.unreadCount(friendRequests)
  }
  ...
  func contact(id: String) -> GalaxySSIContact? {
    contacts.first { $0.id == id || $0.galaxySSIId == id }
  }
  ...
  func loadMessagePage(
    ...
    pageSize: Int = GalaxySSIChatHistoryDatabase.defaultPageSize
  ) -> GalaxySSIChatHistoryPage {
    let cursor = message.map {
      GalaxySSIChatHistoryCursor(
  ...
  func friendRequest(id: String) -> GalaxySSIFriendRequest? {
  ...
  func renameContact(id: String, displayName: String) -> Bool {
    ...
    guard !cleaned.isEmpty,
          let index = contacts.firstIndex(where: { $0.id == id || $0.galaxySSIId == id }) else {
      return false
    }
  ...
  func deleteContact(id: String, deleteMessages: Bool = false, now: Date = Date()) -> Bool {
    ...
    let deviceDesktopId = contacts.first { contact in
      (contact.id == id || contact.galaxySSIId == id) && contact.type == "device"
    ...
    for index in contacts.indices {
      ...
      guard contact.id == id || contact.galaxySSIId == id else { continue }
      deletedIds.insert(contact.id)
      deletedIds.insert(contact.galaxySSIId)
    ...
    for index in friendRequests.indices {
      ...
      guard deletedIds.contains(request.id) || deletedIds.contains(request.galaxySSIId) else { continue }
  ...
  func addCloudModelContact(
    ...
    guard !cleanModelId.isEmpty else {
      throw GalaxySSIError.invalidPayload("Cloud model ID is required.")
    }
    guard !cleanEndpoint.isEmpty else {
      throw GalaxySSIError.invalidPayload("Cloud endpoint is required.")
    }
    guard CloudModelCredentialPolicy.isStoredCredential(cleanAPIKey) else {
      throw GalaxySSIError.missingAPIKey
    }
    let now = Date()
    let providerSlug = GalaxySSIStore.slug(providerName)
    let contactId = "cloud:\(providerSlug)"
    let account = "cloud.\(providerSlug).\(GalaxySSIStore.slug(cleanModelId))"
    ...
    var contact = contacts.first { $0.id == contactId } ?? GalaxySSIContact(
      id: contactId,
      galaxySSIId: contactId,
      name: providerName,
  ...
  func myContactQRText(now: Date = Date()) throws -> String {
    try GalaxySSIContactExchange.makeContactQRText(profile: profile, serverLinks: serverLinks, now: now)
  }

  func createPhonePairingSession(now: Date = Date()) throws -> [String: String] {
    let token = try GalaxySSILinkProtocol.newLinkSecret()
    let secret = try GalaxySSILinkProtocol.newLinkSecret()
    let topic = GalaxySSILinkProtocol.pairingTopic(secret: secret)
  ...
  func activePhonePairingSessions(now: Date = Date()) -> [[String: String]] {
    ...
    let active = stored.filter { session in
      (Int64(session["expires_at"] ?? "") ?? 0) >= nowMillis &&
        GalaxySSILinkProtocol.validLinkSecret(session["secret"] ?? "") &&
        session["topic"] == GalaxySSILinkProtocol.pairingTopic(secret: session["secret"] ?? "")
    }
  ...
  func acceptPhoneControl(_ controlId: String, now: Date = Date()) -> Bool {
    ...
    accepted[controlId] = nowMillis + Double(GalaxySSIPhoneContactControl.maximumAgeMillis)
  ...
  func phoneOpaqueRoutes() -> [GalaxySSILinkRoutes] {
  ...
  func phoneOpaqueRoutes(for galaxySSIId: String) -> GalaxySSILinkRoutes? {
    contacts.first { $0.galaxySSIId == galaxySSIId }?.opaquePhoneRoutes
      ?? friendRequests.first { $0.galaxySSIId == galaxySSIId }?.opaquePhoneRoutes
  }
  ...
  func upsertOpaquePhoneRequest(
    ...
    direction: GalaxySSIFriendRequestDirection = .incoming,
    now: Date = Date()
  ) throws -> GalaxySSIFriendRequest {
    let data = try GalaxySSILinkProtocol.jsonData(card)
    guard let text = String(data: data, encoding: .utf8) else {
      throw GalaxySSIError.invalidPayload("Phone identity card is not UTF-8.")
    }
    var request = try GalaxySSIContactExchange.importContactQRCode(text, now: now)
    if let clientRouteId {
      request.linkClientRouteId = clientRouteId
    } else {
      request.linkClientRouteId = try GalaxySSILinkProtocol.newRouteId()
    }
  ...
  func rememberVerifiedPhoneContactCard(_ card: [String: Any]) {
    let galaxySSIId = card.string("galaxyssi_id")
    guard !galaxySSIId.isEmpty,
          !card.string("signature").isEmpty,
          (try? GalaxySSIContactExchange.validateSignedPhoneContactCard(card)) != nil,
          let data = try? GalaxySSILinkProtocol.jsonData(card),
    ...
    var cards = secrets.string(account: phoneContactCardsKey)
      .flatMap { Data(base64Encoded: $0) }
      .flatMap { try? JSONDecoder().decode([String: String].self, from: $0) } ?? [:]
    cards[galaxySSIId] = text
    if let encoded = try? JSONEncoder().encode(cards) {
      try? secrets.setString(encoded.base64EncodedString(), account: phoneContactCardsKey)
    }
  }

  func verifiedPhoneContactCard(for galaxySSIId: String) -> [String: Any]? {
    guard let cards = secrets.string(account: phoneContactCardsKey)
            .flatMap({ Data(base64Encoded: $0) })
            .flatMap({ try? JSONDecoder().decode([String: String].self, from: $0) }),
          let text = cards[galaxySSIId],
          let card = try? GalaxySSIQRCodePayload.decodeObject(from: text, label: "Contact QR"),
          card.string("galaxyssi_id") == galaxySSIId,
          !card.string("signature").isEmpty,
  ...
  func importContactQRCodeAsFriendRequest(
    ...
    identityBoundRoutes: GalaxySSILinkRoutes? = nil
  ) throws -> GalaxySSIFriendRequest {
    let rawCard = try GalaxySSIQRCodePayload.decodeObject(from: contents, label: "Contact QR")
    let card = GalaxySSIContactExchange.normalizeCompactPhoneContactQR(rawCard) ?? rawCard
    var request = try GalaxySSIContactExchange.importContactQRCode(contents, now: now)
    request.direction = .outgoing
    request.isRead = true
    if card.string("type") == GalaxySSIContactExchange.opaqueContactType {
      ...
      if let identityBoundRoutes, identityBoundRoutes.isOpaqueV2Valid {
        ...
        request.linkClientRouteId = try GalaxySSILinkProtocol.newRouteId()
        request.linkSecret = try GalaxySSILinkProtocol.deriveLinkSecret(
  ...
  func refreshTrustedPhoneRelationship(
    ...
    guard let index = contacts.firstIndex(where: {
            ($0.id == remoteId || $0.galaxySSIId == remoteId) &&
              !$0.deleted && $0.trustState == .verified
          }),
          let refreshed = GalaxySSIPhoneRelationshipRouteRefresh.apply(
  ...
  private func normalizeVerifiedPhoneRelationshipRoutes(now: Date = Date()) -> Bool {
    guard GalaxySSISignalEngine.isAvailable else { return false }
    let signalEngine = GalaxySSISignalEngine(
      profileName: profile.galaxySSIId,
      defaults: defaults,
      secrets: secrets
    )
    var changed = false
    for index in contacts.indices {
      let contact = contacts[index]
      let remoteId = contact.galaxySSIId.ifBlank(contact.id)
      guard remoteId.hasPrefix("galaxyssi:"),
  ...
  @discardableResult
  func addFriendRequest(_ request: GalaxySSIFriendRequest, now: Date = Date()) -> GalaxySSIFriendRequest {
    let existingContact = contact(id: request.galaxySSIId)
    let wasDeleted = existingContact?.deleted == true || existingContact?.trustState == .deleted
    var stored = request
    let previous = friendRequests.first { $0.galaxySSIId == stored.galaxySSIId }
    ...
    stored.isRead = GalaxySSIFriendRequestUnreadPolicy.isReadForPendingRequest(
      previous: previous,
    ...
    if let index = friendRequests.firstIndex(where: { $0.galaxySSIId == stored.galaxySSIId }) {
  ...
  func approveFriendRequest(id: String, now: Date = Date()) -> Bool {
    ...
    let contactId = request.type == "hermes" ? "hermes" : request.galaxySSIId
    ...
    var next = contact(id: contactId) ?? GalaxySSIContact(
      id: contactId,
      galaxySSIId: request.galaxySSIId,
      name: request.name,
    ...
    next.galaxySSIId = request.galaxySSIId
    ...
    if request.opaquePhoneRoutes != nil {
      ...
      let content = String(
        format: GalaxySSILocalization.string(
          "galaxyssi.phone_contact.added_notice",
  ...
  @discardableResult
  func approveFriendRequest(galaxySSIId: String, now: Date = Date()) -> Bool {
  ...
  private func approvedDeliveryMode(
    for request: GalaxySSIFriendRequest,
    type: String
  ) -> GalaxySSIDeliveryMode {
  ...
  @discardableResult
  func rejectFriendRequest(galaxySSIId: String, now: Date = Date()) -> Bool {
    guard let request = friendRequests.first(where: {
      $0.galaxySSIId == galaxySSIId && $0.status == .pending
    }) else { return false }
  ...
  func exportBackupPayload(includeContacts: Bool = true, includeMessages: Bool = true) -> GalaxySSIBackupPayload {
    ...
    return GalaxySSIBackupPayload(
      ...
      agentData: GalaxySSIBackupAgentData(
  ...
  func restoreBackupPayload(_ payload: GalaxySSIBackupPayload, includeMessages: Bool = true) throws {
  ...
  func deleteCloudModel(contactId: String, modelId: String) -> Bool {
    guard let index = contacts.firstIndex(where: { $0.id == contactId || $0.galaxySSIId == contactId }),
  ...
  func addServerLink(
    ...
    var hermes = contact(id: "hermes") ?? GalaxySSIContact.hermes()
  ...
  func markServerPaired(desktopId: String, access: PairingAccess? = nil) {
    ...
    if var hermes = contact(id: "hermes") {
      ...
      hermes.setupDetail = "GalaxySSI Link is paired"
      hermes.updatedAt = Date()
      upsert(hermes)
    }
    for contactIndex in contacts.indices {
      ...
      contacts[contactIndex].setupDetail = "GalaxySSI Link is paired"
  ...
  func updateDesktopDeviceMetadata(
    ...
    guard !cleanDesktopId.isEmpty,
          let metadata = GalaxySSIDesktopDeviceMetadata.from(payload: payload, now: now),
          let index = serverLinks.firstIndex(where: { $0.desktopId == cleanDesktopId }) else {
      return false
    }
    let merged = (serverLinks[index].deviceMetadata ?? GalaxySSIDesktopDeviceMetadata(lastSeenAt: now))
  ...
  func updatePairedDesktopDevice(from payload: [String: Any], link: ServerLink? = nil) -> Bool {
    ...
    let defaultName = (GalaxySSIDesktopDeviceMetadata.from(payload: payload)?.hostName ?? "")
      .ifBlank(payload.string("desktop_display_name"))
      .ifBlank(device.string("display_name"))
      .ifBlank(payload.string("desktop_name"))
      .ifBlank(link?.desktopName ?? "GalaxySSI Desktop")
    ...
    var contact = existingContact ?? GalaxySSIContact(
      id: desktopId,
      galaxySSIId: desktopId,
      name: defaultName,
      ...
      setupStatus: "ready",
      setupDetail: "GalaxySSI Link is paired",
      cloudProvider: "",
      cloudModels: [],
      selectedCloudModelId: "",
      deleted: false,
    ...
    contact.devicePlatform = GalaxySSIDesktopDeviceMetadata.from(payload: payload)?.platform.nonEmpty
    ...
    contact.deviceHostName = GalaxySSIDesktopDeviceMetadata.from(payload: payload)?.hostName.nonEmpty
    contact.setupStatus = "ready"
    contact.setupDetail = "GalaxySSI Link is paired"
  ...
  private func deviceDefaultDisplayNameKey(_ desktopId: String) -> String {
    "galaxyssi.device.default_display_name.\(desktopId)"
  }
  ...
  func removeServer(desktopId: String) {
    ...
    if var hermes = contact(id: "hermes") {
      if let activeLink = serverLinks.first(where: \.paired) ?? serverLinks.first {
        ...
        hermes.setupDetail = activeLink.paired ? "GalaxySSI Link is paired" : "Waiting for desktop confirmation"
      } else {
        hermes.trustState = .unverified
        hermes.desktopId = ""
        hermes.desktopName = ""
        hermes.setupStatus = "needs_pairing"
        hermes.setupDetail = "Waiting for GalaxySSI Desktop pairing"
      }
      hermes.updatedAt = Date()
      upsert(hermes)
    }
  ...
  func removeDesktopPairing(
    ...
    let matchingContacts = contacts.filter { contact in
      contact.id != "system" && contact.id != "hermes" && (
        ...
        contact.galaxySSIId == cleanDesktopId ||
        contact.galaxySSIId.hasPrefix(prefix)
      )
    }
    var removedIds = Set(
      matchingContacts
        .flatMap { [$0.id, $0.galaxySSIId] }
        .filter { !$0.isEmpty }
    )
    ...
    for index in friendRequests.indices where removedIds.contains(friendRequests[index].id) ||
      removedIds.contains(friendRequests[index].galaxySSIId) {
      friendRequests[index].status = .deleted
      friendRequests[index].deletedAt = now
      friendRequests[index].readdRequired = true
    }
  ...
  func updateDesktopAgentContacts(from payload: [String: Any], link incomingLink: ServerLink? = nil) -> Int {
    guard let source = GalaxySSIContactExchange.connectorAgentSource(from: payload) else {
      return 0
    }
  ...
  func importDesktopAgentQRCodeAsContacts(_ contents: String) throws -> Int {
    let object = try GalaxySSIQRCodePayload.decodeObject(from: contents, label: "Agent QR")
    if let source = GalaxySSIContactExchange.connectorAgentSource(from: object) {
    ...
    guard let payload = singleDesktopAgentPayload(from: object, link: link) else {
      throw GalaxySSIError.invalidPayload("Unsupported Agent QR code.")
    }
  ...
  private func upsertDesktopAgentContacts(payloads: [[String: Any]], link incomingLink: ServerLink?) -> Int {
    ...
    payloads.forEach { payload in
      ...
      let desktopName = (GalaxySSIDesktopDeviceMetadata.from(payload: payload)?.hostName ?? "")
        .ifBlank(payload.string("desktop_name"))
      ...
      let setupDetail = payload.string("detail")
        ...
        .ifBlank(
          isPaired ? "GalaxySSI Link is paired" : "Waiting for GalaxySSI Desktop status"
        )
      let setupNextStep = payload.string("setup_next_step")
        .ifBlank(payload.string("setup"))
      let desktopAccessProfile = payload.string("desktop_access_profile")
        .ifBlank((link?.accessProfile ?? "").ifBlank(GalaxySSILinkProtocol.accessRestricted))
      ...
      let now = Date()
      var contact = contact(id: contactId) ?? GalaxySSIContact(
        id: contactId,
        galaxySSIId: contactId,
        name: displayName,
  ...
  private func desktopAgentPayloads(from pairing: PairingQRCode, link: ServerLink) -> [[String: Any]] {
    ...
    return fallbackAgents.map { agent in
      ...
      let payload: [String: Any] = [
        ...
        "detail": link.paired ? "GalaxySSI Link is paired" : "Waiting for GalaxySSI Desktop status",
        "setup": "",
  ...
  private func desktopAgentPayloads(
    ...
    let access = GalaxySSILinkProtocol.pairingAccess(from: parentPayload.dictionary("pairing_access"))
  ...
  private func singleDesktopAgentPayload(from object: [String: Any], link: ServerLink?) -> [String: Any]? {
    ...
    inheritConnectorValue("desktop_name", into: &payload, value: link?.desktopName ?? "GalaxySSI Desktop")
  ...
  private func approvedContactSetupDetail(for request: GalaxySSIFriendRequest) -> String {
    if !request.setupDetail.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
      return request.setupDetail
    }
    switch request.type {
    case "agent":
      return request.mqttInboxTopic.isEmpty ? "Verified from Agent QR" : "GalaxySSI Agent QR verified"
    case "device":
      return request.mqttInboxTopic.isEmpty ? "Verified from device QR" : "GalaxySSI device QR verified"
    case "hermes":
      return "Hermes identity verified from QR"
    default:
      return request.mqttInboxTopic.isEmpty ? "Verified from contact QR" : "GalaxySSI contact QR verified"
    }
  }
  ...
  private func resetToFreshState() {
    ...
    UserDefaultsAgentSelfModelStore(defaults: defaults, secrets: secrets).clear()
    profile = GalaxySSIStore.makeProfile(secrets: secrets, account: identityPrivateKeyAccount)
    contacts = [GalaxySSIContact.hermes(), GalaxySSIContact.system()]
    friendRequests = []
    messagesByContact = GalaxySSIStore.defaultMessages()
    _ = chatHistoryDatabase.replaceAll(messagesByContact)
  ...
  private func upsert(_ contact: GalaxySSIContact) {
    if let index = contacts.firstIndex(where: { $0.id == contact.id || $0.galaxySSIId == contact.galaxySSIId }) {
  ...
  private func contactMatchesSearch(_ contact: GalaxySSIContact, query: String) -> Bool {
  ...
  private static func agentConversationDatabaseURL(defaults: UserDefaults) -> URL {
    let key = "galaxyssi_agent_conversation_database_id_v1"
  ...
  private func loadPersistedState() -> PersistedState? {
    GalaxySSIEncryptedStateStore.load(defaults: defaults, secrets: secrets)
      .flatMap { try? JSONDecoder.galaxySSI.decode(PersistedState.self, from: $0) }
  }
  ...
  private func runtimeMessageSnapshot(for contactId: String) -> [ChatMessage] {
    ...
    return chatHistoryDatabase.page(
      contactId: contactId,
      pageSize: GalaxySSIChatHistoryDatabase.maximumPageSize
    ).messages
  }
  ...
  private static func makeProfile(secrets: GalaxySSISecretStore, account: String) -> GalaxySSIProfile {
    ...
    return GalaxySSIProfile(
      galaxySSIId: signalId,
      name: GalaxySSIDeviceIdentityName.current(galaxySSIId: signalId),
