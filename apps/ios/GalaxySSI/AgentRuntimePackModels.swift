...
enum AgentRuntimeDistributionSources {
  static let githubCatalogURL =
    "https://github.com/galaxyssi/GalaxySSI/releases/download/ios-runtime-v1/ios-runtime-catalog-v1.json"
  ...
  static func downloadCandidates(url: String, languageTag: String) -> [String] {
    guard isChinese(languageTag), isGalaxySSIGitHubReleaseURL(url) else {
      return [url]
    }
    var seen = Set<String>()
  ...
  private static func isGalaxySSIGitHubReleaseURL(_ value: String) -> Bool {
    guard let components = URLComponents(string: value) else {
      return false
    }
    return components.scheme?.lowercased() == "https" &&
      components.host?.lowercased() == "github.com" &&
      components.path.hasPrefix(galaxySSIReleasePath)
  }

  private static let galaxySSIReleasePath = "/galaxyssi/GalaxySSI/releases/download/"
