...
enum GalaxySSITrustState: String, Codable, CaseIterable {
  case unverified
  case verified
  case deleted
}

enum GalaxySSIDeliveryMode: String, Codable, CaseIterable {
  case local
  case link
  case cloudAPI
  case pcConnector = "pc_connector"

  var isGalaxySSILinkFamily: Bool {
    switch self {
  ...
  static func fromWireValue(_ value: String?) -> GalaxySSIDeliveryMode {
...
enum GalaxySSICloudAPIStyle: String, Codable, CaseIterable, Identifiable {
...
struct GalaxySSIProfile: Codable, Equatable {
...
struct GalaxySSIContact: Codable, Identifiable, Equatable, Hashable {
  var id: String
  var galaxySSIId: String
  var name: String
  ...
  var deliveryMode: GalaxySSIDeliveryMode
  var trustState: GalaxySSITrustState
  ...
  var connectorAgentId: String {
    ...
    let rawIds = [id, galaxySSIId].map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }
    for rawId in rawIds where !rawId.isEmpty {
      ...
      if deliveryMode.isGalaxySSILinkFamily, rawId.contains(":") {
  ...
  static func hermes() -> GalaxySSIContact {
    GalaxySSIContact(
      id: "hermes",
      galaxySSIId: "hermes",
      name: "Hermes Agent",
      ...
      setupDetail: "Waiting for GalaxySSI Desktop pairing",
      cloudProvider: "",
      cloudModels: [],
  ...
  static func system() -> GalaxySSIContact {
    GalaxySSIContact(
      id: "system",
      galaxySSIId: "system",
      name: "System",
...
extension GalaxySSIContact {
  var opaquePhoneRoutes: GalaxySSILinkRoutes? {
...
extension GalaxySSIFriendRequest {
  var opaquePhoneRoutes: GalaxySSILinkRoutes? {
    let routes = GalaxySSILinkRoutes(
...
enum GalaxySSIFriendRequestStatus: String, Codable, CaseIterable {
  case pending
  case approved
  case rejected
  case deleted
...
enum GalaxySSIFriendRequestDirection: String, Codable, CaseIterable {
  case incoming
  case outgoing
}

struct GalaxySSIFriendRequest: Codable, Identifiable, Equatable, Hashable {
  var id: String
  var galaxySSIId: String
  var name: String
  var type: String
  ...
  var source: String
  var direction: GalaxySSIFriendRequestDirection
  var isRead: Bool
  var status: GalaxySSIFriendRequestStatus
  var createdAt: Date
  var approvedAt: Date?
  var rejectedAt: Date?
  var deletedAt: Date?
  ...
  init(
    ...
    isRead: Bool = false,
    status: GalaxySSIFriendRequestStatus = .pending,
    createdAt: Date = Date(),
    approvedAt: Date? = nil,
    rejectedAt: Date? = nil,
    deletedAt: Date? = nil,
  ...
  init(from decoder: Decoder) throws {
    ...
    galaxySSIId = try container.decodeIfPresent(String.self, forKey: .galaxySSIId) ?? ""
    ...
    direction = try container.decodeIfPresent(GalaxySSIFriendRequestDirection.self, forKey: .direction) ?? .incoming
    isRead = try container.decodeIfPresent(Bool.self, forKey: .isRead) ?? (direction == .outgoing)
    status = try container.decodeIfPresent(GalaxySSIFriendRequestStatus.self, forKey: .status) ?? .pending
...
struct GalaxySSILinkRoutes: Codable, Equatable, Hashable {
  ...
  var receiveWindow: Set<String> {
    GalaxySSILinkProtocol.topicWindow(
      linkSecret: linkSecret,
      senderFingerprint: remoteFingerprint,
      receiverFingerprint: localFingerprint
    )
...
struct PairingAccess: Codable, Equatable, Hashable {
  ...
  var fullDesktopExecutor: Bool {
    profile == GalaxySSILinkProtocol.accessDesktopExecutor &&
      scopes.contains(GalaxySSILinkProtocol.scopeDesktopExecutor)
  }
}
...
struct ServerLink: Codable, Identifiable, Equatable, Hashable {
  ...
  init(
    ...
    deviceMetadata: GalaxySSIDesktopDeviceMetadata? = nil,
    updatedAt: Date
  ) {
    self.desktopId = desktopId
    self.desktopName = desktopName
  ...
  var fullDesktopExecutor: Bool {
    accessProfile == GalaxySSILinkProtocol.accessDesktopExecutor &&
      accessScopes.contains(GalaxySSILinkProtocol.scopeDesktopExecutor)
  }
  ...
  init(from decoder: Decoder) throws {
    ...
    self.init(
      ...
      routes: try container.decode(GalaxySSILinkRoutes.self, forKey: .routes),
      ...
      deviceMetadata: try container.decodeIfPresent(GalaxySSIDesktopDeviceMetadata.self, forKey: .deviceMetadata),
...
struct VoiceSettings: Codable, Equatable {
  ...
  static let defaultWelcomeText = "I am here. Welcome to GalaxySSI. Say your question or task."
