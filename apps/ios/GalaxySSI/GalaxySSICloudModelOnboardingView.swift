...
struct CloudModelProviderSelectionView: View {
  ...
  var onModelAdded: ((GalaxySSIContact) -> Void)? = nil
    ...
    VStack(spacing: 0) {
      ...
      ScrollView {
        VStack(alignment: .leading, spacing: 12) {
          CloudModelHeroView(
            title: t("galaxyssi.cloud.select_provider", "Select Provider"),
            subtitle: t(
              "galaxyssi.cloud.provider_hero_subtitle",
            ...
            badge: t("galaxyssi.cloud.direct", "Direct")
          )
  ...
  private func providerCount(_ provider: String) -> String {
    String(
      format: t("galaxyssi.cloud.provider_count", "%d"),
  ...
  private func providerSubtitle(_ provider: String) -> String {
    switch provider {
    case "OpenAI":
      return t("galaxyssi.cloud.provider.openai.subtitle", "GPT-5.5, GPT-5.4 mini/nano, and other OpenAI API models")
    case "Anthropic":
      return t("galaxyssi.cloud.provider.anthropic.subtitle", "Claude Opus, Sonnet, Haiku; uses Messages API")
    case "Google Gemini":
      return t("galaxyssi.cloud.provider.gemini.subtitle", "Gemini Pro / Flash / Flash Lite; direct Google API from the phone")
    case "DeepSeek":
      return t("galaxyssi.cloud.provider.deepseek.subtitle", "DeepSeek V4 Pro, V4 Flash, and V4 Flash Vision")
    case "Qwen":
      return t("galaxyssi.cloud.provider.qwen.subtitle", "Qwen 3.7 Max/Plus, Qwen 3.6 Flash; DashScope OpenAI compatible")
    case "OpenRouter":
      return t("galaxyssi.cloud.provider.openrouter.subtitle", "Unified access to multiple cloud models with custom routing")
    default:
      return t("galaxyssi.cloud.provider.custom.subtitle", "OpenAI-compatible custom endpoint")
    }
  }
...
struct CloudModelPickerView: View {
  ...
  private var rows: [CloudModelPreset] {
    ...
    return presets + [
      CloudModelPreset(
        provider: provider,
        name: t("galaxyssi.cloud.custom_model_id", "Custom Model ID"),
        modelId: "model-id",
    ...
    VStack(spacing: 0) {
      ...
      ScrollView {
        VStack(alignment: .leading, spacing: 12) {
          ...
          sectionTitle(t("galaxyssi.cloud.section_model", "Model"))
          VStack(spacing: 8) {
            ForEach(rows) { preset in
              CloudModelNavigationRow(
                ...
                tint: providerTint(provider),
                badge: t("galaxyssi.common.select", "Select")
              ) {
  ...
  private func subtitle(for preset: CloudModelPreset) -> String {
    if preset.modelId == "model-id" && provider != "Custom" {
      return String(
        format: t("galaxyssi.cloud.custom_model_subtitle", "Use another or newly released model from %@"),
        provider
      )
    }
...
struct CloudModelConfigurationView: View {
  ...
  init(
    ...
    onModelAdded: ((GalaxySSIContact) -> Void)? = nil
  ) {
    self.preset = preset
  ...
  var body: some View {
    VStack(spacing: 0) {
      GalaxySSITopBar(
        title: t("galaxyssi.cloud.config_title", "Configure Model"),
        leading: {
          GalaxySSIBackButton()
        },
        trailing: {
      ...
      ScrollView {
        VStack(alignment: .leading, spacing: 12) {
          ...
          sectionTitle(t("galaxyssi.cloud.section_contact", "Contact"))
          CloudModelTextFieldRow(
            title: t("galaxyssi.cloud.contact_name", "Contact Name"),
            text: $displayName
          )
          sectionTitle("Provider")
          CloudModelValueRow(
          ...
          sectionTitle(t("galaxyssi.cloud.section_model", "Model"))
          CloudModelTextFieldRow(
            title: t("galaxyssi.cloud.model_id", "Model ID"),
            text: $modelId,
          ...
          sectionTitle(t("galaxyssi.cloud.section_key", "Key"))
          ...
          Button {
            ...
            Text(t("galaxyssi.cloud.save_start_chat", "Save and Start Chat"))
              .font(.system(size: 17, weight: .semibold))
              .foregroundColor(.white)
              .frame(maxWidth: .infinity, minHeight: 46)
              .background(canSave ? Color.galaxySSIAccent : Color.galaxySSITextSecondary)
...
private enum CloudModelOnboardingCopy {
  static func providerSubtitle(_ provider: String, language: String) -> String {
    switch provider {
    case "OpenAI":
      return t("galaxyssi.cloud.provider.openai.subtitle", "GPT-5.5, GPT-5.4 mini/nano, and other OpenAI API models", language: language)
    case "Anthropic":
      return t("galaxyssi.cloud.provider.anthropic.subtitle", "Claude Opus, Sonnet, Haiku; uses Messages API", language: language)
    case "Google Gemini":
      return t("galaxyssi.cloud.provider.gemini.subtitle", "Gemini Pro / Flash / Flash Lite; direct Google API from the phone", language: language)
    case "DeepSeek":
      return t("galaxyssi.cloud.provider.deepseek.subtitle", "DeepSeek V4 Pro, V4 Flash", language: language)
    case "Qwen":
      return t("galaxyssi.cloud.provider.qwen.subtitle", "Qwen 3.7 Max/Plus, Qwen 3.6 Flash; DashScope OpenAI compatible", language: language)
    case "OpenRouter":
      return t("galaxyssi.cloud.provider.openrouter.subtitle", "Unified access to multiple cloud models with custom routing", language: language)
    default:
      return t("galaxyssi.cloud.provider.custom.subtitle", "OpenAI-compatible custom endpoint", language: language)
    }
  }
