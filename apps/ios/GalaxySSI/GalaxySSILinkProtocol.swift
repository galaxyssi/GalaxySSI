...
enum GalaxySSILinkProtocol {
  static let name = "galaxyssi-link"
  static let version = 2
  static let accessContract = "galaxyssi.pairing-access/1.0"
  ...
  static func newRouteId() throws -> String {
    ...
    guard status == errSecSuccess else {
      throw GalaxySSIError.invalidPayload("Unable to create a secure route ID.")
    }
  ...
  static func deriveLinkSecret(
    ...
    guard let secretData = Data(base64URLEncoded: pairingSecret), secretData.count == 32 else {
      throw GalaxySSIError.invalidPayload("Pairing secret is malformed.")
    }
    let identities = [firstFingerprint, secondFingerprint].sorted()
    guard identities.allSatisfy({ !$0.isEmpty }) else {
      throw GalaxySSIError.invalidPayload("Both identity fingerprints are required.")
    }
  ...
  static func deriveIdentityBoundLinkSecret(
    ...
    guard sharedSecret.count >= 32 else {
      throw GalaxySSIError.invalidPayload("Signal identity agreement is too short.")
    }
    let binding = try canonicalPhoneIdentityBinding(firstFingerprint, secondFingerprint)
    let label = Data(("galaxyssi-phone-link-v3\0" + binding).utf8)
  ...
  static func deriveIdentityBoundRouteId(
    ...
    guard let secret = Data(base64URLEncoded: linkSecret), secret.count == 32 else {
      throw GalaxySSIError.invalidPayload("Phone link secret is malformed.")
    }
    let binding = try canonicalPhoneIdentityBinding(firstFingerprint, secondFingerprint)
    let label = Data(("galaxyssi-phone-route-v3\0" + binding).utf8)
    let code = HMAC<SHA256>.authenticationCode(
      for: label,
  ...
  private static func canonicalPhoneIdentityBinding(
    ...
    guard identities.allSatisfy({
      ...
      throw GalaxySSIError.invalidPayload("Phone relationship identities are invalid.")
    }
  ...
  static func sealWirePacket(_ payload: Data, secret: String) throws -> Data {
    guard let secretData = Data(base64URLEncoded: secret), secretData.count == 32 else {
      throw GalaxySSIError.invalidPayload("Link secret is malformed.")
    }
    ...
    guard encoded.count <= maxOpaquePacketBytes else {
      throw GalaxySSIError.invalidPayload("Opaque packet exceeds broker limit.")
    }
    return encoded
  }
  ...
  static func openWirePacket(_ wire: Data, secret: String) throws -> Data {
    guard let secretData = Data(base64URLEncoded: secret), secretData.count == 32,
      ...
      throw GalaxySSIError.invalidPayload("Opaque packet is malformed or truncated.")
    }
    ...
    guard plaintext.count >= 5, plaintext[plaintext.startIndex] == UInt8(version) else {
      throw GalaxySSIError.invalidPayload("Unsupported opaque packet version.")
    }
    ...
    guard payloadSize <= plaintext.count - 5 else {
      throw GalaxySSIError.invalidPayload("Invalid opaque packet length.")
    }
  ...
  static func decodePairingQRCode(from text: String, now: Date = Date()) throws -> PairingQRCode {
    let object = try GalaxySSIQRCodePayload.decodeObject(from: text, label: "Pairing QR")
    guard let normalized = normalizePairingQRCode(object) else {
      throw GalaxySSIError.invalidPairingQRCode("Unsupported GalaxySSI pairing QR format.")
    }
  ...
  static func normalizePairingQRCode(_ source: [String: Any]) -> [String: Any]? {
    ...
    let desktopName = source.string("n").ifBlank("GalaxySSI Desktop")
  ...
  static func validatePairingQRCode(_ qr: [String: Any], now: Date = Date()) throws -> PairingQRCode {
    guard qr.string("type") == "opaque_pairing" else {
      throw GalaxySSIError.invalidPairingQRCode("type must be opaque_pairing.")
    }
    guard qr.int("version") == version else {
      throw GalaxySSIError.invalidPairingQRCode("protocol version mismatch.")
    }
    ...
    guard validLinkSecret(pairingSecretString), !expectedTopic.isEmpty else {
      throw GalaxySSIError.invalidPairingQRCode("pairing secret must be 32 bytes.")
    }
    guard qr.string("pairing_topic") == expectedTopic else {
      throw GalaxySSIError.invalidPairingQRCode("pairing topic does not match secret.")
    }
    guard qr.string("pairing_token").count >= 32 else {
      throw GalaxySSIError.invalidPairingQRCode("pairing token is too short.")
    }
    ...
    guard !qr.string("desktop_id").isEmpty else {
      throw GalaxySSIError.invalidPairingQRCode("desktop ID is missing.")
    }
    guard qr.string("identity_key_sha256").count == 64 else {
      throw GalaxySSIError.invalidPairingQRCode("desktop identity fingerprint is malformed.")
    }
    guard let access = pairingAccess(from: qr.dictionary("pairing_access")) else {
      throw GalaxySSIError.invalidPairingQRCode("pairing access grant is invalid.")
    }
    ...
    guard createdAtMilliseconds > 0,
      ...
      throw GalaxySSIError.invalidPairingQRCode("pairing QR has expired.")
    }
    ...
    return PairingQRCode(
      ...
      desktopName: GalaxySSIDesktopDeviceMetadata.displayName(
        from: qr,
        fallback: "GalaxySSI Desktop"
      ),
  ...
  static func makeEnvelope(
    ...
    if let content = payload["content"] as? String,
      ...
      throw GalaxySSIError.invalidPayload("text exceeds Link limit.")
    }
    ...
    guard try jsonData(envelope).count <= maxEnvelopeBytes else {
      throw GalaxySSIError.invalidPayload("envelope exceeds Link limit.")
    }
  ...
  static func jsonData(_ object: Any) throws -> Data {
    guard JSONSerialization.isValidJSONObject(object) else {
      throw GalaxySSIError.invalidPayload("object is not JSON encodable.")
    }
