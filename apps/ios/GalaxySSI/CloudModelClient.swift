...
struct CloudModelClient {
  ...
  private func sendOpenAICompatibleAttempt(
    ...
    request.httpBody = try GalaxySSILinkProtocol.jsonData([
      "model": model.modelId,
      "messages": messages,
      "stream": false
    ])
  ...
  func jsonRequest(url: String, apiKey: String) throws -> URLRequest {
    ...
    if url.localizedCaseInsensitiveContains("openrouter.ai") {
      request.setValue("https://galaxyssi.local", forHTTPHeaderField: "HTTP-Referer")
      request.setValue("GalaxySSI", forHTTPHeaderField: "X-Title")
    }
    return request
  }
  ...
  func responseObject(for request: URLRequest, throwHTTPFailure: Bool = false) async throws -> [String: Any] {
    ...
    if let http = response as? HTTPURLResponse, !(200...299).contains(http.statusCode) {
      ...
      if CloudContextOverflowPolicy.isContextOverflow(statusCode: http.statusCode, responseBody: body) {
        throw GalaxySSIError.invalidPayload("Cloud request exceeded the model context window. Try a shorter chat history or smaller attachment.")
      }
      throw GalaxySSIError.invalidPayload("Cloud request failed with \(http.statusCode): \(body.prefix(240))")
    }
  ...
  private static var baseSystemPrompt: String {
    "You are GalaxySSI, a private superintelligence interface. Be concise, useful, and preserve user privacy. When a response benefits from structure, use clear sections, tables, or code blocks."
  }
}
