...
struct GalaxySSIGlobalAgentControlView: View {
  ...
  private var snapshot: GalaxySSIGlobalAgentDashboardSnapshot {
  ...
  var body: some View {
    VStack(spacing: 0) {
      GalaxySSITopBar(
        ...
        trailing: {
          GalaxySSIAndroidIconButton(systemName: "arrow.clockwise", action: refreshRuntime)
        }
      )
      ScrollView {
        VStack(alignment: .leading, spacing: 12) {
          ...
          obsidianSection
          privacySection
          Text(t("cc_global_footer", "Private conversations are never added to the global world model. External side effects still follow GalaxySSI confirmation policy."))
            .font(.system(size: 12))
  ...
  private var heroSection: some View {
    ...
    return VStack(alignment: .leading, spacing: 12) {
        ...
        VStack(alignment: .leading, spacing: 6) {
          HStack(spacing: 8) {
            ...
            GalaxySSIGlobalAgentBadge(
              ...
              tint: current.settings.enabled ? .galaxySSIAccent : .orange
            )
          }
          ...
          HStack(spacing: 8) {
            ...
            GalaxySSIGlobalAgentBadge(
              ...
              tint: current.settings.allowCloudCognition ? .purple : .galaxySSIAccent
            )
          }
        }
        Spacer(minLength: 0)
      ...
      HStack(spacing: 8) {
        GalaxySSIGlobalAgentMetricView(
      ...
      if current.unresolvedConflictCount > 0 {
        NavigationLink(
          destination: GalaxySSIGlobalAgentDetailView(kind: .conflicts, snapshot: current)
        ) {
          GalaxySSIGlobalAgentPlainRow(
  ...
  private var intelligenceSection: some View {
    ...
    return section(t("cc_global_section_intelligence", "Intelligence acquisition")) {
      ...
      navigationRow(
        ...
        systemImage: "arrow.triangle.2.circlepath",
        tint: current.continuityQuarantinedCount > 0 ? .orange : .galaxySSIAccent
      )
  ...
  private var obsidianSection: some View {
    section(t("cc_obsidian_section", "Obsidian knowledge projection")) {
      GalaxySSISecurityActionRow(
        ...
        subtitle: t("cc_obsidian_vault_subtitle", "Select a folder for reviewed GalaxySSI knowledge and Agent conversations"),
      ...
      GalaxySSISecurityNavigationRow(
        ...
        GalaxySSIObsidianCandidatesView(onChange: refreshRuntime)
      }
      .disabled(!obsidianSettings.enabled)
      GalaxySSISecurityActionRow(
  ...
  private func navigationRow(
    kind: GalaxySSIGlobalAgentDetailKind,
    snapshot: GalaxySSIGlobalAgentDashboardSnapshot,
    title: String,
    subtitle: String,
    systemImage: String,
    tint: Color
    ...
    if kind == .insights {
      return AnyView(
        GalaxySSISecurityNavigationRow(
          ...
          GalaxySSIGlobalAgentInsightInboxView()
        }
      )
    }
    if kind == .runs {
    ...
    return AnyView(
      GalaxySSISecurityNavigationRow(
        ...
        GalaxySSIGlobalAgentDetailView(kind: kind, snapshot: snapshot)
      }
    )
  }
  ...
  private func loopNavigationRow(
    ...
    NavigationLink(destination: GalaxySSIGlobalAgentDetailView(kind: kind, snapshot: snapshot)) {
      GalaxySSIGlobalAgentPlainRow(
        title: title,
        subtitle: subtitle,
        systemImage: systemImage,
  ...
  private func processNow() {
    guard store.globalAgentSettings.enabled else {
      statusMessage = t("galaxyssi.status.paused", "Paused")
      return
    }
...
struct GalaxySSIGlobalAgentInsightInboxView: View {
  ...
  var body: some View {
    VStack(spacing: 0) {
      GalaxySSITopBar(
        title: t("agent_global_insights_title", "GalaxySSI insights"),
        leading: { GalaxySSIBackButton() },
      ...
      ScrollView {
        VStack(alignment: .leading, spacing: 12) {
          if !statusMessage.isEmpty {
            GalaxySSISecurityStatusRow(
              title: t("agent_global_feedback_saved", "Feedback saved. GalaxySSI will adapt future insights."),
              subtitle: statusMessage,
              systemImage: "checkmark.circle",
              tint: .galaxySSIAccent,
              badge: t("galaxyssi.status.ready", "Ready")
          ...
          if items.isEmpty {
            GalaxySSIGlobalAgentPlainRow(
              title: t("agent_global_insights_empty", "No recent GalaxySSI insights."),
              subtitle: t("cc_global_pending_insights_subtitle", "Findings waiting for the right delivery moment"),
              systemImage: "sparkles",
              tint: .purple,
              badge: "0",
  ...
  private func sourceLabel(_ item: GlobalProactiveInboxItem) -> String {
    ...
    let source = store.agentSession(id: item.sourceConversationId)?.title
      ...
      .ifBlank("GalaxySSI") ?? item.topic.ifBlank(item.sourceConversationId).ifBlank("GalaxySSI")
  ...
  private func recordFeedback(_ item: GlobalProactiveInboxItem, _ kind: GlobalAgentFeedbackKind) {
    if store.recordGlobalInsightFeedback(inboxItem: item, kind: kind) {
      statusMessage = t("agent_global_feedback_saved", "Feedback saved. GalaxySSI will adapt future insights.")
    } else {
...
private struct GalaxySSIGlobalInsightCard: View {
        ...
        VStack(alignment: .leading, spacing: 4) {
          HStack(spacing: 8) {
            Text(item.title.ifBlank(targetLabel))
              ...
              .lineLimit(2)
            if item.isNew {
              GalaxySSIGlobalAgentBadge(text: "NEW", tint: .purple)
            }
            if let feedbackLabel {
              GalaxySSIGlobalAgentBadge(text: feedbackLabel, tint: .galaxySSIAccent)
            }
          }
          Text(sourceLabel)
      ...
      LazyVGrid(columns: feedbackColumns, alignment: .leading, spacing: 8) {
        if canOpenTopic {
          Button {
            ...
            GalaxySSIGlobalFeedbackChip(title: openTitle, emphasized: false)
          }
        }
        Button { onFeedback(item, .helpful) } label: {
          GalaxySSIGlobalFeedbackChip(title: helpfulTitle, emphasized: item.feedbackKind == .helpful)
        }
        Button { onFeedback(item, .notRelevant) } label: {
          GalaxySSIGlobalFeedbackChip(title: notRelevantTitle, emphasized: item.feedbackKind == .notRelevant)
        }
        Button { onFeedback(item, .tooFrequent) } label: {
          GalaxySSIGlobalFeedbackChip(title: tooFrequentTitle, emphasized: item.feedbackKind == .tooFrequent)
        }
      }
      .buttonStyle(.plain)
    }
...
private struct GalaxySSIGlobalFeedbackChip: View {
  var title: String
  var emphasized: Bool
  ...
  var body: some View {
    Text(title)
      ...
      .foregroundColor(emphasized ? .white : .galaxySSITextPrimary)
      .lineLimit(1)
      .minimumScaleFactor(0.8)
      .padding(.horizontal, 9)
      .padding(.vertical, 7)
      .background(emphasized ? Color.galaxySSIAccent : Color.galaxySSIButtonSoft)
      .cornerRadius(7)
  }
}

struct GalaxySSIGlobalAgentDetailView: View {
  @Environment(\.galaxySSIInterfaceLanguage) private var interfaceLanguage
  var kind: GalaxySSIGlobalAgentDetailKind
  var snapshot: GalaxySSIGlobalAgentDashboardSnapshot
  ...
  private var rows: [GalaxySSIGlobalAgentRowItem] {
  ...
  private var tint: Color {
    ...
    case .links, .runs, .continuity: return .galaxySSIAccent
...
private struct GalaxySSIGlobalAgentMetricView: View {
  var title: String
  var value: String
...
struct GalaxySSIGlobalAgentBadge: View {
  var text: String
  var tint: Color
...
private struct GalaxySSIGlobalAgentToggleRow: View {
  var title: String
  var subtitle: String
  var systemImage: String
  var tint: Color
  ...
  var body: some View {
    HStack(spacing: 12) {
      ZStack {
        ...
        Image(systemName: systemImage)
          .font(.system(size: 18, weight: .semibold))
          .foregroundColor(isEnabled ? tint : .galaxySSITextSecondary)
      }
      .frame(width: 42, height: 42)
      VStack(alignment: .leading, spacing: 3) {
        Text(title)
          .font(.system(size: 16, weight: .semibold))
          .foregroundColor(isEnabled ? .galaxySSITextPrimary : .galaxySSITextSecondary)
          .lineLimit(1)
...
private struct GalaxySSIGlobalAgentPlainRow: View {
  var title: String
  var subtitle: String
  var systemImage: String
  var tint: Color
...
private extension GalaxySSIGlobalAgentTone {
  var color: Color {
    switch self {
    ...
    case .neutral: return .galaxySSITextSecondary
    }
  }
}
