...
struct GalaxySSIDesktopDeviceMetadata: Codable, Equatable, Hashable {
  ...
  static func from(payload: [String: Any], now: Date = Date()) -> GalaxySSIDesktopDeviceMetadata? {
    let device = payload.dictionary("desktop_device") ?? [:]
    let metadata = GalaxySSIDesktopDeviceMetadata(
  ...
  func merged(with newer: GalaxySSIDesktopDeviceMetadata) -> GalaxySSIDesktopDeviceMetadata {
    GalaxySSIDesktopDeviceMetadata(
