...
@MainActor
enum GalaxySSINavigationContentPrewarm {
  private static var cached: Snapshot?

  struct Snapshot {
    var key: String
    var hub: GalaxySSIConversationHubPreparedContent
    var modelSelection: GalaxySSIAgentModelSelectionPreparedContent
    var settings: GalaxySSISettingsSummarySnapshot
  }

  static func prepare(store: GalaxySSIStore) async {
    ...
    let settings = await GalaxySSISettingsSummaryCache.prepare(store: store)
    let prepared = await Task.detached(priority: .utility) {
      let hub = GalaxySSIConversationHubPreparedContent(
        ...
        contacts: GalaxySSIConversationHubModels.contacts(contacts, query: "")
      )
  ...
  static func snapshot(for store: GalaxySSIStore) -> Snapshot? {
    guard let cached, cached.key == sourceKey(for: store) else { return nil }
    return cached
  }

  private static func sourceKey(for store: GalaxySSIStore) -> String {
    ...
    return "\(store.activeAgentConversationId)|\(conversations)|\(contacts)|\(localProfiles)|\(cloudModels)|\(callableTargets)|\(GalaxySSISettingsSummaryCache.key(for: store))"
  }
}
