...
struct GalaxySSIConversationComposer: View {
  @Binding var draft: String
  @Binding var attachments: [GalaxySSIDraftAttachment]
  ...
  @StateObject private var peerVoiceRecorder = GalaxySSIPeerVoiceMessageRecorder()
  ...
  private var inputShell: some View {
    GalaxySSIGrowingComposerEditor(
      text: $draft,
      placeholder: t("galaxyssi.message.input_hint", "Message or hold to talk..."),
      focus: $inputFocused,
  ...
  private var voiceCaptureSurface: some View {
    VStack(spacing: 12) {
      ...
      Text(voiceCapturePending
        ? t("galaxyssi.voice.preparing_title", "Preparing voice input")
        ...
        if dedicatedPeerVoiceCapture {
          peerVoiceRecorder.dragChanged(
            ...
            onFinish: { recording in
              onVoiceTranscript(GalaxySSIVoiceTranscriptSubmission(
                text: "",
...
private struct GalaxySSIChatVoiceTranscript: View {
...
private struct GalaxySSIChatVoiceWaveform: View {
  var phase: Double
  var amplitude: Double
  var cancelPending: Bool
  var color: Color = .galaxySSIAccent
