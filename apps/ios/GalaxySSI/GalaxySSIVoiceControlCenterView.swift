...
struct GalaxySSIVoiceControlCenterView: View {
      ...
      ScrollView {
        VStack(alignment: .leading, spacing: 12) {
          GalaxySSISecurityHeroView(
            ...
            tint: settings.wakeListeningEnabled ? .galaxySSIAccent : .galaxySSIInsightText,
  ...
  private func capabilityLabel(_ capability: VoiceProviderCapability) -> String {
    ...
    case .unavailable:
      return t("galaxyssi.status.not_available", "Unavailable")
    }
  }
...
private struct GalaxySSIVoiceMetric: View {
  var value: String
  var label: String
  var tint: Color
...
private struct GalaxySSIVoiceToggleRow: View {
  var title: String
  var subtitle: String
    ...
    Button(action: action) {
      HStack(spacing: 12) {
        ...
        Image(systemName: isOn ? "checkmark.circle.fill" : "circle")
          ...
          .foregroundColor(isOn ? .galaxySSIAccent : .galaxySSITextSecondary)
      }
      .padding(12)
      .background(Color.galaxySSISurface)
      .clipShape(RoundedRectangle(cornerRadius: 8, style: .continuous))
