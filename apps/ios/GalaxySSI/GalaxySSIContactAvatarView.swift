...
struct AvatarView: View {
  ...
  var body: some View {
    ZStack {
      ...
      } else if GalaxySSIContactAvatarPolicy.usesGeneratedIdenticon(for: contact) {
        GalaxySSIIdenticonView(
          pattern: GalaxySSIIdenticon.fromIdentityFingerprint(identityIdenticonSeed)
        )
  ...
  private var assetName: String? {
    ...
    if contact.deliveryMode == .cloudAPI {
      if let providerAssetName = GalaxySSIAgentAvatarAssetCatalog.cloudProviderAssetName(for: identityFields) {
        return providerAssetName
      }
    } else if let agentAssetName = GalaxySSIAgentAvatarAssetCatalog.assetName(for: identityFields) {
      return agentAssetName
    }

    let identity = identityFields.joined(separator: " ").lowercased()
    if contact.deliveryMode.isGalaxySSILinkFamily || identity.contains("galaxyssi") {
      return "GalaxySSILogo"
    }
    return nil
  }
...
enum GalaxySSIContactAvatarPolicy {
  ...
  static func usesGeneratedIdenticon(for contact: GalaxySSIContact) -> Bool {
...
struct GalaxySSIIdenticonPattern: Equatable {
  ...
  var color: Color {
    GalaxySSIIdenticon.color(at: colorIndex)
  }
...
enum GalaxySSIIdenticon {
  ...
  static func fromIdentityFingerprint(_ fingerprint: String) -> GalaxySSIIdenticonPattern {
    let normalized = fingerprint.trimmingCharacters(in: .whitespacesAndNewlines)
      .lowercased()
      .ifBlank("galaxyssi-local-identity")
    let digest = Array(SHA256.hash(data: Data(normalized.utf8)))
    var cells = [Bool](repeating: false, count: GalaxySSIIdenticonPattern.gridSize * GalaxySSIIdenticonPattern.gridSize)
    for row in 0..<GalaxySSIIdenticonPattern.gridSize {
      for sourceColumn in 0...2 {
        let sourceIndex = row * 3 + sourceColumn
        let filled = digest[sourceIndex / 8] & (1 << (sourceIndex % 8)) != 0
        cells[row * GalaxySSIIdenticonPattern.gridSize + sourceColumn] = filled
        cells[row * GalaxySSIIdenticonPattern.gridSize + (4 - sourceColumn)] = filled
      }
    }
...
struct GalaxySSIIdenticonView: View {
  var pattern: GalaxySSIIdenticonPattern
  ...
  var body: some View {
    Canvas { context, size in
      ...
      let cellSize = (side - gridInset * 2) / CGFloat(GalaxySSIIdenticonPattern.gridSize)
      for row in 0..<GalaxySSIIdenticonPattern.gridSize {
        for column in 0..<GalaxySSIIdenticonPattern.gridSize where pattern.isFilled(row: row, column: column) {
          let rect = CGRect(
