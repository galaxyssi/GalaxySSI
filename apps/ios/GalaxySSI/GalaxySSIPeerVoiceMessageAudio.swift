...
enum GalaxySSIPeerVoiceMessageAudio {
...
struct GalaxySSIPeerVoiceRecording: Equatable {
  var data: Data
...
struct GalaxySSIPeerMessageAttachmentStore {
  ...
  func persistOutgoingVoice(
    ...
    let destination = directory.appendingPathComponent(
      "msg_\(identity).\(normalizedExtension).\(GalaxySSIAttachmentAtRestCipher.containerExtension)",
      isDirectory: false
    )
    if let sourceURL,
    ...
    if let sourceURL, fileSize(sourceURL) > 0 {
      ...
      throw GalaxySSIError.invalidPayload("Voice recording is unavailable.")
    }
    guard !plaintext.isEmpty else {
      throw GalaxySSIError.invalidPayload("Voice message copy is incomplete.")
    }
  ...
  private func outgoingVoiceURL(identity: String, fileExtension: String) -> URL {
    rootURL
      ...
      .appendingPathComponent(
        "msg_\(identity).\(fileExtension).\(GalaxySSIAttachmentAtRestCipher.containerExtension)",
        isDirectory: false
      )
  }
  ...
  private func parseVoiceIdentity(_ value: String) -> (id: String, extension: String)? {
    ...
    if name.pathExtension.lowercased() == GalaxySSIAttachmentAtRestCipher.containerExtension {
  ...
  private func normalizedIdentity(_ value: String) throws -> String {
    ...
    guard !normalized.isEmpty,
      ...
      throw GalaxySSIError.invalidPayload("Voice message identity is invalid.")
    }
    return normalized
  }
...
@MainActor
final class GalaxySSIPeerVoiceMessageRecorder: NSObject, ObservableObject {
  ...
  private var recorder: GalaxySSIPeerVoicePCMRecorder?
  private var holdTask: Task<Void, Never>?
  private var timer: Timer?
  private var startedAt: Date?
  private var touchActive = false
  private var messages = GalaxySSIAgentHoldToTalkMessages.default
  private var onFinish: ((GalaxySSIPeerVoiceRecording) -> Void)?
  ...
  func dragChanged(
    translation: CGSize,
    messages: GalaxySSIAgentHoldToTalkMessages,
    onFinish: @escaping (GalaxySSIPeerVoiceRecording) -> Void,
  ...
  private func requestPermissionAndStart() async {
    ...
    do {
      let recorder = GalaxySSIPeerVoicePCMRecorder()
      try recorder.start()
  ...
  private func finish(send: Bool, status: String) {
    ...
    do {
      ...
      releaseRecorder()
      completion?(GalaxySSIPeerVoiceRecording(
        data: encoded.data,
  ...
  private func tick() {
    ...
    if elapsed >= GalaxySSIPeerVoiceMessageAudio.maximumDuration {
      finish(send: true, status: "")
    }
  }
