...
actor GalaxySSIOpenWakeWordListener {
  ...
  private var detector: GalaxySSIOpenWakeWordDetector?
  ...
  func start(
    ...
    do {
      ...
      let detector = try GalaxySSIOpenWakeWordDetector(modelName: modelName)
      let engine = AVAudioEngine()
      let input = engine.inputNode
      let hardwareFormat = input.inputFormat(forBus: 0)
      guard hardwareFormat.sampleRate > 0 else {
        throw GalaxySSIOpenWakeWordError.unsupportedAudioFormat(0)
      }
      guard let targetFormat = AVAudioFormat(
        commonFormat: .pcmFormatInt16,
        sampleRate: GalaxySSIOpenWakeWordConstants.sampleRate,
        channels: 1,
        interleaved: true
      ), let converter = AVAudioConverter(from: hardwareFormat, to: targetFormat) else {
        throw GalaxySSIOpenWakeWordError.unsupportedAudioFormat(hardwareFormat.sampleRate)
      }
      ...
      ringBuffer = [Int16](
        repeating: 0,
        count: Int(GalaxySSIOpenWakeWordConstants.sampleRate * windowSeconds)
      )
      writeIndex = 0
      samplesWritten = 0
