...
enum GalaxySSIAttachmentAtRestError: Error, Equatable {
...
final class GalaxySSIAttachmentAtRestCipher {
  static let shared = GalaxySSIAttachmentAtRestCipher()
  ...
  func encrypt(_ plaintext: Data, purpose: String) throws -> Data {
    ...
    let headerData = try JSONEncoder.galaxySSIAttachment.encode(header)
    guard headerData.count <= Self.maximumHeaderBytes else {
      throw GalaxySSIAttachmentAtRestError.invalidContainer
    }
    ...
    guard let combined = sealed.combined else {
      throw GalaxySSIAttachmentAtRestError.writeFailed
    }
  ...
  func decrypt(_ container: Data, expectedPurpose: String? = nil) throws -> Data {
    ...
    if let expectedPurpose,
      ...
      throw GalaxySSIAttachmentAtRestError.authenticationFailed
    }
    let plaintext: Data
    do {
    ...
    } catch let error as GalaxySSIAttachmentAtRestError {
      throw error
    } catch {
      throw GalaxySSIAttachmentAtRestError.authenticationFailed
    }
    guard Int64(plaintext.count) == parsed.header.plaintextSize,
      ...
      throw GalaxySSIAttachmentAtRestError.integrityFailed
    }
    return plaintext
  }
  ...
  private func parse(_ container: Data) throws -> (
    ...
    guard let header = try? JSONDecoder().decode(Header.self, from: headerData),
      ...
      throw GalaxySSIAttachmentAtRestError.unsupportedVersion
    }
    return (
      header,
  ...
  private func encryptionKey(createIfMissing: Bool) throws -> SymmetricKey {
    ...
    guard createIfMissing else {
      throw GalaxySSIAttachmentAtRestError.keyUnavailable
    }
...
private extension JSONEncoder {
  static var galaxySSIAttachment: JSONEncoder {
