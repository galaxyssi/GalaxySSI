...
struct GalaxySSIAgentHoldToTalkMessages {
  ...
  static let `default` = GalaxySSIAgentHoldToTalkMessages(
...
@MainActor
final class GalaxySSIAgentHoldToTalkController: ObservableObject {
  ...
  private var lastMessages = GalaxySSIAgentHoldToTalkMessages.default
  ...
  private var onFinishedTranscript: ((GalaxySSIVoiceTranscriptSubmission) -> Void)?
  ...
  func dragChanged(
    ...
    onFinish: @escaping (GalaxySSIVoiceTranscriptSubmission) -> Void,
  ...
  private func deliver(_ value: String) -> Bool {
    ...
    onFinishedTranscript?(GalaxySSIVoiceTranscriptSubmission(
      text: cleanText,
