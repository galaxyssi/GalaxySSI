...
struct GalaxySSIAgentScreenDetailRow: Identifiable, Equatable {
  var id: String
  var title: String
  var detail: String
...
struct GalaxySSIAgentScreenDetailSection: Identifiable, Equatable {
  var id: String
  var title: String
  var rows: [GalaxySSIAgentScreenDetailRow]

  init(id: String, title: String, rows: [GalaxySSIAgentScreenDetailRow]) {
    self.id = id
    self.title = title
...
struct GalaxySSIAgentScreenContextSnapshot: Equatable {
  var screen: AgentScreenContext
  var sections: [GalaxySSIAgentScreenDetailSection]
}

enum GalaxySSIAgentScreenContextSnapshotBuilder {
  static func make(
    ...
    let screen = AgentScreenContext(
      ...
      pageTitle: t("galaxyssi.tab.agent", "Agent"),
    ...
    guard screenObservationAllowed else {
      return GalaxySSIAgentScreenContextSnapshot(screen: screen, sections: [])
    }
    return GalaxySSIAgentScreenContextSnapshot(
      screen: screen,
  ...
  private static func detailSections(
    visibleTexts: [String],
    actionRows: [GalaxySSIAgentScreenDetailRow],
    inputRows: [GalaxySSIAgentScreenDetailRow],
    scrollRows: [GalaxySSIAgentScreenDetailRow],
    launchRows: [GalaxySSIAgentScreenDetailRow],
    notifications: AgentNotificationContext,
    clipboard: AgentClipboardContext,
    deviceStatus: AgentDeviceStatusContext,
    t: (String, String) -> String
  ) -> [GalaxySSIAgentScreenDetailSection] {
    let visibleRows = visibleTexts.prefix(8).enumerated().map { index, value in
      GalaxySSIAgentScreenDetailRow(
        id: "visible-\(index)",
        title: value,
  ...
  private static func notificationRows(
    ...
    guard !notifications.items.isEmpty else {
      return [
        GalaxySSIAgentScreenDetailRow(
          ...
          title: t("agent_screen_notifications_empty_ios", "No GalaxySSI notifications available"),
          systemImage: "bell",
          isNotice: true
        )
      ]
    ...
    return notifications.items.prefix(3).enumerated().map { index, item in
      ...
      let detail = replyAvailable
        ...
        : baseDetail
      return GalaxySSIAgentScreenDetailRow(
        ...
        command: sensitive
          ? nil
          : replyAvailable
            ? "reply notification \(item.packageName.ifBlank("GalaxySSI")) :: "
            : "read notifications",
        isNotice: sensitive
      )
    }
  ...
  private static func elementInventory(
    _ rows: [GalaxySSIAgentScreenDetailRow],
    className: String,
  ...
  private static func visibleTextValues(
    ...
    var values: [String] = [
      "GalaxySSI",
      t("galaxyssi.agent.brand.subtitle", "Super Agent"),
      t("galaxyssi.agent.session.new", "New session"),
      unreadTotal > 0 ? String(format: t("galaxyssi.agent.unread", "%d unread"), unreadTotal) : t("galaxyssi.agent.tab.subtitle", "Phone-native super agent"),
      t("agent_section_screen_details", "Screen Details"),
      t("galaxyssi.agent.section.process", "Process"),
      t("galaxyssi.agent.step.observe", "Read current screen structure"),
      t("galaxyssi.agent.step.analyze", "Analyze user goal"),
      t("galaxyssi.agent.step.plan", "Generate executable plan"),
      t("galaxyssi.agent.step.act", "Execute after confirmation"),
      t("galaxyssi.agent.section.info", "Info"),
      String(format: t("galaxyssi.agent.current_app", "Current App: %@"), "GalaxySSI iOS")
    ]
  ...
  private static func actionRows(t: (String, String) -> String) -> [GalaxySSIAgentScreenDetailRow] {
    [
      commandRow(
        id: "new-session",
  ...
  private static func inputRows(t: (String, String) -> String) -> [GalaxySSIAgentScreenDetailRow] {
    [
      commandRow(
        id: "agent-goal-input",
        title: t("agent_screen_field_agent_goal", "Agent goal input"),
  ...
  private static func scrollRows(t: (String, String) -> String) -> [GalaxySSIAgentScreenDetailRow] {
    [
      commandRow(
        id: "agent-transcript",
  ...
  private static func launchRows(t: (String, String) -> String) -> [GalaxySSIAgentScreenDetailRow] {
    [
      launchRow(id: "agent", title: t("galaxyssi.tab.agent", "Agent"), command: "open Agent", t: t),
      launchRow(id: "messages", title: t("galaxyssi.agent_sessions.title", "Sessions"), command: "open Messages", t: t),
      launchRow(id: "contacts", title: t("galaxyssi.tab.contacts", "Contacts"), command: "open Contacts", t: t),
      launchRow(id: "discover", title: t("galaxyssi.tab.discover", "Discover"), command: "open Discover", t: t),
      launchRow(id: "settings", title: t("galaxyssi.tab.settings", "Settings"), command: "open Settings", t: t)
    ]
  }
  ...
  private static func launchRow(
    ...
    GalaxySSIAgentScreenDetailRow(
      id: "launch-\(id)",
      title: title,
      detail: String(format: t("agent_screen_launchable_app_summary_ios", "%@ / open"), "GalaxySSI"),
      systemImage: "app",
      command: command
    )
  }
...
struct GalaxySSIAgentScreenContextCard: View {
  var screen: AgentScreenContext
  var sections: [GalaxySSIAgentScreenDetailSection]
  ...
  private var pageTitle: String {
    screen.pageTitle.ifBlank(screen.foregroundApp).ifBlank("GalaxySSI iOS")
  }
  ...
  private var filteredSections: [GalaxySSIAgentScreenDetailSection] {
    sections.compactMap { section in
      let rows = section.rows.filter { row in
        matches(section.title) || matches(row.searchText)
      }
      guard !rows.isEmpty else { return nil }
      return GalaxySSIAgentScreenDetailSection(id: section.id, title: section.title, rows: Array(rows.prefix(8)))
    }
  }
  ...
  private func screenSection(_ section: GalaxySSIAgentScreenDetailSection) -> some View {
  ...
  @ViewBuilder
  private func screenRow(_ row: GalaxySSIAgentScreenDetailRow) -> some View {
  ...
  private func rowContent(_ row: GalaxySSIAgentScreenDetailRow, commandable: Bool) -> some View {
    HStack(alignment: .top, spacing: 10) {
      Image(systemName: row.systemImage)
        .font(.system(size: 15, weight: .semibold))
        .foregroundColor(row.isNotice ? .galaxySSITextSecondary : .galaxySSIAccent)
...
struct GalaxySSIAgentScreenContextDetailView: View {
  var screen: AgentScreenContext
  var sections: [GalaxySSIAgentScreenDetailSection]
  var onCommand: (String) -> Void
  var t: (String, String) -> String
