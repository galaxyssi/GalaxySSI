...
final class GalaxySSIChatHistoryDatabaseTests: XCTestCase {
  func testEncryptedHistoryPagesWithoutDroppingMessages() throws {
    let root = FileManager.default.temporaryDirectory
      .appendingPathComponent("GalaxySSIChatHistoryDatabaseTests-\(UUID().uuidString)", isDirectory: true)
    let file = root.appendingPathComponent("history.sqlite")
    defer { try? FileManager.default.removeItem(at: root) }
    let database = GalaxySSIChatHistoryDatabase(fileURL: file, secrets: InMemorySecretStore())
    ...
    var cursor: GalaxySSIChatHistoryCursor?
  ...
  func testConversationQueriesUpdatesDeletesAndUnreadSummary() throws {
    let root = FileManager.default.temporaryDirectory
      .appendingPathComponent("GalaxySSIChatHistoryDatabaseTests-\(UUID().uuidString)", isDirectory: true)
    defer { try? FileManager.default.removeItem(at: root) }
    let database = GalaxySSIChatHistoryDatabase(
  ...
  func testBatchDeleteRemovesOnlyRequestedConversationMessages() {
    let root = FileManager.default.temporaryDirectory
      .appendingPathComponent("GalaxySSIChatHistoryDeleteTests-\(UUID().uuidString)", isDirectory: true)
    defer { try? FileManager.default.removeItem(at: root) }
    let database = GalaxySSIChatHistoryDatabase(
      fileURL: root.appendingPathComponent("history.sqlite"),
      secrets: InMemorySecretStore()
