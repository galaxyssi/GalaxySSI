...
extension GalaxySSIStoreTests {
  ...
  func testAgentConversationContextTransportKeepsAttachmentReferenceWithoutPrivateBytes() throws {
    let richOutput = """
    {"version":1,"blocks":[{"id":"image-1","type":"image","title":"homework.jpg","uri":"content://galaxyssi/private/homework.jpg","data_b64":"private-image-bytes","mime_type":"image/jpeg","metadata":{"size_bytes":"245760"}}]}
    """
    let context = AgentConversationContext(
    ...
    XCTAssertFalse(transport.contains("content://galaxyssi/private"))
  ...
  func testPhoneRuntimeRequiresExplicitCodeOrProjectOperation() {
    ...
    XCTAssertTrue(AgentPhoneRuntimePolicy.shouldUsePhoneRuntime(
      goal: "Continue https://github.com/galaxyssi/GalaxySSI on this phone"
    ))
    XCTAssertTrue(AgentPhoneRuntimePolicy.shouldUsePhoneRuntime(
  ...
  func testUnifiedCommandResultUsesAndroidWireNames() throws {
    ...
    let encoded = String(decoding: try JSONEncoder.galaxySSI.encode(result), as: UTF8.self)
  ...
  func testCodexStyleResponsePolicyDropsInputArtifactsButKeepsGeneratedFiles() throws {
    let raw = richDocument([
      [
        ...
        "type": "file",
        "title": "test.xlsx",
        "uri": "galaxyssi-artifact://task/downloads/input/01-test.xlsx"
      ],
      [
        "id": "output",
        "type": "file",
        "title": "summary.csv",
        "uri": "galaxyssi-artifact://task/outputs/summary.csv"
      ]
    ])
  ...
  func testAppendDeliveryTraceUpdatesStatusAndKeepsPriorStages() {
    ...
    XCTAssertEqual(updated?.deliveryTrace.last?.detail, "galaxyssi/topic")
  }
  ...
  func testDestroyAllPrivateDataRegeneratesIdentityAndClearsSecrets() throws {
    ...
    let originalGalaxySSIId = store.profile.galaxySSIId
    ...
    XCTAssertNotEqual(store.profile.galaxySSIId, originalGalaxySSIId)
  ...
  func testCloudModelCredentialPolicyRejectsPlaceholders() {
    ...
    XCTAssertFalse(CloudModelCredentialPolicy.isAutoRoutableCredential("sk-galaxyssi-smoke-key"))
  ...
  func testAgentConnectorAvailabilityMatchesAndroidCloudModelReadiness() {
    ...
    XCTAssertFalse(
      AgentConnectorAvailability.cloudModelReady(model: complete, apiKey: "sk-galaxyssi-smoke-key", provider: "deepseek")
    )
    XCTAssertFalse(
      AgentConnectorAvailability.cloudModelReady(
    ...
    var contact = GalaxySSIContact.system()
