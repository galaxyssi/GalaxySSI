...
extension GalaxySSIStoreTests {
  ...
  func testAgentExecutionPresentationPolicyMatchesAndroidLocalAndRemoteLocations() {
    ...
    XCTAssertEqual(phone.executorLabel, "GalaxySSI")
  ...
  func testAgentExecutionPresentationPolicyResolvesRouteActionAndTaskRecordLocations() {
    ...
    let linux = AgentExecutionPresentationPolicy.location(
      route: AgentRoute(kind: .localSystem, targetTitle: "GalaxySSI"),
  ...
  func testAgentExecutionPresentationPolicyDecodesAndroidWireNames() throws {
    let phase = try JSONDecoder.galaxySSI.decode(AgentPhase.self, from: Data(#""WAITING_RESPONSE""#.utf8))
    let route = try JSONDecoder.galaxySSI.decode(AgentRouteKind.self, from: Data(#""DESKTOP_AGENT""#.utf8))
    let location = try JSONDecoder.galaxySSI.decode(AgentExecutionLocationKind.self, from: Data(#""connected_device""#.utf8))
    let runtime = try JSONDecoder.galaxySSI.decode(AgentExecutionRuntimeKind.self, from: Data(#""PHONE_LINUX""#.utf8))
    let fallbackPhase = try JSONDecoder.galaxySSI.decode(AgentPhase.self, from: Data(#""not-supported""#.utf8))
    let fallbackRoute = try JSONDecoder.galaxySSI.decode(AgentRouteKind.self, from: Data(#""not-supported""#.utf8))
    let fallbackLocation = try JSONDecoder.galaxySSI.decode(AgentExecutionLocationKind.self, from: Data(#""not-supported""#.utf8))
    let fallbackRuntime = try JSONDecoder.galaxySSI.decode(AgentExecutionRuntimeKind.self, from: Data(#""not-supported""#.utf8))
  ...
  func testAgentNativeToolLifecycleRunControlAdapterMatchesAndroidTimelinePayload() {
    ...
    XCTAssertEqual(event.payload["tool_id"]?.stringValue, "galaxyssi.workspace.file.read.text")
  ...
  func testAgentRunRecoveryModelsUseAndroidWireNames() throws {
    ...
    let decodedRegistration = try JSONDecoder().decode(
      ...
      from: Data(#"{"agent_id":"codex","location":"TRUSTED_DESKTOP","connection_kind":"GALAXYSSI_LINK"}"#.utf8)
    )
    ...
    XCTAssertEqual(decodedRegistration.connectionKind, .galaxyssiLink)
    ...
    XCTAssertTrue(encodedRegistration.contains(#""connection_kind":"GALAXYSSI_LINK""#))
  ...
  func testAgentExplicitToolHandleIsOpaqueScopedAndDoesNotExposeResource() throws {
    ...
    let publicJSON = String(decoding: try JSONEncoder.galaxySSI.encode(opened), as: UTF8.self)
    now = 1_250
  ...
  func testAgentExplicitToolHandleModelsUseAndroidWireNames() throws {
    let scope = try JSONDecoder.galaxySSI.decode(
    ...
    let recordJSON = String(decoding: try JSONEncoder.galaxySSI.encode(record), as: UTF8.self)
    let statusJSON = String(decoding: try JSONEncoder.galaxySSI.encode(status), as: UTF8.self)
  ...
  func testAgentPrivateDataInventoryModelsUseAndroidWireNames() throws {
    let descriptor = try JSONDecoder.galaxySSI.decode(
    ...
    let fallbackPolicy = try JSONDecoder.galaxySSI.decode(
      AgentPrivateDataExportPolicy.self,
      from: Data(#""future""#.utf8)
    )
    let encoded = String(
      decoding: try JSONEncoder.galaxySSI.encode(
  ...
  func testAgentConnectorTimingPolicyMatchesAndroidAttachmentDeadlinesAndWireNames() throws {
    ...
    let encoded = try JSONEncoder.galaxySSI.encode(attachment)
    ...
    let stage = try JSONDecoder.galaxySSI.decode(
      AgentConnectorTimeoutStage.self,
      from: Data(#""READ_ONLY_STALE""#.utf8)
    )
    let resource = try JSONDecoder.galaxySSI.decode(
  ...
  func testGlobalProactiveInboxModelsUseAndroidWireNames() throws {
    ...
    XCTAssertEqual(legacy?.title, "GalaxySSI \u{5efa}\u{8bae}")
