...
final class AgentIOSCoreMemoryTests: XCTestCase {
  func testExtractsExplicitIdentityDeviceAndProjectFacts() throws {
    let input = "\u{6211}\u{7684}\u{540D}\u{5B57}\u{662F}\u{9648}\u{661F}\u{FF0C}\u{6211}\u{7684}\u{624B}\u{673A}\u{662F} Galaxy S26 Ultra\u{FF0C}\u{5F53}\u{524D}\u{9879}\u{76EE}\u{662F} GalaxySSI\u{3002}"
    ...
    XCTAssertEqual(candidates.first { $0.key == AgentIOSCoreMemoryExtractor.currentProjectKey }?.value, "The user's current project is GalaxySSI.")
  }
  ...
  func testAcceptsExpandedExplicitIdentityDeviceAndProjectPhrases() {
    ...
    XCTAssertEqual(
      AgentIOSCoreMemoryExtractor.extract("I am working on GalaxySSI")
        .first { $0.key == AgentIOSCoreMemoryExtractor.currentProjectKey }?.value,
      "The user's current project is GalaxySSI."
    )
    XCTAssertEqual(
      AgentIOSCoreMemoryExtractor.extract("\u{6211}\u{5728}\u{505a}\u{7684}\u{9879}\u{76ee}\u{662f} GalaxySSI")
        .first { $0.key == AgentIOSCoreMemoryExtractor.currentProjectKey }?.value,
      "The user's current project is GalaxySSI."
    )
  }
  ...
  func testPreservesCanonicalKeySeparatorsAndMigratesLegacyCoreKeys() {
    ...
    let store = InMemoryAgentMemoryStore(items: [
      ...
      AgentMemoryItem(kind: .task, value: "The user's current project is GalaxySSI.", key: "coreprojectcurrent"),
