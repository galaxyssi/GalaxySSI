...
final class VoiceInteractionCoordinatorTests: XCTestCase {
  ...
  func testVoiceFeatureFlagPersistsCoordinatorSetting() {
    let suiteName = "galaxyssi-voice-flags-\(UUID().uuidString)"
  ...
  func testVoiceFeatureFlagPersistsPcmCaptureSetting() {
    let suiteName = "galaxyssi-voice-pcm-flags-\(UUID().uuidString)"
    let userDefaults = UserDefaults(suiteName: suiteName)!
    defer { userDefaults.removePersistentDomain(forName: suiteName) }
  ...
  func testVoiceFeatureFlagPersistsLocalWhisperRuntimeV2Setting() {
    let suiteName = "galaxyssi-voice-whisper-runtime-flags-\(UUID().uuidString)"
    let userDefaults = UserDefaults(suiteName: suiteName)!
    defer { userDefaults.removePersistentDomain(forName: suiteName) }
  ...
  func testLocalWhisperActivationEnablesOnlyUnconfiguredCoreFlags() {
    let suiteName = "galaxyssi-voice-local-whisper-activation-\(UUID().uuidString)"
    let userDefaults = UserDefaults(suiteName: suiteName)!
    defer { userDefaults.removePersistentDomain(forName: suiteName) }
  ...
  func testVoiceFeatureFlagPersistsWhisperSecondPassSetting() {
    let suiteName = "galaxyssi-voice-whisper-second-pass-flags-\(UUID().uuidString)"
    let userDefaults = UserDefaults(suiteName: suiteName)!
    defer { userDefaults.removePersistentDomain(forName: suiteName) }
  ...
  func testVoiceFeatureFlagsPersistAdvancedASRSettings() {
    let suiteName = "galaxyssi-voice-advanced-asr-flags-\(UUID().uuidString)"
    let userDefaults = UserDefaults(suiteName: suiteName)!
    defer { userDefaults.removePersistentDomain(forName: suiteName) }
