...
final class GalaxySSIAttachmentTests: XCTestCase {
  ...
  func testRuntimePlaintextCleanupRemovesOnlyKnownTransientFiles() throws {
    ...
    let captureDirectory = root.appendingPathComponent("galaxyssi/visible-capture", isDirectory: true)
    ...
    GalaxySSIRuntimePlaintextProtection.clearKnownTemporaryFiles(roots: [root])
  ...
  func testAttachmentAtRestLifecycleRemovesLegacyPlaintextRootsOnly() throws {
    let root = FileManager.default.temporaryDirectory
      .appendingPathComponent("GalaxySSILegacyAttachmentTests-\(UUID().uuidString)", isDirectory: true)
    defer { try? FileManager.default.removeItem(at: root) }
    ...
    GalaxySSIAttachmentAtRestCipher.removeLegacyPlaintextRoots(roots: [root])
  ...
  func testAttachmentAtRestCipherAuthenticatesPurposeAndRejectsTampering() throws {
    let root = FileManager.default.temporaryDirectory
      .appendingPathComponent("GalaxySSICipherTests-\(UUID().uuidString)", isDirectory: true)
    defer { try? FileManager.default.removeItem(at: root) }
  ...
  func testAttachmentAtRestCipherMigratesLegacyPlaintext() throws {
    let root = FileManager.default.temporaryDirectory
      .appendingPathComponent("GalaxySSICipherMigrationTests-\(UUID().uuidString)", isDirectory: true)
    defer { try? FileManager.default.removeItem(at: root) }
    let cipher = GalaxySSIAttachmentAtRestCipher(secrets: InMemorySecretStore())
  ...
  func testOutgoingPeerVoiceMovesFromCacheToDurableMessageStorage() throws {
    let container = FileManager.default.temporaryDirectory
      .appendingPathComponent("GalaxySSIPeerVoiceStoreTests-\(UUID().uuidString)", isDirectory: true)
  ...
  func testOutgoingOpusVoiceEncryptsDirectlyFromMemory() throws {
    let container = FileManager.default.temporaryDirectory
      .appendingPathComponent("GalaxySSIOpusVoiceStoreTests-\(UUID().uuidString)", isDirectory: true)
  ...
  func testEncryptedPeerVoiceResolvesInMemoryWithoutPlaintextMaterialization() throws {
    let container = FileManager.default.temporaryDirectory
      .appendingPathComponent("GalaxySSIInMemoryVoiceTests-\(UUID().uuidString)", isDirectory: true)
  ...
  func testInMemoryVoiceResolutionMigratesLegacyCacheToEncryptedStorage() throws {
    let container = FileManager.default.temporaryDirectory
      .appendingPathComponent("GalaxySSILegacyMemoryVoiceTests-\(UUID().uuidString)", isDirectory: true)
    let root = container.appendingPathComponent("attachments", isDirectory: true)
    let cache = container.appendingPathComponent("cache", isDirectory: true)
    defer { try? FileManager.default.removeItem(at: container) }
    try FileManager.default.createDirectory(at: cache, withIntermediateDirectories: true)
  ...
  func testLegacyCachedPeerVoiceMigratesWhenResolvedForPlayback() throws {
    let container = FileManager.default.temporaryDirectory
      .appendingPathComponent("GalaxySSIPeerVoiceMigrationTests-\(UUID().uuidString)", isDirectory: true)
    let root = container.appendingPathComponent("files/peer-message-attachments-v1", isDirectory: true)
    let cache = container.appendingPathComponent("cache", isDirectory: true)
    defer { try? FileManager.default.removeItem(at: container) }
    try FileManager.default.createDirectory(at: cache, withIntermediateDirectories: true)
  ...
  func testCompletedIncomingAttachmentFollowsChatLifetime() {
    ...
    XCTAssertFalse(GalaxySSIPeerMessageAttachmentStore.shouldPruneIncoming(
      receivedAt: old,
      hasCompletedData: true,
      now: now,
      maximumAge: month
    ))
    XCTAssertTrue(GalaxySSIPeerMessageAttachmentStore.shouldPruneIncoming(
      receivedAt: old,
  ...
  func testAttachmentDescriptorsMatchAndroidWireNames() {
    ...
    let descriptors = GalaxySSIAttachmentPayloadBuilder.descriptors(for: [attachment])
  ...
  func testAttachmentDescriptorsRespectInlineBudget() {
    let large = GalaxySSIDraftAttachment(
      displayName: "large.bin",
      mimeType: "application/octet-stream",
      data: Data(repeating: 1, count: GalaxySSIAttachmentPayloadBuilder.maximumInlineBytes + 1)
    )

    let item = GalaxySSIAttachmentPayloadBuilder.descriptors(for: [large]).first
  ...
  func testRejectsOversizedAndTooManyAttachments() {
    let oversized = GalaxySSIDraftAttachment(
      displayName: "huge.bin",
      mimeType: "application/octet-stream",
      data: Data(repeating: 0, count: GalaxySSIAttachmentPayloadBuilder.maximumAttachmentBytes + 1)
    )
    let small = GalaxySSIDraftAttachment(displayName: "a.txt", mimeType: "text/plain", data: Data("a".utf8))
    let existing = Array(repeating: small, count: GalaxySSIAttachmentPayloadBuilder.maximumAttachmentCount)

    XCTAssertFalse(GalaxySSIAttachmentPayloadBuilder.accepted(oversized, existing: []))
    XCTAssertFalse(GalaxySSIAttachmentPayloadBuilder.accepted(small, existing: existing))
  }

  func testSanitizesUnsafeNames() {
    XCTAssertEqual(GalaxySSIAttachmentPayloadBuilder.sanitizeName(" ..bad/name?.txt "), "bad_name_.txt")
    XCTAssertEqual(GalaxySSIAttachmentPayloadBuilder.sanitizeName(""), "attachment")
  }
  ...
  func testAgentWorkspaceScopeMatchesAndroidStableIdsAndToolBinding() {
    ...
    let scopedInput = AgentWorkspaceScope.bindToolInput(
      toolId: "galaxyssi.workspace.file.read_text",
      input: ["path": "note.txt"],
      workspaceId: workspaceId
    )
    let untouchedInput = AgentWorkspaceScope.bindToolInput(
      toolId: "galaxyssi.other.tool",
      input: ["path": "note.txt"],
      workspaceId: workspaceId
    )
  ...
  func testAgentAttachmentWorkspaceStagerWritesMetadataAndFiles() throws {
    ...
    let first = GalaxySSIDraftAttachment(
      displayName: " ..bad/name?.txt ",
      mimeType: "text/plain",
      data: Data("hello".utf8)
    )
    let second = GalaxySSIDraftAttachment(
      displayName: " ..bad/name?.txt ",
      mimeType: "text/plain",
    ...
    XCTAssertEqual(staged[0].sha256, GalaxySSIAttachmentPayloadBuilder.sha256(first.data))
  ...
  func testAgentAttachmentTransferProtocolMatchesAndroidRangesAndStableIdentity() throws {
    let routeId = try GalaxySSILinkProtocol.newRouteId()
  ...
  func testPeerImageTransferProgressUpdatesRichContentMetadata() throws {
    ...
    let update = try XCTUnwrap(GalaxySSIPeerAttachmentTransferUpdate(payload: [
    ...
    XCTAssertEqual(
      GalaxySSIPeerAttachmentTransferProgress.activeProgress(metadata: updated.metadata),
      42
    )
    XCTAssertEqual(
      GalaxySSIPeerAttachmentTransferProgress.percent(receivedBytes: 999, sizeBytes: 1_000),
      99
    )
    let second = try XCTUnwrap(GalaxySSIPeerAttachmentTransferUpdate(payload: [
    ...
    XCTAssertEqual(
      AgentRichContentCodec.decode(
        GalaxySSIPeerAttachmentTransferProgress.applying(second, to: richOutput)
      ).count,
      2
    )
  }
  ...
  func testPeerImageThumbnailIdentityIgnoresProgressAndEncodingIsBounded() throws {
    var block = AgentRichBlock(
      ...
      metadata: [
        ...
        "transfer_state": GalaxySSIPeerAttachmentTransferProgress.downloading
      ]
    )
    let identity = GalaxySSIPeerImageThumbnailPolicy.cacheIdentity(block)
    block.metadata["transfer_progress"] = "100"
    block.metadata["transfer_state"] = GalaxySSIPeerAttachmentTransferProgress.complete
    XCTAssertEqual(GalaxySSIPeerImageThumbnailPolicy.cacheIdentity(block), identity)
    ...
    let thumbnail = try XCTUnwrap(
      GalaxySSIPeerImageThumbnailRepository.encodeThumbnail(png, maxPixelSize: 512)
    )
  ...
  func testIncomingPeerAttachmentAssemblesResumesAndResolvesDurably() throws {
    ...
    let localId = "galaxyssi:local"
    let remoteId = "galaxyssi:remote"
    ...
    XCTAssertEqual(
      ...
      GalaxySSIPeerAttachmentTransferProgress.downloading
    )
  ...
  func testIncomingPeerAttachmentRejectsWrongRouteAndTamperedChunk() throws {
    ...
    XCTAssertNil(
      store.ingest(
        payload: manifest,
        sourceId: "galaxyssi:remote",
        localGalaxySSIId: "galaxyssi:local",
        routes: routes
      )
    )
  ...
  func testPeerMessageCopyFallsBackToAttachmentNames() {
    ...
    XCTAssertEqual(GalaxySSIMessageActionPolicy.copyText(for: message), "photo.png\nreport.pdf")
  }

  private func temporaryAttachmentRoot() -> URL {
    FileManager.default.temporaryDirectory
      .appendingPathComponent("GalaxySSIAttachmentTests-\(UUID().uuidString)", isDirectory: true)
      .appendingPathComponent("agent-native-workspaces", isDirectory: true)
  }

  private func temporaryOutboundTransferRoot() -> URL {
    FileManager.default.temporaryDirectory
      .appendingPathComponent("GalaxySSIOutboundAttachmentTests-\(UUID().uuidString)", isDirectory: true)
      .appendingPathComponent("agent-link-outgoing-attachments-v1", isDirectory: true)
  }

  private func temporaryIncomingTransferRoot() -> URL {
    FileManager.default.temporaryDirectory
      .appendingPathComponent("GalaxySSIIncomingAttachmentTests-\(UUID().uuidString)", isDirectory: true)
