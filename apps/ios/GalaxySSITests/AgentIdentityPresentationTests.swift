...
final class AgentIdentityPresentationTests: XCTestCase {
  ...
  func testGeminiAgentUsesGeminiProviderAsset() {
    XCTAssertEqual(
      GalaxySSIAgentAvatarAssetCatalog.assetName(for: ["desktop_t14:gemini", "Gemini CLI"]),
      "CloudProviderGemini"
    )
  }
  ...
  func testUnknownContactUsesStableGeneratedIdenticon() {
    XCTAssertTrue(
      GalaxySSIContactAvatarPolicy.usesGeneratedIdenticon(
  ...
  private func contact(id: String, name: String) -> GalaxySSIContact {
    GalaxySSIContact(
      id: id,
      galaxySSIId: id,
      name: name,
