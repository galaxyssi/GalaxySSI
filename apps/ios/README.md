...
- Android-style iOS Desktop Control wire models for `desktop.*` action ids, multi-surface display/window catalogs, combined remote-control state snapshots, bounded screenshot streams, three-layer perception snapshots, durable task pause/takeover summaries, and identity-bound action receipt verification
