...
- Android-style MCP package archive inspection for signed and unsigned local packages with manifest digest checks, safe ZIP entry validation, runtime entrypoint enforcement, and iOS stored-entry activation boundaries
...
- Android-style remote MCP session client for initialize negotiation, initialized notifications, tools/list pagination, tools/call result parsing, JSON-RPC error mapping, and capability gating
...
- Android-style workspace native tool executor for app-private file and ZIP archive workflows through native tool registry calls
...
- Android-style MCP tool security and audit policy for risk assessment, permission decisions, parameter redaction, Android-wire audit records, bounded audit retention, and scoped clearing
