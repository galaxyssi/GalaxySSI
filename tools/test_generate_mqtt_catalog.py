...
class BrokerCatalogTests(unittest.TestCase):
    ...
    def test_rejects_unbounded_or_inconsistent_budgets(self):
        for mutate in [lambda value: value["limits"].update(global_inflight_packets=0),
                       ...
                       lambda value: value["timing"].update(unmeasured_path_rtt_ms=0),
                       ...
                       lambda value: value["limits"].update(per_peer_pending_receipts=2048),
                       lambda value: value["limits"].update(max_pending_receipts=8192),
                       lambda value: value["timing"].update(receipt_retry_ms=30000),
