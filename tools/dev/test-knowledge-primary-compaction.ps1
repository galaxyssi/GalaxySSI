param(
    ...
    [ValidateSet('focused', 'recovery', 'retained')][string]$Phase = 'focused',
    [string]$OutputDirectory = 'build/knowledge-compaction-runs'
)
...
$fixture = 'test-primary-compact-' + [guid]::NewGuid().ToString('N') + '.db'
$steps = if ($Phase -eq 'recovery') { @('prepare', 'before-frames', 'verify-before', 'before-catalog', 'verify-before', 'after-catalog', 'verify-after') } else { @($Phase) }
$index = 0
foreach ($step in $steps) {
    ...
    if ($Phase -eq 'focused') {
        $expected = 10
        $options += @('-e', 'class', 'com.galaxyssi.chat.KnowledgePrimaryCompactionDeviceTest')
    } elseif ($Phase -eq 'retained') {
        $options += @('-e', 'class', 'com.galaxyssi.chat.KnowledgePrimaryCompactionRetainedDeviceTest', '-e', 'retainedKnowledgeFixture',
            'test-knowledge-source-replace-621d79d8-9ce1-4d12-95b8-ebde10511a3f.db')
    } else {
        $options += @('-e', 'class', 'com.galaxyssi.chat.KnowledgePrimaryCompactionRecoveryDeviceTest',
            '-e', 'compactionFixture', $fixture, '-e', 'compactionPhase', $step)
    }
    $options += 'com.galaxyssi.chat.test/androidx.test.runner.AndroidJUnitRunner'
    Write-Output "Running compaction $step. Evidence: $directory"
    ...
    if ($step -in @('before-frames', 'before-catalog', 'after-catalog')) {
        ...
        if ($code -ne 0 -or $passed -ne $expected -or ($lines -match '^INSTRUMENTATION_STATUS_CODE: -[123]$|FAILURES!!!') -or
            !($lines -match "^OK \($expected tests?\)$")) { throw "Compaction test failed; see $directory" }
        $lines | Select-Object -Last 7 | Write-Output
    }
}
Write-Output "Verified compaction $Phase. Evidence: $directory"
