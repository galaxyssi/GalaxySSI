param(
    ...
    [string]$OutputDirectory = 'build/knowledge-read-admission-runs'
)
...
$classes = @('KnowledgeReadAdmissionDeviceTest', 'KnowledgeReadAdmissionScaleDeviceTest',
    'KnowledgeRecordCryptoDeviceTest', 'AgentKnowledgeDatabaseDeviceTest', 'KnowledgeSearchSnapshotDeviceTest',
    'AgentKnowledgeFtsDeviceTest', 'KnowledgeHybridSearchDeviceTest', 'KnowledgeRetrievalAdmissionDeviceTest',
    'KnowledgePayloadCompactionDeviceTest', 'KnowledgePayloadUsageDeviceTest', 'KnowledgePayloadDeviceTest',
    'KnowledgeSourceSnapshotDeviceTest', 'KnowledgeBackupDeviceTest')
$selection = ($classes | ForEach-Object { "com.galaxyssi.chat.$_" }) -join ','
Write-Output "Running read admission regressions. Evidence: $directory"
...
$metrics | Where-Object {
    $_ -match '^\s*(\d+)\.\d+.*KNOWLEDGE_(READ_ADMISSION|FTS)' -and [long]$Matches[1] -ge $started
...
if ($exitCode -ne 0 -or $passed -ne 86 -or $failures -ne 0 -or !($lines -contains 'OK (86 tests)')) {
    throw "Read regression failed: adb=$exitCode passed=$passed failures=$failures; see $directory"
}
$lines | Select-Object -Last 8 | Write-Output
Write-Output "Verified 86 device cases. Evidence: $directory"
