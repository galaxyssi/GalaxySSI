...
$output = Join-Path $root "build/initial-planning-$CaseId"
...
$name = "test-initial-planner-$CaseId"
$test = 'com.galaxyssi.chat.AgentInitialPlanningDeviceTest'
...
if ($Phase -eq 'all') {
    ...
    Write-Output "Preparing pending initial planning on $model"
    $prepare = Invoke-Adb -AllowFailure -Arguments @('shell', 'am', 'instrument', '-w', '-r', '-e', 'class',
        "$test#prepareAutomaticStartupRecoveryBeforeAPlanIsPublished", '-e', 'initial_planning_prepare', 'true',
        '-e', 'initial_planning_case', $CaseId, $runner)
    ...
    Write-Output 'Rebooting before opening the app normally; no task is resubmitted'
...
Write-Output 'Opening the normal app entry point and observing automatic startup recovery'
$verify = Invoke-Adb -AllowFailure -Arguments @('shell', 'am', 'instrument', '-w', '-r', '-e', 'class',
    "$test#appStartupWorkerRecoversTheOriginalPendingPlannerWithoutExplicitResubmission", '-e', 'initial_planning_verify', 'true',
    '-e', 'initial_planning_case', $CaseId, $runner)
...
if ($verify -notmatch 'OK \(1 test\)' -or $verify -match 'FAILURES!!!|INSTRUMENTATION_STATUS_CODE: -[234]') {
    throw 'Automatic recovery failed; inspect the retained case and resume only verification'
}
$newPid = Invoke-Adb @('shell', 'run-as', 'com.galaxyssi.chat', 'cat', "files/$name.verified")
if ($newPid -eq $oldPid) { throw 'Expected a new process' }
$afterBoot = Invoke-Adb @('shell', 'cat', '/proc/sys/kernel/random/boot_id')
if ($afterBoot -eq $prepared.boot) { throw 'Physical reboot not verified' }
$report = [ordered]@{
    case_id = $CaseId; serial = $Serial; device = $model; old_pid = $oldPid; new_pid = $newPid
    boot_before = $prepared.boot; boot_after = $afterBoot; automatic_startup_worker = $true
    native_read = 'memory_status'; task_resubmitted = $false; production_data_cleared = $false; passed = $true
}
